...
Status IrEmitterUnnested::EmitConvolutionThunkFromMlir(MlirEmitterInput input) {
  ...
  // TODO(nluehr): Populate descriptor cluster_name, op_name, and op_type.
...
Status VerifyBatchNormForThunkEmission(
    ...
    absl::optional<mlir::ArrayRef<NamedValue>> optional_values =
        absl::nullopt) {
  ...
  if (optional_values.has_value()) {
    for (const NamedValue& v : *optional_values) {
      if (!v.value.getType()
...
Status IrEmitterUnnested::EmitBatchNormThunkFromMlir(MlirEmitterInput input) {
  ...
  if (auto bn_train =
    ...
    absl::optional<mlir::ArrayRef<NamedValue>> optional_values = absl::nullopt;
    if (bn_train.reserve_space() && bn_train.scratch()) {
      optional_values = {{bn_train.reserve_space(), "reserve_space"},
                         {bn_train.scratch(), "workspace"}};
    }
    ...
    output_slices.push_back(output_data);
    output_slices.push_back(output_mean);
    output_slices.push_back(output_inv_stddev);
    if (bn_train.reserve_space() && bn_train.scratch()) {
      TF_ASSIGN_OR_RETURN(auto reserve_space,
                          GetAllocationSliceForMlir(bn_train.reserve_space()));
      TF_ASSIGN_OR_RETURN(auto scratch,
                          GetAllocationSliceForMlir(bn_train.scratch()));
      output_slices.push_back(reserve_space);
  ...
  if (auto bn_grad =
    ...
    if (bn_grad.reserve_space() && bn_grad.scratch()) {
      optional_values = {{bn_grad.reserve_space(), "reserve_space"},
                         {bn_grad.scratch(), "workspace"}};
    }
    TF_RETURN_IF_ERROR(VerifyBatchNormForThunkEmission(
        /*statistics_operands=*/
        {{bn_grad.scale(), "scale"},
...
Status IrEmitterUnnested::HandleAsyncOutSend(HloInstruction* hlo) {
  TF_ASSIGN_OR_RETURN(auto input, GetMlirEmitterInput(hlo));
  CHECK(
      ShapeUtil::Equal(hlo->operand(0)->shape(), hlo->async_out_send_shape()));
  return EmitAsyncOutSendFromMlir(input);
}

Status IrEmitterUnnested::EmitAsyncOutSendFromMlir(MlirEmitterInput input) {
  auto async_out_op =
      mlir::cast<mlir::lmhlo::AsyncOutSendOp>(input.op);
  TF_ASSIGN_OR_RETURN(BufferAllocation::Slice slice,
                      GetAllocationSliceForMlir(async_out_op.operand()));
  ...
  AddThunkToThunkSequence(absl::make_unique<AsyncOutSendThunk>(
      input.thunk_info, std::move(source_slice),
...
Status IrEmitterUnnested::EmitOp(MlirEmitterInput mlir_input) {
  ...
  if (mlir::isa<mlir::lmhlo::AsyncOutSendOp>(mlir_input.op)) {
    return EmitAsyncOutSendFromMlir(mlir_input);
  }
