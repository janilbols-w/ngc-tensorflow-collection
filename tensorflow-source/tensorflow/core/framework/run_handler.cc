...
RunHandlerEnvironment::EnvThread* RunHandlerEnvironment::CreateThread(
  ...
  return env_->StartThread(thread_options_, thread_name, [=]() {
    ...
    if (nvtx::detail::EnableNamingCpuThreads()) {
      string nvtx_thread_name =
          "[Tensorflow][TP RunHandlerThreadPool]" + thread_name;
      nvtxNameOsThreadA(syscall(SYS_gettid), nvtx_thread_name.c_str());
    }
    f();
  });
}
