...
StatusOr<std::unique_ptr<Converter>> Converter::Create(
    ...
    absl::string_view engine_name, bool use_explicit_precision, int dla_core,
    int dla_fallback_layers, OpKernelContext* ctx) {
  std::unique_ptr<Converter> converter = absl::WrapUnique(new Converter(
      ...
      engine_name, dla_core, dla_fallback_layers,
...
TOut safe_reinterpret(TIn const& in)
{
  static_assert(sizeof(TOut) == sizeof(TIn), "Sizes of datatypes don't match");
  TOut out;
...
Converter::Converter(TrtPrecisionMode precision_mode, bool use_calibration,
    ...
    : ctx_(ctx),
      ...
      dla_core_(dla_core),
      dla_fallback_layers_(dla_fallback_layers),
      use_explicit_precision_(use_explicit_precision) {
...
Status Converter::BuildCudaEngine(
    ...
    size_t max_workspace_size_bytes, bool enable_sparse_compute,
  ...
  static SparseComputeMode sparse_compute_mode = [](bool _enable_sparse_compute)
  {
    SparseComputeMode _sparse_compute_mode;
    int64 _sparse_benchmark_mode;
    /*This feature is only to be used for benchmarking purpose. This feature
    artificially introduces a sparse weight pattern compatible with Sparse
    TensorCores of NVIDIA Ampere GPU architecture. It will completely corrupt
    ...
    TF_CHECK_OK(tensorflow::ReadInt64FromEnvVar("TF_TRT_SPARSE_BENCHMARK_MODE",
                                                /*default_val=*/-1,
                                                &_sparse_benchmark_mode));

    string sparse_log_msg = "[TF-TRT] Sparse compute capability is ";
    if (_sparse_benchmark_mode >= 0) {
      sparse_log_msg = StrCat(sparse_log_msg, "set to benchmark mode. ",
          "Shall only be used for sparse computing benchmark and debug.");
      _sparse_compute_mode = SparseComputeMode::BENCHMARK;
    }
    else if (_enable_sparse_compute) {
  ...
  if (sparse_compute_mode == SparseComputeMode::ENABLED ||
      sparse_compute_mode == SparseComputeMode::BENCHMARK) {
  ...
  // DLA suppports FP16&INT8, but TRT int8 Calibration do not support DLA.
  if (precision_mode_ != TrtPrecisionMode::FP32 && dla_core_ >= 0) {
      if (trt_builder_->getNbDLACores() == 0) {
        LOG(ERROR) << "Trying to use DLA core " << dla_core_
                  << " on a platform that doesn't have any DLA cores" << std::endl;
      } else {
        VLOG(1) << "Use DLA Core " << dla_core_;
        builder_config->setDefaultDeviceType(nvinfer1::DeviceType::kDLA);
        builder_config->setDLACore(dla_core_);
  ...
  if (dla_core_ >= 0 && dla_fallback_layers_ >= 0) {
    VLOG(1) << "Fallback layers to GPU after " << dla_fallback_layers_ << " layers";
    for (int i = 0; i < network()->getNbLayers(); ++i)
    {
      if (i >= dla_fallback_layers_) {
          VLOG(1) << "Fallback layers:" << network()->getLayer(i)->getName();
          builder_config->setDeviceType(network()->getLayer(i), nvinfer1::DeviceType::kGPU);
      }
    }
  }
  ...
  switch(sparse_compute_mode) {
    case SparseComputeMode::BENCHMARK:
      trt_network_name = StrCat(trt_network_name, "Benchmark");
      break;
...
Status ConvertGraphDefToEngine(
    ...
    bool use_explicit_precision, int dla_core, int dla_fallback_layers,
    tensorflow::grappler::Cluster* cluster, const string& device) {
  engine->reset();
  ...
  auto statusor = Converter::Create(precision_mode, use_calibration, trt_logger,
                                    ...
                                    use_explicit_precision, dla_core,
                                    dla_fallback_layers, ctx);
  ...
  TF_RETURN_IF_ERROR(converter->BuildCudaEngine(
      engine, max_batch_size, max_workspace_size_bytes, enable_sparse_compute,
      allocator, calibrator, profiles));
