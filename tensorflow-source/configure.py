...
def set_cc_opt_flags(environ_cp):
  ...
  for opt in cc_opt_flags.split():
    ...
    write_to_bazelrc('build --host_copt=%s' % opt)
...
def set_tf_cutensor_version(environ_cp):
  """Set TF_CUTENSOR_VERSION."""
  if not is_linux():
    raise ValueError('Currently cuTENSOR is only supported on Linux platform.')

  if not int(environ_cp.get('TF_NEED_CUTENSOR', False)):
    return

  ask_cutensor_version = (
      'Please specify the cuTENSOR version you want to use. '
      '[Leave empty to default to cuTENSOR %s]: ') % _DEFAULT_CUTENSOR_VERSION
  tf_cutensor_version = get_from_env_or_user_or_default(
      environ_cp, 'TF_CUTENSOR_VERSION', ask_cutensor_version,
      _DEFAULT_CUTENSOR_VERSION)
  environ_cp['TF_CUTENSOR_VERSION'] = tf_cutensor_version
...
def validate_cuda_config(environ_cp):
  ...
  if is_linux():
    ...
    if int(environ_cp.get('TF_NEED_CUTENSOR', False)):
      cuda_libraries.append('cutensor')
  ...
  if config.get('cutensor_version', None):
    print('Found cuTENSOR %s in:' % config['cutensor_version'])
    print('    %s' % config['cutensor_library_dir'])
    print('    %s' % config['cutensor_include_dir'])
  ...
  if (environ_cp.get('TF_NEED_CUDA') == '1' and
    ...
    for _ in range(_DEFAULT_PROMPT_ASK_ATTEMPTS):
      ...
      if is_linux():
        ...
        set_tf_cutensor_version(environ_cp)
