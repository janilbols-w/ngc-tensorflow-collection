...
std::shared_ptr<Transposer> TransposerFactory::GetTransposer(
  ...
  if (IsFusedLayerNormGrad(node)) {
    // Reuse the AddNTransposer, since we need to update {0, 1} fanin edges and
    // {0} fanout edges for FusedLayerNormGrad as for AddN.
    return GetOrCreateIfNotFound<AddNTransposer>("FusedLayerNormGrad");
  }
  if (IsFusedInstanceNormGrad(node)) {
    return GetOrCreateIfNotFound<FusedBatchNormGradTransposer>(
        "FusedInstanceNormGrad");
  }
  if (IsIdentityN(node)) {
