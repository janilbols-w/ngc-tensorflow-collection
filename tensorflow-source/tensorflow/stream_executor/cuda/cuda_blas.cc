...
bool DisableReducedPrecisionReduction() {
  static bool disable_reduced_precision_reduction = [] {
    bool disable_cublas_reduced_precision_reduction = false;
    TF_CHECK_OK(tensorflow::ReadBoolFromEnvVar(
        "TF_DISABLE_REDUCED_PRECISION_REDUCTION",
        /*default_val=*/false, &disable_cublas_reduced_precision_reduction));
    return disable_cublas_reduced_precision_reduction;
  }();
...
port::Status CUDABlas::DoBlasInternalImpl(FuncT cublas_func, Stream *stream,
  ...
  if (DisableReducedPrecisionReduction()) {
    math_type = static_cast<cublasMath_t>(
        CUBLAS_MATH_DISALLOW_REDUCED_PRECISION_REDUCTION | math_type);
  }
#endif
  if (math_type != CUBLAS_DEFAULT_MATH) {
