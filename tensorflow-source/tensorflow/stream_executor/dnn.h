...
// Specifies the descriptor for a MHA model.
class MhaDescriptor {
 public:
  virtual ~MhaDescriptor() {}
  virtual size_t ParamsSizeInBytes() const { return 0; }
};

// Specifies either the inputs in a MHA model.
//
...
class MhaSeqDataDescriptor {
 public:
  virtual ~MhaSeqDataDescriptor() {}
};
...
class DnnSupport {
 public:
  ...
  // Create an MHA descriptor based on model shapes and configurations.
  // The caller retains the ownership of the descriptor.
  virtual port::StatusOr<std::unique_ptr<dnn::MhaDescriptor>>
  createMhaDescriptor(int num_heads, int batch_size, int q_size, int k_size,
                      int v_size, int q_proj_size, int k_proj_size,
                      int v_proj_size, int o_proj_size, int seq_len_q,
                      int seq_len_k, dnn::DataType data_type,
                      dnn::DataType comp_prec, float dropout_rate,
                      bool is_proj_bias, uint64 seed,
                      ScratchAllocator* state_allocator) {
    return port::Status(port::error::UNIMPLEMENTED,
                        "createMhaDescriptor is unimplemented");
  }

  virtual port::StatusOr<std::unique_ptr<dnn::MhaSeqDataDescriptor>>
  createMhaSeqDataDescriptor(int batch_size, int max_seq_length, int vect_size,
                             int seq_array_size,
                             const absl::Span<const int>& seq_array,
                             dnn::DataType data_type) {
    return port::Status(port::error::UNIMPLEMENTED,
                        "createMhaSeqDataDescriptor is unimplemented");
  }
  ...
  virtual bool DoMhaForward(Stream* stream, const dnn::MhaDescriptor& mha_desc,
  ...
  virtual bool DoMhaBackward(Stream* stream, const dnn::MhaDescriptor& mha_desc,
                             const dnn::MhaSeqDataDescriptor& input_q_desc,
                             const DeviceMemory<double>& input_q_data,
                             const dnn::MhaSeqDataDescriptor& input_k_desc,
                             const DeviceMemory<double>& input_k_data,
