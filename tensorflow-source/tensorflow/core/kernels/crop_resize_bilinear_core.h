...
template<> std::string format(const Eigen::half& val) {
  std::string str = sprint_bits(val);
  return str.substr(0,1) + " " + str.substr(1,1) + "..." + str.substr(2,4) + " " + str.substr(6,10);
}
template<> std::string format(const float& val) {
  std::string str = sprint_bits(val);
...
class SSEPrinter {
  public:
	...
	if (i == 0) printf("%.2f",(float)((T*)vals)[i]);
	else printf(", %.2f",(float)((T*)vals)[i]);
      }
      printf("}\n");
    }
...
template<> const char* SSEPrinter<uint8>::get_typename() {return "uint8";}
template<> const char* SSEPrinter<int8>::get_typename() {return "int8";}
template<> const char* SSEPrinter<uint16>::get_typename() {return "uint16";}
template<> const char* SSEPrinter<int16>::get_typename() {return "int16";}
template<> const char* SSEPrinter<int32>::get_typename() {return "int32";}
...
bool compute_single_interpolation_weight(
...
template <typename U>
U cast_to(float v, float min_val, float max_val, U u_min_val,
                 U u_max_val) {
  if (v < min_val)
    return u_min_val;
  else if (v > max_val)
...
template <>
float cast_to<float>(float v, float min_val, float max_val,
...
//
// The remaining code implements explicitly vectorized versions of a bilinear image resizer.
// Images with 1, 2, 3 or 4 channels are supported.
// The image resizer reads samples of type T and writes samples of type U.
// T and U can be any of the following: uint8, int8, uint16, int16, int32, Eigen::half, bfloat16 and float.
// There are separate codes for SSE4.1 and AVX2. Enabling AVX2 also enables FP16C instruction set,
// which contains instructions that convert between Eigen::half and float. The SSE4.1 code path emulates
...
// Input data type T must be one of uint8, int8, uint16, int16, int32, Eigen::half, bfloat16 or float.
//
...
class VectorLoader {
  public:
    ...
    // T must be one of uint8, int8, uint16, int16, int32, Eigen::half, bfloat16 or float.
...
#ifdef __AVX2__
    // pack 4 pixels with 1 channel, 2 channels and 3channels respectively in separate 128 bit lanes.
...
#ifdef __AVX2__
    // load top left and bottom left interpolation inputs into output argument left.
    // load top right and bottom right interpolation inputs into output argument right.
    // pixels have 1 channel.
    // load1 case, i.e. 4 left and right inputs are loaded with a single unaligned SSE load.
    void load1_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
	const __m128i* shuffle_masks,
	__m256& left0,
	__m256& right0
	);
    // load top left and bottom left interpolation inputs into output argument left.
	...
	__m256& right0, __m256& right1, __m256& right2
	);
    // load top left and bottom left interpolation inputs into output argument left.
    // load top right and bottom right interpolation inputs into output argument right.
	...
	int offset0,
	const __m128i* shuffle_masks,
	__m256& left0, __m256& left1, __m256& left2, __m256& left3,
	__m256& right0, __m256& right1, __m256& right2, __m256& right3
	);
    // load top left and bottom left interpolation inputs into output argument left.
    // load top right and bottom right interpolation inputs into output argument right.
    // pixels have 1 channel.
    // load2 case, i.e. 4 left inputs are loaded with first SSE load and 4 right inputs are loaded with second SSE load.
    void load2_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
    ...
    // load top right and bottom right interpolation inputs into output argument right.
    // pixels have 1 channel.
    // load4 case, i.e. each pair of left and right inputs are loaded with a separate SSE load.
    void load4_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
    ...
    // 4 pixels, each with left and right input necessitates 8 separate SSE loads per input row.
    void load8_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
    ...
    // pixels have 1 channel.
    // load1 case, i.e. all inputs for one input row are loaded with a single SSE load.
    void load1_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
	...
	int offset0,
	const __m128i* shuffle_masks,
	__m128& tl0, __m128& tl1,
	__m128& bl0, __m128& bl1,
	__m128& tr0, __m128& tr1,
	...
	int offset0,
	const __m128i* shuffle_masks,
	__m128& tl0, __m128& tl1, __m128& tl2, __m128& tl3,
	__m128& bl0, __m128& bl1, __m128& bl2, __m128& bl3,
	__m128& tr0, __m128& tr1, __m128& tr2, __m128& tr3,
	__m128& br0, __m128& br1, __m128& br2, __m128& br3
	);
    // load top left interpolation inputs into output argument tl.
    // load bottom left interpolation inputs into output argument bl.
    // load top right interpolation inputs into output argument tr.
    // load bottom right interpolation inputs into output argument br.
    // pixels have 1 channel.
    // load2 case, i.e. left inputs are loaded with first SSE load, right inputs are loaded with second SSE load.
    void load2_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
    ...
    // load bottom right interpolation inputs into output argument br.
    // pixels have 1 channel.
    // load4 case, i.e. left and right inputs are loaded with a separate SSE load for each pixel.
    void load4_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
    ...
    // load bottom right interpolation inputs into output argument br.
    // pixels have 1 channel.
    // load8 case, i.e. left and right inputs are loaded with separate SSE loads for each pixel.
    void load8_1ch(
        const T* lower_ptr,
	const T* upper_ptr,
	int offset0,
  ...
  private:
    // helper methods
#ifdef __AVX2__
    // pack 4 pixels with 1, 2, 3 or 4 channels into lower portion of SSE vector word.
...
#ifdef __AVX2__
template<class T> void VectorLoader<T>::pack4_1b_1ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  v3 = _mm256_slli_si256(v3, 3);
  __m256i and_mask = _mm256_setr_epi32(255, 0, 0, 0, 255, 0, 0, 0);
  v2 = _mm256_or_si256(v3, _mm256_slli_si256(_mm256_and_si256(and_mask, v2), 2));
  v1 = _mm256_or_si256(v2, _mm256_slli_si256(_mm256_and_si256(and_mask, v1), 1));
  v0 = _mm256_or_si256(v1, _mm256_and_si256(and_mask, v0));
}
template<class T> void VectorLoader<T>::pack4_2b_1ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  v3 = _mm256_slli_si256(v3, 6);
  __m256i and_mask = _mm256_setr_epi32(65535, 0, 0, 0, 65535, 0, 0, 0);
  v2 = _mm256_or_si256(v3, _mm256_slli_si256(_mm256_and_si256(and_mask, v2), 4));
  v1 = _mm256_or_si256(v2, _mm256_slli_si256(_mm256_and_si256(and_mask, v1), 2));
  v0 = _mm256_or_si256(v1, _mm256_and_si256(and_mask, v0));
}
template<class T> void VectorLoader<T>::pack4_4b_1ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  v3 = _mm256_slli_si256(v3, 12);
  __m256i and_mask = _mm256_setr_epi32(-1, 0, 0, 0, -1, 0, 0, 0);
  v2 = _mm256_or_si256(v3, _mm256_slli_si256(_mm256_and_si256(and_mask, v2), 8));
  v1 = _mm256_or_si256(v2, _mm256_slli_si256(_mm256_and_si256(and_mask, v1), 4));
  v0 = _mm256_or_si256(v1, _mm256_and_si256(and_mask, v0));
}

template<class T> void VectorLoader<T>::pack4_1b_2ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  __m256i and_mask = _mm256_setr_epi32(65535, 0, 0, 0, 65535, 0, 0, 0);
  v0 = _mm256_or_si256(_mm256_and_si256(v0, and_mask), _mm256_slli_si256(v1, 2));
  v1 = _mm256_or_si256(_mm256_and_si256(v2, and_mask), _mm256_slli_si256(v3, 2));
}
template<class T> void VectorLoader<T>::pack4_2b_2ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  __m256i and_mask = _mm256_setr_epi32(-1, 0, 0, 0, -1, 0, 0, 0);
  v0 = _mm256_or_si256(_mm256_and_si256(v0, and_mask), _mm256_slli_si256(v1, 4));
  v1 = _mm256_or_si256(_mm256_and_si256(v2, and_mask), _mm256_slli_si256(v3, 4));
}
template<class T> void VectorLoader<T>::pack4_4b_2ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  __m256i and_mask = _mm256_setr_epi32(-1, -1, 0, 0, -1, -1, 0, 0);
  v0 = _mm256_or_si256(_mm256_and_si256(v0, and_mask), _mm256_slli_si256(v1, 8));
  v1 = _mm256_or_si256(_mm256_and_si256(v2, and_mask), _mm256_slli_si256(v3, 8));
}

template<class T> void VectorLoader<T>::pack4_1b_3ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  __m256i and_mask = _mm256_setr_epi32(16777215, 0, 0, 0, 16777215, 0, 0, 0);
  v0 = _mm256_or_si256(_mm256_and_si256(v0, and_mask), _mm256_slli_si256(v1, 3));
  and_mask = _mm256_srli_si256(and_mask, 1);
  v1 = _mm256_or_si256(_mm256_and_si256(_mm256_srli_si256(v1, 1), and_mask), _mm256_slli_si256(v2, 2));
  and_mask = _mm256_srli_si256(and_mask, 1);
  v2 = _mm256_or_si256(_mm256_and_si256(_mm256_srli_si256(v2, 2), and_mask), _mm256_slli_si256(v3, 1));
}
template<class T> void VectorLoader<T>::pack4_2b_3ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  __m256i and_mask = _mm256_setr_epi32(-1, 65535, 0, 0, -1, 65535, 0, 0);
  v0 = _mm256_or_si256(_mm256_and_si256(v0, and_mask), _mm256_slli_si256(v1, 6));
  and_mask = _mm256_srli_si256(and_mask, 2);
  v1 = _mm256_or_si256(_mm256_and_si256(_mm256_srli_si256(v1, 2), and_mask), _mm256_slli_si256(v2, 4));
  and_mask = _mm256_srli_si256(and_mask, 2);
  v2 = _mm256_or_si256(_mm256_and_si256(_mm256_srli_si256(v2, 4), and_mask), _mm256_slli_si256(v3, 2));
}
template<class T> void VectorLoader<T>::pack4_4b_3ch_(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  __m256i and_mask = _mm256_setr_epi32(-1, -1, -1, 0, -1, -1, -1, 0);
  v0 = _mm256_or_si256(_mm256_and_si256(v0, and_mask), _mm256_slli_si256(v1, 12));
  and_mask = _mm256_srli_si256(and_mask, 4);
  v1 = _mm256_or_si256(_mm256_and_si256(_mm256_srli_si256(v1, 4), and_mask), _mm256_slli_si256(v2, 8));
  and_mask = _mm256_srli_si256(and_mask, 4);
  v2 = _mm256_or_si256(_mm256_and_si256(_mm256_srli_si256(v2, 8), and_mask), _mm256_slli_si256(v3, 4));
}

template<> void VectorLoader<uint8>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_1b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int8>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_1b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<uint16>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int16>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int32>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_4b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<Eigen::half>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<bfloat16>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<float>::pack_1ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_4b_1ch_(v0,v1,v2,v3);
}

template<> void VectorLoader<uint8>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_1b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int8>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_1b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<uint16>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int16>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int32>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_4b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<Eigen::half>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<bfloat16>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<float>::pack_2ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_4b_2ch_(v0,v1,v2,v3);
}

template<> void VectorLoader<uint8>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_1b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int8>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_1b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<uint16>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int16>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int32>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_4b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<Eigen::half>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<bfloat16>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<float>::pack_3ch(__m256i& v0, __m256i& v1, __m256i& v2, __m256i& v3){
  pack4_4b_3ch_(v0,v1,v2,v3);
}
#else
template<class T> void VectorLoader<T>::pack4_1b_1ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
...
template<class T> void VectorLoader<T>::pack4_2b_1ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
...
template<class T> void VectorLoader<T>::pack4_4b_1ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
...
template<class T> void VectorLoader<T>::pack4_1b_2ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  __m128i and_mask = _mm_setr_epi32(65535, 0, 0, 0);
...
template<class T> void VectorLoader<T>::pack4_2b_2ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  __m128i and_mask = _mm_setr_epi32(-1, 0, 0, 0);
...
template<class T> void VectorLoader<T>::pack4_4b_2ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
...
template<class T> void VectorLoader<T>::pack4_1b_3ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  ...
  v1 = _mm_or_si128(_mm_and_si128(_mm_srli_si128(v1, 1), and_mask), _mm_slli_si128(v2, 2));
  and_mask = _mm_srli_si128(and_mask, 1);
  v2 = _mm_or_si128(_mm_and_si128(_mm_srli_si128(v2, 2), and_mask), _mm_slli_si128(v3, 1));
}
template<class T> void VectorLoader<T>::pack4_2b_3ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  ...
  v1 = _mm_or_si128(_mm_and_si128(_mm_srli_si128(v1, 2), and_mask), _mm_slli_si128(v2, 4));
  and_mask = _mm_srli_si128(and_mask, 2);
  v2 = _mm_or_si128(_mm_and_si128(_mm_srli_si128(v2, 4), and_mask), _mm_slli_si128(v3, 2));
}
template<class T> void VectorLoader<T>::pack4_4b_3ch_(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  ...
  v1 = _mm_or_si128(_mm_and_si128(_mm_srli_si128(v1, 4), and_mask), _mm_slli_si128(v2, 8));
  and_mask = _mm_srli_si128(and_mask, 4);
  v2 = _mm_or_si128(_mm_and_si128(_mm_srli_si128(v2, 8), and_mask), _mm_slli_si128(v3, 4));
}

template<> void VectorLoader<uint8>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_1b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int8>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_1b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<uint16>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int16>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int32>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_4b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<Eigen::half>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<bfloat16>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_1ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<float>::pack_1ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_4b_1ch_(v0,v1,v2,v3);
}

template<> void VectorLoader<uint8>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_1b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int8>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_1b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<uint16>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int16>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int32>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_4b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<Eigen::half>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<bfloat16>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_2ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<float>::pack_2ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_4b_2ch_(v0,v1,v2,v3);
}

template<> void VectorLoader<uint8>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_1b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int8>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_1b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<uint16>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int16>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<int32>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_4b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<Eigen::half>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<bfloat16>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_2b_3ch_(v0,v1,v2,v3);
}
template<> void VectorLoader<float>::pack_3ch(__m128i& v0, __m128i& v1, __m128i& v2, __m128i& v3){
  pack4_4b_3ch_(v0,v1,v2,v3);
}
#endif
...
template<> __m256i VectorLoader<uint16>::extract_right_1ch(const __m256i& left){
...
template<> __m256i VectorLoader<Eigen::half>::extract_right_1ch(const __m256i& left){
  return extract_right_2b_(left);
}
template<> __m256i VectorLoader<bfloat16>::extract_right_1ch(const __m256i& left){
  return extract_right_2b_(left);
}
...
template<> __m256i VectorLoader<uint16>::extract_right_2ch(const __m256i& left){
  return extract_right_4b_(left);
}
...
template<> __m256i VectorLoader<Eigen::half>::extract_right_2ch(const __m256i& left){
  return extract_right_4b_(left);
}
template<> __m256i VectorLoader<bfloat16>::extract_right_2ch(const __m256i& left){
  return extract_right_4b_(left);
}
...
template<> __m256i VectorLoader<uint16>::extract_right_3ch(const __m256i& left){
...
template<> __m256i VectorLoader<Eigen::half>::extract_right_3ch(const __m256i& left){
  return extract_right_6b_(left);
}
template<> __m256i VectorLoader<bfloat16>::extract_right_3ch(const __m256i& left){
  return extract_right_6b_(left);
}
...
template<> __m256i VectorLoader<uint16>::extract_right_4ch(const __m256i& left){
  return extract_right_8b_(left);
}
...
template<> __m256i VectorLoader<Eigen::half>::extract_right_4ch(const __m256i& left){
  return extract_right_8b_(left);
}
template<> __m256i VectorLoader<bfloat16>::extract_right_4ch(const __m256i& left){
  return extract_right_8b_(left);
}
...
template<> __m128i VectorLoader<uint16>::extract_right_1ch(const __m128i& left){
  return extract_right_2b_(left);
}
...
template<> __m128i VectorLoader<Eigen::half>::extract_right_1ch(const __m128i& left){
  return extract_right_2b_(left);
}
template<> __m128i VectorLoader<bfloat16>::extract_right_1ch(const __m128i& left){
  return extract_right_2b_(left);
}
...
template<> __m128i VectorLoader<uint16>::extract_right_2ch(const __m128i& left){
  return extract_right_4b_(left);
}
...
template<> __m128i VectorLoader<Eigen::half>::extract_right_2ch(const __m128i& left){
  return extract_right_4b_(left);
}
template<> __m128i VectorLoader<bfloat16>::extract_right_2ch(const __m128i& left){
  return extract_right_4b_(left);
}
...
template<> __m128i VectorLoader<uint16>::extract_right_3ch(const __m128i& left){
  return extract_right_6b_(left);
}
...
template<> __m128i VectorLoader<Eigen::half>::extract_right_3ch(const __m128i& left){
  return extract_right_6b_(left);
}
template<> __m128i VectorLoader<bfloat16>::extract_right_3ch(const __m128i& left){
  return extract_right_6b_(left);
}
...
template<> __m128i VectorLoader<uint16>::extract_right_4ch(const __m128i& left){
  return extract_right_8b_(left);
}
...
template<> __m128i VectorLoader<Eigen::half>::extract_right_4ch(const __m128i& left){
  return extract_right_8b_(left);
}
template<> __m128i VectorLoader<bfloat16>::extract_right_4ch(const __m128i& left){
  return extract_right_8b_(left);
}
...
template<> __m256 VectorLoader<uint8>::to_fp32(__m256i raw) {
  ...
  _mm_cvtepu8_epi32(_mm256_extractf128_si256(raw, 1)), 
  1);
...
template<> __m256 VectorLoader<int8>::to_fp32(__m256i raw) {
  ...
  _mm_cvtepi8_epi32(_mm256_extractf128_si256(raw, 1)), 
  1);
  return _mm256_cvtepi32_ps(raw);
}
template<> __m256 VectorLoader<uint16>::to_fp32(__m256i raw) {
  ...
  _mm_cvtepu16_epi32(_mm256_extractf128_si256(raw, 1)), 
  1);
  return _mm256_cvtepi32_ps(raw);
}
template<> __m256 VectorLoader<int16>::to_fp32(__m256i raw) {
  ...
  _mm_cvtepi16_epi32(_mm256_extractf128_si256(raw, 1)), 
  1);
  return _mm256_cvtepi32_ps(raw);
}
...
template<> __m256 VectorLoader<Eigen::half>::to_fp32(__m256i raw) {
  ...
  _mm_cvtph_ps(_mm256_extractf128_si256(raw, 1)), 
  1);
}
template<> __m256 VectorLoader<bfloat16>::to_fp32(__m256i raw) {
  ...
  // can convert with << 16, which we fuse with initial shuffle into epi32 positions.
  __m256i shuf_hi32 = _mm256_setr_epi8(-128, -128, 0, 1, -128, -128, 2, 3, -128, -128, 4, 5, -128, -128, 6, 7, -128, -128, 0, 1, -128, -128, 2, 3, -128, -128, 4, 5, -128, -128, 6, 7);
...
template<> __m128 VectorLoader<Eigen::half>::to_fp32(__m128i raw) {
  ...
  // fp32 :: 31=sign_bit, 30-23=exponent, 22-0=mantissa :: exp zero offset is 127
  //      :: exponent of -127 (all 0) and +128 (all 1) are special numbers.
  //
  // Assuming the fp16 values is stored in the lower 16 bits of an int32 'fp16_val'.
  //
  ...
  __m128i shuf_lo32 = _mm_setr_epi8(0, 1, -128, -128, 2, 3, -128, -128, 4, 5, -128, -128, 6, 7, -128, -128);
  __m128i fp16_val = _mm_shuffle_epi8(raw, shuf_lo32);
  // ..extract sign bit
  __m128i fp32_sign = _mm_slli_epi32(_mm_and_si128(fp16_val,_mm_set1_epi32(32768)), 16);
  ...
  __m128i fp16_exponent_all1_mask = _mm_cmpeq_epi32(fp16_exponent_sl10, _mm_set1_epi32(31<<10));
  __m128i fp16_exponent_all0_mask = _mm_cmpeq_epi32(fp16_exponent_sl10, _mm_setzero_si128());
  __m128i fp16_denormal_mask = _mm_or_si128(fp16_exponent_all0_mask, fp16_exponent_all1_mask);
  __m128i fp32_exponent_before_xor = _mm_and_si128(_mm_set1_epi32(2139095040), _mm_srai_epi32(_mm_slli_epi32(fp16_exponent_sl10, 17), 4));
  __m128i fp32_exponent_xor_mask = _mm_andnot_si128(fp16_denormal_mask, _mm_set1_epi32(7<<27));
  __m128i fp32_exponent = _mm_xor_si128(fp32_exponent_xor_mask, fp32_exponent_before_xor);
  // ..or everything into one word
  __m128i fp32_val = _mm_or_si128(_mm_or_si128(fp32_sign, fp32_exponent), fp32_mantissa);
...
template<> __m128 VectorLoader<bfloat16>::to_fp32(__m128i raw) {
  // bfloat16 is essentially fp32 with mantissa truncated from 23 to 7 bits.
  // can convert with << 16, which we fuse with initial shuffle into epi32 positions.
  __m128i shuf_hi32 = _mm_setr_epi8(-128, -128, 0, 1, -128, -128, 2, 3, -128, -128, 4, 5, -128, -128, 6, 7);
...
#ifdef __AVX2__
template<class T> __m256i VectorLoader<T>::extract_right_1b_(const __m256i& left){
  return _mm256_srli_si256(left, 1);
}
template<class T> __m256i VectorLoader<T>::extract_right_2b_(const __m256i& left){
  return _mm256_srli_si256(left, 2);
}
template<class T> __m256i VectorLoader<T>::extract_right_3b_(const __m256i& left){
  return _mm256_srli_si256(left, 3);
}
template<class T> __m256i VectorLoader<T>::extract_right_4b_(const __m256i& left){
  return _mm256_srli_si256(left, 4);
}
template<class T> __m256i VectorLoader<T>::extract_right_6b_(const __m256i& left){
  return _mm256_srli_si256(left, 6);
}
template<class T> __m256i VectorLoader<T>::extract_right_8b_(const __m256i& left){
  return _mm256_srli_si256(left, 8);
}
#else
template<class T> __m128i VectorLoader<T>::extract_right_1b_(const __m128i& left){
  return _mm_srli_si128(left,1);
}
template<class T> __m128i VectorLoader<T>::extract_right_2b_(const __m128i& left){
  return _mm_srli_si128(left,2);
}
template<class T> __m128i VectorLoader<T>::extract_right_3b_(const __m128i& left){
  return _mm_srli_si128(left,3);
}
template<class T> __m128i VectorLoader<T>::extract_right_4b_(const __m128i& left){
  return _mm_srli_si128(left,4);
}
template<class T> __m128i VectorLoader<T>::extract_right_6b_(const __m128i& left){
  return _mm_srli_si128(left,6);
}
template<class T> __m128i VectorLoader<T>::extract_right_8b_(const __m128i& left){
  return _mm_srli_si128(left,8);
}
#endif

#ifdef __AVX2__
template<class T> void VectorLoader<T>::load1_1ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  __m256i raw = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  left0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[0])));
  right0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[1])));
}
template<class T> void VectorLoader<T>::load1_2ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  __m256i raw = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  left0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[0])));
  left1 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[1])));
  right0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[2])));
  right1 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[3])));
}
template<class T> void VectorLoader<T>::load1_3ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  left0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[0])));
  left1 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[1])));
  left2 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[2])));
  right0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[3])));
  right1 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[4])));
  right2 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[5])));
}
template<class T> void VectorLoader<T>::load1_4ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  left1 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[1])));
  left2 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[2])));
  left3 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[3])));
  right0 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[4])));
  right1 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[5])));
  right2 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[6])));
  right3 = to_fp32(_mm256_shuffle_epi8(raw, _mm256_broadcastsi128_si256(shuffle_masks[7])));
}
template<class T> void VectorLoader<T>::load2_1ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  __m256i raw1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i raw2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+1)), 1);
...
template<class T> void VectorLoader<T>::load2_2ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  __m256i raw1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i raw2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+2)), 1);
  __m256i mask = _mm256_broadcastsi128_si256(shuffle_masks[0]);
  left0 = to_fp32(_mm256_shuffle_epi8(raw1, mask));
  right0 = to_fp32(_mm256_shuffle_epi8(raw2, mask));
...
template<class T> void VectorLoader<T>::load2_3ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  __m256i raw1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i raw2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+3)), 1);
  __m256i mask = _mm256_broadcastsi128_si256(shuffle_masks[0]);
  left0 = to_fp32(_mm256_shuffle_epi8(raw1, mask));
  right0 = to_fp32(_mm256_shuffle_epi8(raw2, mask));
  mask = _mm256_broadcastsi128_si256(shuffle_masks[1]);
...
template<class T> void VectorLoader<T>::load2_4ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    const __m128i* shuffle_masks,
  ...
  __m256i raw1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i raw2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+4))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+4)), 1);
  __m256i mask = _mm256_broadcastsi128_si256(shuffle_masks[0]);
  left0 = to_fp32(_mm256_shuffle_epi8(raw1, mask));
  right0 = to_fp32(_mm256_shuffle_epi8(raw2, mask));
  mask = _mm256_broadcastsi128_si256(shuffle_masks[1]);
...
template<class T> void VectorLoader<T>::load4_1ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
  ...
  __m256i l0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  ...
  if (offset1 == offset0) {
    ...
    l1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1)), 1);
  ...
  if (offset2 == offset1) {
    ...
    l2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2)), 1);
  ...
  if (offset3 == offset2) {
    ...
    l3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3)), 1);
...
template<class T> void VectorLoader<T>::load4_2ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
...
template<class T> void VectorLoader<T>::load4_3ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
...
template<class T> void VectorLoader<T>::load4_4ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
...
template<class T> void VectorLoader<T>::load8_1ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
  ...
  __m256i l0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i r0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+1)), 1);
  __m256i l1, r1;
  if (offset1 == offset0) {
    l1 = l0;
    r1 = r0;
    ...
    l1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1)), 1);
    r1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1+1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1+1)), 1);
  }
  __m256i l2, r2;
  if (offset2 == offset1) {
    l2 = l1;
    ...
    l2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2)), 1);
    r2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2+1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2+1)), 1);
  }
  __m256i l3, r3;
  if (offset3 == offset2) {
    l3 = l2;
    ...
    l3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3)), 1);
    r3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3+1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3+1)), 1);
  }
  pack_1ch(l0,l1,l2,l3);
  left0 = to_fp32(l0);
  pack_1ch(r0,r1,r2,r3);
...
template<class T> void VectorLoader<T>::load8_2ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
  ...
  __m256i l0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i r0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+2)), 1);
  __m256i l1, r1;
  if (offset1 == offset0) {
    l1 = l0;
    r1 = r0;
    ...
    l1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1)), 1);
    r1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1+2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1+2)), 1);
  }
  __m256i l2, r2;
  if (offset2 == offset1) {
    l2 = l1;
    ...
    l2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2)), 1);
    r2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2+2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2+2)), 1);
  }
  __m256i l3, r3;
  if (offset3 == offset2) {
    l3 = l2;
    ...
    l3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3)), 1);
    r3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3+2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3+2)), 1);
  }
  pack_2ch(l0,l1,l2,l3);
  left0 = to_fp32(l0);
  left1 = to_fp32(l1);
...
template<class T> void VectorLoader<T>::load8_3ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
  ...
  __m256i l0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i r0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+3)), 1);
  __m256i l1, r1;
  if (offset1 == offset0) {
    l1 = l0;
    r1 = r0;
    ...
    l1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1)), 1);
    r1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1+3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1+3)), 1);
  }
  __m256i l2, r2;
  if (offset2 == offset1) {
    l2 = l1;
    ...
    l2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2)), 1);
    r2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2+3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2+3)), 1);
  }
  __m256i l3, r3;
  if (offset3 == offset2) {
    l3 = l2;
    ...
    l3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3)), 1);
    r3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3+3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3+3)), 1);
  }
  pack_3ch(l0,l1,l2,l3);
  left0 = to_fp32(l0);
  left1 = to_fp32(l1);
...
template<class T> void VectorLoader<T>::load8_4ch(
    const T* lower_ptr,
    const T* upper_ptr,
    int offset0,
    int offset1,
  ...
  __m256i l0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0)), 1);
  __m256i r0  = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset0+4))), _mm_loadu_si128((__m128i*)(upper_ptr+offset0+4)), 1);
  __m256i l1, r1;
  if (offset1 == offset0) {
    l1 = l0;
    r1 = r0;
    ...
    l1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1)), 1);
    r1 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset1+4))), _mm_loadu_si128((__m128i*)(upper_ptr+offset1+4)), 1);
  }
  __m256i l2, r2;
  if (offset2 == offset1) {
    l2 = l1;
    ...
    l2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2)), 1);
    r2 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset2+4))), _mm_loadu_si128((__m128i*)(upper_ptr+offset2+4)), 1);
  }
  __m256i l3, r3;
  if (offset3 == offset2) {
    l3 = l2;
    ...
    l3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3)), 1);
    r3 = _mm256_insertf128_si256(_mm256_castsi128_si256(_mm_loadu_si128((__m128i*)(lower_ptr+offset3+4))), _mm_loadu_si128((__m128i*)(upper_ptr+offset3+4)), 1);
  }
  left0 = to_fp32(l0);
  left1 = to_fp32(l1);
  left2 = to_fp32(l2);
...
// Output type U must be one of uint8, int8, uint16, int16, int32, Eigen::half, bfloat16 or float.
//
...
class VectorWriter {
  public:
    ...
    // U must be one of uint8, int8, uint16, int16, int32, Eigen::half, bfloat16 or float.
  ...
  private:
    ...
    void write_1b_2ch(U* destination, __m128* vec) {
      ...
      ivec1 = _mm_or_si128(_mm_and_si128(mask, ivec1), _mm_slli_si128(_mm_and_si128(mask, ivec2), 4));
    ...
    void write_2b_2ch(U* destination, __m128* vec) {
      ...
      ivec1 = _mm_or_si128(_mm_and_si128(mask, ivec1), _mm_slli_si128(_mm_and_si128(mask, ivec2), 8));
...
template<> __m128i VectorWriter<Eigen::half>::from_fp32(__m128 vec) {
  ...
  __m128i fp32_exponent_lsb = _mm_and_si128(_mm_set1_epi32(255), _mm_srli_epi32(hw, 23));
  __m128i fp32_mantissa = _mm_and_si128(_mm_set1_epi32(8388607), hw);
  // ..test for NaN
  __m128i exponent_ones = _mm_cmpeq_epi32(fp32_exponent_lsb, _mm_set1_epi32(255));
  ...
  __m128i fp16_exponent_lsb = _mm_sub_epi32(fp32_exponent_lsb, _mm_set1_epi32(112));
  // ..clip output if fp16_exponent > 30
  __m128i saturated_mask = _mm_andnot_si128(exponent_ones, _mm_cmpgt_epi32(fp16_exponent_lsb, _mm_set1_epi32(30)));
  // ..generate subnormal number if fp16_exponent == 0
  // ..flush to zero if fp16_exponent < 0
  __m128i subnormal_mask = _mm_cmpeq_epi32(fp16_exponent_lsb, _mm_setzero_si128());
  __m128i underflow_mask = _mm_cmplt_epi32(fp16_exponent_lsb, _mm_setzero_si128());
  __m128i fp16_mantissa = _mm_srli_epi32(fp32_mantissa, 13);
  // ..handle abnormal values
  __m128i normal_number = _mm_or_si128(_mm_slli_epi32(fp16_exponent_lsb,10), fp16_mantissa);
  __m128i subnormal_number = _mm_or_si128(_mm_set1_epi32(512), _mm_srli_epi32(fp16_mantissa,1));
  ...
  number = _mm_or_si128(_mm_andnot_si128(subnormal_mask, number), _mm_and_si128(subnormal_mask, subnormal_number));
  number = _mm_or_si128(_mm_andnot_si128(saturated_mask, number), _mm_and_si128(saturated_mask, saturated_number));
  number = _mm_or_si128(_mm_andnot_si128(infinity_mask, number), _mm_and_si128(infinity_mask, infinity_number));
  number = _mm_or_si128(_mm_andnot_si128(NaN_mask, number), _mm_and_si128(NaN_mask, NaN_number));
  // ..or in sign bit
  number = _mm_or_si128(fp16_sign_bit_msb, _mm_slli_epi32(number, 16));
  // ..move 16 bit words to lower portion of sse vector;
  __m128i shuf_from_hi32 = _mm_setr_epi8(   2,    3,    6,    7,   10,   11,   14,   15, -128, -128, -128, -128, -128, -128, -128, -128);
...
template<> __m128i VectorWriter<bfloat16>::from_fp32(__m128 vec) {
  ...
  // we do this with a shuffle that also moves everything to lower portion of sse vector word
  __m128i shuf_from_hi32 = _mm_setr_epi8(   2,    3,    6,    7,   10,   11,   14,   15, -128, -128, -128, -128, -128, -128, -128, -128);
...
template<> void VectorWriter<uint8>::write_1ch(uint8* destination, __m128* vec) {
  write_1b_1ch(destination, vec);
}
template<> void VectorWriter<int8>::write_1ch(int8* destination, __m128* vec) {
  write_1b_1ch(destination, vec);
}
template<> void VectorWriter<uint16>::write_1ch(uint16* destination, __m128* vec) {
  write_2b_1ch(destination, vec);
}
template<> void VectorWriter<int16>::write_1ch(int16* destination, __m128* vec) {
  write_2b_1ch(destination, vec);
}
template<> void VectorWriter<int32>::write_1ch(int32* destination, __m128* vec) {
  write_4b_1ch(destination, vec);
}
template<> void VectorWriter<Eigen::half>::write_1ch(Eigen::half* destination, __m128* vec) {
  write_2b_1ch(destination, vec);
}
template<> void VectorWriter<bfloat16>::write_1ch(bfloat16* destination, __m128* vec) {
  write_2b_1ch(destination, vec);
}
template<> void VectorWriter<float>::write_1ch(float* destination, __m128* vec) {
  _mm_storeu_si128((__m128i*)(destination), _mm_castps_si128(vec[0]));
}

template<> void VectorWriter<uint8>::write_2ch(uint8* destination, __m128* vec) {
  write_1b_2ch(destination, vec);
}
template<> void VectorWriter<int8>::write_2ch(int8* destination, __m128* vec) {
  write_1b_2ch(destination, vec);
}
template<> void VectorWriter<uint16>::write_2ch(uint16* destination, __m128* vec) {
  write_2b_2ch(destination, vec);
}
template<> void VectorWriter<int16>::write_2ch(int16* destination, __m128* vec) {
  write_2b_2ch(destination, vec);
}
template<> void VectorWriter<int32>::write_2ch(int32* destination, __m128* vec) {
  write_4b_2ch(destination, vec);
}
template<> void VectorWriter<Eigen::half>::write_2ch(Eigen::half* destination, __m128* vec) {
  write_2b_2ch(destination, vec);
}
template<> void VectorWriter<bfloat16>::write_2ch(bfloat16* destination, __m128* vec) {
  write_2b_2ch(destination, vec);
}
template<> void VectorWriter<float>::write_2ch(float* destination, __m128* vec) {
  _mm_storeu_si128((__m128i*)(destination), _mm_castps_si128(vec[0]));
  _mm_storeu_si128((__m128i*)(destination+4), _mm_castps_si128(vec[1]));
}

template<> void VectorWriter<uint8>::write_3ch(uint8* destination, __m128* vec) {
  write_1b_3ch(destination, vec);
}
template<> void VectorWriter<int8>::write_3ch(int8* destination, __m128* vec) {
  write_1b_3ch(destination, vec);
}
template<> void VectorWriter<uint16>::write_3ch(uint16* destination, __m128* vec) {
  write_2b_3ch(destination, vec);
}
template<> void VectorWriter<int16>::write_3ch(int16* destination, __m128* vec) {
  write_2b_3ch(destination, vec);
}
template<> void VectorWriter<int32>::write_3ch(int32* destination, __m128* vec) {
  write_4b_3ch(destination, vec);
}
template<> void VectorWriter<Eigen::half>::write_3ch(Eigen::half* destination, __m128* vec) {
  write_2b_3ch(destination, vec);
}
template<> void VectorWriter<bfloat16>::write_3ch(bfloat16* destination, __m128* vec) {
  write_2b_3ch(destination, vec);
}
template<> void VectorWriter<float>::write_3ch(float* destination, __m128* vec) {
  _mm_storeu_si128((__m128i*)(destination), _mm_castps_si128(vec[0]));
  _mm_storeu_si128((__m128i*)(destination+4), _mm_castps_si128(vec[1]));
  _mm_storeu_si128((__m128i*)(destination+8), _mm_castps_si128(vec[2]));
}

template<> void VectorWriter<uint8>::write_4ch(uint8* destination, __m128* vec) {
  write_1b_4ch(destination, vec);
}
template<> void VectorWriter<int8>::write_4ch(int8* destination, __m128* vec) {
  write_1b_4ch(destination, vec);
}
template<> void VectorWriter<uint16>::write_4ch(uint16* destination, __m128* vec) {
  write_2b_4ch(destination, vec);
}
template<> void VectorWriter<int16>::write_4ch(int16* destination, __m128* vec) {
  write_2b_4ch(destination, vec);
}
template<> void VectorWriter<int32>::write_4ch(int32* destination, __m128* vec) {
  write_4b_4ch(destination, vec);
}
template<> void VectorWriter<Eigen::half>::write_4ch(Eigen::half* destination, __m128* vec) {
  write_2b_4ch(destination, vec);
}
template<> void VectorWriter<bfloat16>::write_4ch(bfloat16* destination, __m128* vec) {
  write_2b_4ch(destination, vec);
}
template<> void VectorWriter<float>::write_4ch(float* destination, __m128* vec) {
  _mm_storeu_si128((__m128i*)(destination), _mm_castps_si128(vec[0]));
  _mm_storeu_si128((__m128i*)(destination+4), _mm_castps_si128(vec[1]));
  _mm_storeu_si128((__m128i*)(destination+8), _mm_castps_si128(vec[2]));
...
class CropResizeCastImage : public VectorLoader<T>, public VectorWriter<U> {
  public:
	...
	const int allowed_load_groups = 15) : 
      ...
      channels_(channels),
      min_ix_(min_ix), max_ix_(max_ix),
      min_iy_(min_iy), max_iy_(max_iy), ys_(ys),
	...
	Configure_();
	loader_ = new VectorLoader<T>();
	writer_ = new VectorWriter<U>();
	vec_ = new __m128[20];
	tl_ = vec_;
	tr_ = tl_ + 4;
	bl_ = tr_ + 4;
  ...
  private:
    ...
    // helper objects
    VectorLoader<T> *loader_;
    VectorWriter<U> *writer_;
    __m128 *vec_, *tl_, *tr_, *bl_, *br_, *res_;

    // helper methods
    void ResizeRow_load1_1ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load2_1ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load4_1ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load8_1ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load1_2ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load2_2ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load4_2ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load8_2ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load1_3ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load2_3ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load4_3ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load8_3ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load1_4ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load2_4ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load4_4ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_load8_4ch_(const __m128 y_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    void ResizeRow_general_(const float ys_lerp, const T* ysA_input_lower_ptr, const T* ysA_input_upper_ptr, U* ysA_output_ptr);
    ...
    __m128 *load1_mmxs_lerp_, *load2_mmxs_lerp_, *load4_mmxs_lerp_, *load8_mmxs_lerp_;
    ...
    bool Load1_ok_(const int min_xidx, const int max_xidx);  // xs - pointer to first xs for this load group
    bool Load2_ok_(const int min_xidx, const int max_xidx);  // xs - pointer to first xs for this load group
...
void CropResizeCastImage<T,U>::Resize(const T* input_image, U* output_image) {
  ...
  // 
  U uEx =
      cast_to<U>(extrapolated_value_, _f_min_val, _f_max_val, _u_min_val, _u_max_val);
  // extrapolate top
  if (min_iy_ > 0) {
    U* p = flip_y_ ? output_image + out_row_size_ * (out_height_ - min_iy_) : output_image;
    int nn = out_row_size_ * min_iy_;
    for (int i = 0; i < nn; ++i) p[i] = uEx;
  }
  // extrapolate bottom
  if (max_iy_ < out_height_ - 1) {
    U* p = flip_y_ ? output_image : output_image + out_row_size_ * (max_iy_ + 1);
  ...
  for (y = y0_;  y+1 <= y1_;  y+=2) {
    ...
    const T* ysA_input_lower_ptr = input_image + ys_[iyA].lower * in_width_ * channels_;
    const T* ysA_input_upper_ptr = input_image + ys_[iyA].upper * in_width_ * channels_;
    U* ysA_output_ptr = output_image + y * out_width_ * channels_;
    const int iyB = flip_y_ ? out_height_ - 1 - min_iy_ - (y+1) : (y+1) - min_iy_;
    const float yB_lerp = ys_[iyB].lerp;
    const __m128 ysB_lerp = _mm_set1_ps(yB_lerp);
    const T* ysB_input_lower_ptr = input_image + ys_[iyB].lower * in_width_ * channels_;
    const T* ysB_input_upper_ptr = input_image + ys_[iyB].upper * in_width_ * channels_;
    U* ysB_output_ptr = output_image + (y+1) * out_width_ * channels_;
    if (channels_ == 1) {
      this->ResizeRow_load1_1ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load1_1ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load2_1ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load2_1ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load4_1ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load4_1ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load8_1ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load8_1ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_general_(yA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_general_(yB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
    } else if (channels_ == 2) {
      this->ResizeRow_load1_2ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load1_2ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load2_2ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load2_2ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load4_2ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load4_2ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load8_2ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load8_2ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_general_(yA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_general_(yB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
    } else if (channels_ == 3) {
      this->ResizeRow_load1_3ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load1_3ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load2_3ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load2_3ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load4_3ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load4_3ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load8_3ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load8_3ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_general_(yA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_general_(yB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
    } else if (channels_ == 4) {
      this->ResizeRow_load1_4ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load1_4ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load2_4ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load2_4ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load4_4ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load4_4ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_load8_4ch_(ysA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_load8_4ch_(ysB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
      this->ResizeRow_general_(yA_lerp,ysA_input_lower_ptr,ysA_input_upper_ptr,ysA_output_ptr);
      this->ResizeRow_general_(yB_lerp,ysB_input_lower_ptr,ysB_input_upper_ptr,ysB_output_ptr);
    } else {
      assert( false );
    }
    //printf("*2 :: y=%d, channels_=%d, num_load8_=%d\n",y,channels_,num_load8_);
  }
  for (;  y <= y1_;  ++y) {
    y_ = y;
    const int iyA = flip_y_ ? out_height_ - 1 - min_iy_ - y : y - min_iy_;
...
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_general_(const float ys_lerp, const T* ys_input_lower_ptr, const T* ys_input_upper_ptr, U* output_y_ptr) {
...
#define CHANNELS 1
// Resize all points that fall in the 'load4from1' group for an entire row of a 1 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load1_1ch_(
  ...
  for (int current = 0;  current < num_load1_;  ++current) {
    __m128* mmxs_lerp = (__m128*)(load1_shuffle_masks_+current*CHANNELS*3);
    __m128i* shuffle_masks = (__m128i*)mmxs_lerp + CHANNELS;
#ifdef __AVX2__
    __m256 left0, right0;
    this->load1_1ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load1_offsets_[current],shuffle_masks,
	left0,right0);

    __m256 x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[0])));
    __m256 hori0 = _mm256_add_ps(left0, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right0, left0)));
...
#endif
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    this->write_1ch(ysA_output_ptr+load1_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from2' group for an entire row of a 1 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load2_1ch_(
  ...
  for (int current = 0;  current < num_load2_;  ++current) {
    __m128* mmxs_lerp = (__m128*)(load2_shuffle_masks_+current*CHANNELS*2);
    __m128i* shuffle_masks = (__m128i*)mmxs_lerp + CHANNELS;
#ifdef __AVX2__
    __m256 left0, right0;
    this->load2_1ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load2_offsets_[current],shuffle_masks,
	left0,right0);
...
#endif
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    this->write_1ch(ysA_output_ptr+load2_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from4' group for an entire row of a 1 channel image.
template<class T, class U>
...
#ifdef __AVX2__
    __m256 left0, right0;
    this->load4_1ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load4_offsets_[current*4],load4_offsets_[current*4+1],load4_offsets_[current*4+2],load4_offsets_[current*4+3],
	left0,right0);
...
#endif
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    this->write_1ch(ysA_output_ptr+load4_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from8' group for an entire row of a 1 channel image.
template<class T, class U>
...
#ifdef __AVX2__
    __m256 left0, right0;
    this->load8_1ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load8_offsets_[current*4],load8_offsets_[current*4+1],load8_offsets_[current*4+2],load8_offsets_[current*4+3],
	left0,right0);
...
#endif
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    this->write_1ch(ysA_output_ptr+load8_x_[current]*CHANNELS,res_);
  }
}
#undef CHANNELS

#define CHANNELS 2
// Resize all points that fall in the 'load4from1' group for an entire row of a 2 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load1_2ch_(
  ...
  for (int current = 0;  current < num_load1_;  ++current) {
    ...
    __m256 x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[0])));
    __m256 hori0 = _mm256_add_ps(left0, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right0, left0)));
    x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[1])));
    __m256 hori1 = _mm256_add_ps(left1, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right1, left1)));
    ...
    this->load1_2ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load1_offsets_[current],shuffle_masks,
	tl0,tl1,bl0,bl1,tr0,tr1,br0,br1);
...
#endif
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    this->write_2ch(ysA_output_ptr+load1_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from2' group for an entire row of a 2 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load2_2ch_(
  ...
  for (int current = 0;  current < num_load2_;  ++current) {
    ...
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    this->write_2ch(ysA_output_ptr+load2_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from4' group for an entire row of a 2 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load4_2ch_(
  ...
  for (int current = 0;  current < num_load4_;  ++current) {
...
#ifdef __AVX2__
    __m256 left0, left1, right0, right1;
    this->load4_2ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load4_offsets_[current*4],load4_offsets_[current*4+1],load4_offsets_[current*4+2],load4_offsets_[current*4+3],
	left0,left1,right0,right1);
    ...
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    this->write_2ch(ysA_output_ptr+load4_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from8' group for an entire row of a 2 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load8_2ch_(
  ...
  for (int current = 0;  current < num_load8_;  ++current) {
...
#ifdef __AVX2__
    __m256 left0, left1, right0, right1;
    this->load8_2ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load8_offsets_[current*4],load8_offsets_[current*4+1],load8_offsets_[current*4+2],load8_offsets_[current*4+3],
	left0,left1,right0,right1);
    ...
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    this->write_2ch(ysA_output_ptr+load8_x_[current]*CHANNELS,res_);
  }
}
#undef CHANNELS

#define CHANNELS 3
// Resize all points that fall in the 'load4from1' group for an entire row of a 3 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load1_3ch_(
  ...
  for (int current = 0;  current < num_load1_;  ++current) {
    ...
    x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[1])));
    __m256 hori1 = _mm256_add_ps(left1, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right1, left1)));
    x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[2])));
    __m256 hori2 = _mm256_add_ps(left2, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right2, left2)));
    ...
    this->load1_3ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load1_offsets_[current],shuffle_masks,
	tl0,tl1,tl2,bl0,bl1,bl2,tr0,tr1,tr2,br0,br1,br2);
    ...
    res_[0] = _mm_add_ps(top0, _mm_mul_ps(y_lerp, _mm_sub_ps(bot0, top0)));
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    this->write_3ch(ysA_output_ptr+load1_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from2' group for an entire row of a 3 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load2_3ch_(
  ...
  for (int current = 0;  current < num_load2_;  ++current) {
    ...
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    this->write_3ch(ysA_output_ptr+load2_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from4' group for an entire row of a 3 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load4_3ch_(
  ...
  for (int current = 0;  current < num_load4_;  ++current) {
...
#ifdef __AVX2__
    __m256 left0, left1, left2, right0, right1, right2;
    this->load4_3ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load4_offsets_[current*4],load4_offsets_[current*4+1],load4_offsets_[current*4+2],load4_offsets_[current*4+3],
	left0,left1,left2,right0,right1,right2);
    ...
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    this->write_3ch(ysA_output_ptr+load4_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from8' group for an entire row of a 3 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load8_3ch_(
  ...
  for (int current = 0;  current < num_load8_;  ++current) {
...
#ifdef __AVX2__
    __m256 left0, left1, left2, right0, right1, right2;
    this->load8_3ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load8_offsets_[current*4],load8_offsets_[current*4+1],load8_offsets_[current*4+2],load8_offsets_[current*4+3],
	left0,left1,left2,right0,right1,right2);
    ...
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    this->write_3ch(ysA_output_ptr+load8_x_[current]*CHANNELS,res_);
  }
}
#undef CHANNELS

#define CHANNELS 4
// Resize all points that fall in the 'load4from1' group for an entire row of a 4 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load1_4ch_(
  ...
  for (int current = 0;  current < num_load1_;  ++current) {
    ...
    x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[2])));
    __m256 hori2 = _mm256_add_ps(left2, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right2, left2)));
    x_lerp = _mm256_castsi256_ps(_mm256_broadcastsi128_si256(_mm_castps_si128(mmxs_lerp[3])));
    __m256 hori3 = _mm256_add_ps(left3, _mm256_mul_ps(x_lerp, _mm256_sub_ps(right3, left3)));
...
#else
    __m128 tl0,tl1,tl2,tl3,bl0,bl1,bl2,bl3,tr0,tr1,tr2,tr3,br0,br1,br2,br3;
    this->load1_4ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load1_offsets_[current],shuffle_masks,
	tl0,tl1,tl2,tl3,bl0,bl1,bl2,bl3,tr0,tr1,tr2,tr3,br0,br1,br2,br3);
    ...
    res_[1] = _mm_add_ps(top1, _mm_mul_ps(y_lerp, _mm_sub_ps(bot1, top1)));
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    res_[3] = _mm_add_ps(top3, _mm_mul_ps(y_lerp, _mm_sub_ps(bot3, top3)));
    this->write_4ch(ysA_output_ptr+load1_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from2' group for an entire row of a 4 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load2_4ch_(
  ...
  for (int current = 0;  current < num_load2_;  ++current) {
    ...
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    res_[3] = _mm_add_ps(top3, _mm_mul_ps(y_lerp, _mm_sub_ps(bot3, top3)));
    this->write_4ch(ysA_output_ptr+load2_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from4' group for an entire row of a 4 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load4_4ch_(
  ...
  for (int current = 0;  current < num_load4_;  ++current) {
...
#ifdef __AVX2__
    __m256 left0, left1, left2, left3, right0, right1, right2, right3;
    this->load4_4ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load4_offsets_[current*4],load4_offsets_[current*4+1],load4_offsets_[current*4+2],load4_offsets_[current*4+3],
	left0,left1,left2,left3,right0,right1,right2,right3);
    ...
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    res_[3] = _mm_add_ps(top3, _mm_mul_ps(y_lerp, _mm_sub_ps(bot3, top3)));
    this->write_4ch(ysA_output_ptr+load4_x_[current]*CHANNELS,res_);
  }
}
// Resize all points that fall in the 'load4from8' group for an entire row of a 4 channel image.
template<class T, class U>
void CropResizeCastImage<T,U>::ResizeRow_load8_4ch_(
  ...
  for (int current = 0;  current < num_load8_;  ++current) {
...
#ifdef __AVX2__
    __m256 left0, left1, left2, left3, right0, right1, right2, right3;
    this->load8_4ch(ysA_input_lower_ptr,ysA_input_upper_ptr,
	load8_offsets_[current*4],load8_offsets_[current*4+1],load8_offsets_[current*4+2],load8_offsets_[current*4+3],
	left0,left1,left2,left3,right0,right1,right2,right3);
    ...
    res_[2] = _mm_add_ps(top2, _mm_mul_ps(y_lerp, _mm_sub_ps(bot2, top2)));
    res_[3] = _mm_add_ps(top3, _mm_mul_ps(y_lerp, _mm_sub_ps(bot3, top3)));
    this->write_4ch(ysA_output_ptr+load8_x_[current]*CHANNELS,res_);
  }
}
#undef CHANNELS
...
void CropResizeCastImage<T,U>::Configure_() {
  ...
  for (int x = x0_; x <= x1_; ++x) {
    ...
    printf("x = %d => in_x=[%d,%d], load_group=%d\n",x,min_xidx,max_xidx,load_group);
    */
  ...
  for (int x = x0_; x <= x1_; ++x) {
    ...
    if (load_group == 0) {
      ...
      char* shufmasks1 = (char*)(load1_shuffle_masks_ + current * channels_ * 3 + channels_);
      char* shufmasks2 = shufmasks1 + 16 * channels_;
      for (int j = 0;  j < 32*channels_;  ++j) shufmasks1[j] = -128;
      for (int pix = 0;  pix < 4;  ++pix) {
	const int ix = flip_x_ ? out_width_ - 1 - min_ix_ - (x+pix) : (x+pix) - min_ix_;
	float lerp = xs_[ix].lerp;
	int widx0 = xs_[ix].lower - load1_offsets_[current];  // word index within SSE vector
	...
	for (int ch = 0;  ch < channels_;  ++ch) {
	  ...
	  int shufidx = idx%4;
	  int widx = widx0 + ch;
	  //printf(" ....... ch=%d, idx=%d, shufvec=%d, shufidx=%d, widx=%d, shufmasks1[%ld...]=...\n",ch,idx,shufvec,shufidx,widx,shufvec*16+shufidx*sizeof(T));
	  for (int b = 0;  b < sizeof(T);  ++b) {
	    shufmasks1[shufvec*16+shufidx*sizeof(T)+b] = widx*sizeof(T) + b;
	    shufmasks2[shufvec*16+shufidx*sizeof(T)+b] = (widx+channels_)*sizeof(T) + b;
	  }
	}
      }
      ...
      char* shufmasks1 = (char*)(load2_shuffle_masks_ + current * channels_ * 2 + channels_);
      for (int j = 0;  j < 16*channels_;  ++j) shufmasks1[j] = -128;
      for (int pix = 0;  pix < 4;  ++pix) {
	const int ix = flip_x_ ? out_width_ - 1 - min_ix_ - (x+pix) : (x+pix) - min_ix_;
	float lerp = xs_[ix].lerp;
	int widx0 = xs_[ix].lower - load2_offsets_[current];  // word index within SSE vector
	for (int ch = 0;  ch < channels_;  ++ch) {
	  int idx = pix*channels_+ch;
	  xs_lerp[idx] = lerp;
	  int shufvec = idx/4;
	...
	for (int ch = 0;  ch < channels_;  ++ch) {
      ...
      /* debug
      printf("load4from4_%dch :: x=%d - index={%ld",channels_,x,index[0]*sizeof(T));
	...
	for (int ch = 0;  ch < channels_;  ++ch) {
      ...
      for (int pix = 1;  pix < 4;  ++pix) printf(",%d",load8_offsets_[current*4+pix]);
      printf("}");
      for (int ch = 0;  ch < channels_;  ++ch) {
...
int CropResizeCastImage<T,U>::DetermineLoadGroup_(const int x) {
  ...
  if (num_remaining >= 4) {
    ...
    // "there are no arguments to ‘xxx’ that depend on a template parameter, so a declaration of ‘xxx’ must be available"
    // This is an issue for all member functions that have only builtin type arguments and happens because
    // argument dependent lookup is not done for these arguments (so I've been told).
    if (this->ComputeXIndexRange_(x,min_xidx,max_xidx)) {
      if ((allowed_load_groups_ & 1) && this->Load1_ok_(min_xidx,max_xidx)) {
	return 1;
      } else if ((allowed_load_groups_ & 2) && this->Load2_ok_(min_xidx,max_xidx)) {
	return 2;
      } else if ((allowed_load_groups_ & 4) && this->Load4_ok_(min_xidx,max_xidx)) {
	return 3;
      } else if ((allowed_load_groups_ & 8) && this->Load8_ok_(min_xidx,max_xidx)) {
	return 4;
      } else {
	return 0;
      }
    } else {
      // assumption xs[i].lower + channels == xs[i].upper NOT true for this quintuple.
      return 0;
    }
  } else {
...
template<class T, class U>
bool CropResizeCastImage<T,U>::ComputeXIndexRange_(const int x, int& min_xidx, int& max_xidx) {
  ...
  for (int pix = 0;  pix < 4;  ++pix) {
	...
	if (curr_xidx > max_xidx) max_xidx = curr_xidx;
      ...
      //printf(" -- xs[i].lower+channels=%d != %d=xs[i].upper -- lerp=%.7f\n",xs_[ix].lower+channels_,xs_[ix].upper,xs_[ix].lerp);
...
template<class T, class U>
bool CropResizeCastImage<T,U>::Load1_ok_(const int min_xidx, const int max_xidx) {
...
// for the load group pointed to by xs.
template<class T, class U>
bool CropResizeCastImage<T,U>::Load2_ok_(const int min_xidx, const int max_xidx) {
  // num_pixels_to_load_left_input = max_xs_low - min_xs_low + 1
...
// for the load group pointed to by xs.
template<class T, class U>
bool CropResizeCastImage<T,U>::Load4_ok_(const int min_xidx, const int max_xidx) {
...
// for the load group pointed to by xs.
template<class T, class U>
bool CropResizeCastImage<T,U>::Load8_ok_(const int min_xidx, const int max_xidx) {
...
template<> bool CropResizeCastImage<uint8,uint8>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint8,int8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<uint8,uint16>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint8,int16>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint8,int32>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint8,Eigen::half>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint8,float>::clip_necessary() {return false;}

template<> bool CropResizeCastImage<int8,uint8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int8,int8>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int8,uint16>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int8,int16>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int8,int32>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int8,Eigen::half>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int8,float>::clip_necessary() {return false;}

template<> bool CropResizeCastImage<uint16,uint8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<uint16,int8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<uint16,uint16>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint16,int16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<uint16,int32>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<uint16,Eigen::half>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<uint16,float>::clip_necessary() {return false;}

template<> bool CropResizeCastImage<int16,uint8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int16,int8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int16,uint16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int16,int16>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int16,int32>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int16,Eigen::half>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int16,float>::clip_necessary() {return false;}

template<> bool CropResizeCastImage<int32,uint8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int32,int8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int32,uint16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int32,int16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int32,int32>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<int32,Eigen::half>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<int32,float>::clip_necessary() {return false;}

template<> bool CropResizeCastImage<Eigen::half,uint8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<Eigen::half,int8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<Eigen::half,uint16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<Eigen::half,int16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<Eigen::half,int32>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<Eigen::half,Eigen::half>::clip_necessary() {return false;}
template<> bool CropResizeCastImage<Eigen::half,float>::clip_necessary() {return false;}

template<> bool CropResizeCastImage<float,uint8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<float,int8>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<float,uint16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<float,int16>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<float,int32>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<float,Eigen::half>::clip_necessary() {return true;}
template<> bool CropResizeCastImage<float,float>::clip_necessary() {return false;}

#endif // __SSE4_1__

template <typename T, typename U>
void crop_resize_single_image_common(const T* image, const int64 in_height,
                              const int64 in_width, const int64 out_height,
                              const int64 out_width, const int channels,
                              const int min_ix, const int max_ix,
                              const CachedInterpolation* xs, const int min_iy,
...
// full specializations of crop_resize_single_image_common for data types that have vectorized implementations.
// at the moment, this is uint8, int8, uint16, int16, int32, Eigen::half, bfloat16 and float.

#define CROP_RESIZE_SINGLE_IMAGE_VECT(T_type, U_type) \
template<> \
void crop_resize_single_image_common<T_type,U_type>(const T_type* image, const int64 in_height, \
    const int64 in_width, const int64 out_height, \
    const int64 out_width, const int channels, \
    const int min_ix, const int max_ix, \
    const CachedInterpolation* xs, const int min_iy, \
    const int max_iy, const CachedInterpolation* ys, \
    const float extrapolated_value, const bool flip_x, \
    const bool flip_y, U_type* output) { \
  if (channels <= 4) { \
    CropResizeCastImage<T_type,U_type>* resizer = new CropResizeCastImage<T_type,U_type>(in_height,in_width,out_height,out_width,channels,min_ix,max_ix,xs, \
	min_iy,max_iy,ys,extrapolated_value,flip_x,flip_y,false,15); \
    resizer->Resize(image,output); \
    delete resizer; \
  } else { \
    crop_resize_single_image(image,in_height,in_width,out_height,out_width,channels,min_ix,max_ix,xs,min_iy,max_iy,ys,extrapolated_value,flip_x,flip_y,output); \
  } \
}
...
// full specializations of crop_resize_single_image_common for data types that don't have vectorized implementations.
