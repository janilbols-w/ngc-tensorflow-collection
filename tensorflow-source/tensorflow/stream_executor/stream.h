...
class Stream {
 public:
  ...
  Stream &ThenFindBatchNormalizationTrainingExReserveSpaceSize(
      uint64_t batch_size, uint64_t feature_count, uint64_t y_size,
  ...
  // Enqueue a forward operation of the MHA model onto the stream.
  // See DnnSupport::DoMhaForward for more details.
  Stream& ThenMhaForward(const dnn::MhaDescriptor& mha_desc,
  ...
  Stream& ThenMhaBackward(const dnn::MhaDescriptor& mha_desc,
                          const dnn::MhaSeqDataDescriptor& input_q_desc,
                          const DeviceMemory<double>& input_q_data,
                          const dnn::MhaSeqDataDescriptor& input_k_desc,
                          const DeviceMemory<double>& input_k_data,
