...
@ops.RegisterGradient("CudnnRNNV4")
def _cudnn_rnn_backward_v4(op, *grads):
  """Gradients for the CudnnRNNV4 op."""
  if not op.get_attr("is_training"):
    raise ValueError(
        "To use CudnnRNNV4 in gradients, is_training must be set to True.")
  return gen_cudnn_rnn_ops.cudnn_rnn_backprop_v4(
      input=op.inputs[0],
      input_h=op.inputs[1],
      input_c=op.inputs[2],
      params=op.inputs[3],
