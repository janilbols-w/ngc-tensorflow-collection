...
Status ConvertConcat(OpConverterParams* params) {
  ...
  if (params->use_implicit_batch)
    expected_input = TrtInputArg::kTensor;
  else
    expected_input = TrtInputArg::kBoth;
  ...
  for (int i = 0; i < num_inputs; i++) {
    if (params->use_implicit_batch && inputs.at(i).is_weights())
    {
      return errors::Unimplemented("Inputs to ConcatV2 cannot be constant in "
        "implicit batch mode, at ", node_def.name());
    }
...
Status ConvertGather(OpConverterParams* params) {
  ...
  int expected_trt_output_rank;
  if (params->use_implicit_batch) {
    expected_trt_output_rank =
      tf_gather_output_rank - (params_input.is_tensor() ? 2 : 1);
  } else {
    expected_trt_output_rank = tf_gather_output_rank -
