...
StatusOr<TrtUniquePtrType<nvinfer1::ICudaEngine>> TRTEngineOp::BuildEngine(
  ...
  auto status = convert::ConvertGraphDefToEngine(
      ...
      enable_sparse_compute_, conversion_input_shapes, &logger,
      cache_resource->allocator_.get(), calibrator, &engine, use_calibration,
      use_implicit_batch_, nullptr, &cache_resource->profiles_, name(),
      use_explicit_precision_, &cluster);
