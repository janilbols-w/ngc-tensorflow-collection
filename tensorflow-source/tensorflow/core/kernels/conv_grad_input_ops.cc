...
void GetAutotuneConvolveBackwardDataConfigs(
    OpKernelContext* ctx, const se::dnn::BatchDescriptor& input_desc,
  ...
  if (CudnnUseFrontend()) {
    OP_REQUIRES(
        ...
        stream->parent()->GetConvolveExecutionPlans(
            ...
            output_desc, conv_desc, use_fallback, &plans),
    ...
    for (const auto& plan : plans) {
      configs.push_back(
          se::dnn::AlgorithmConfig(se::dnn::AlgorithmDesc{
                                       plan->getTag(), plan->get_raw_desc()},
...
void LaunchConv2DBackpropInputOp<GPUDevice, T>::operator()(
  ...
  if (cudnn_use_autotune &&
    ...
    GetAutotuneConvolveBackwardDataConfigs<T>(
        ctx, input_desc, filter_desc, conv_desc, output_desc,
    ...
    bool found_working_config = DoAutotuneConvolveBackwardDataConfigs<T>(
        ctx, input_desc, filter_desc, filter_ptr, conv_desc, output_desc,
    ...
    if (CudnnUseFrontend()) {
      ...
      if (!found_working_config) {
        ...
        GetAutotuneConvolveBackwardDataConfigs<T>(
            ctx, input_desc, filter_desc, conv_desc, output_desc,
            conv_parameters, algorithms, plans, /*use_fallback*/true, configs);
      ...
      LogConvAutotuneResults(
          ...
          in_backprop_ptr, filter_ptr, out_backprop_ptr, input_desc,
          filter_desc, output_desc, conv_desc, stream->parent(), results);
      OP_REQUIRES_OK(
