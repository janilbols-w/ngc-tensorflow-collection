...
/*static*/ StatusOr<GemmConfig> GemmConfig::For(const HloInstruction* gemm) {
  ...
  TF_ASSIGN_OR_RETURN(GemmConfig ret_config, GemmConfig::For(
  ...
  ret_config.cluster_name = gemm->GetModule()->name();  // Needed for NVTX ranges
  ret_config.op_name = gemm->metadata().op_name();      // Needed for NVTX ranges
  ret_config.op_type = gemm->metadata().op_type();      // Needed for NVTX ranges
  return ret_config;
}
...
Status RunGemm(const GemmConfig& config, se::DeviceMemoryBase lhs_buffer,
  ...
  tensorflow::nvtx::ScopedRangeIfEnabled<tensorflow::nvtx::CoreDomain>
      nvtx_range(config.op_type, [&]() {
        return tensorflow::nvtx::GetThunkExecutionRangeMessage(
            config.cluster_name, config.op_name, config.op_type);
      });
