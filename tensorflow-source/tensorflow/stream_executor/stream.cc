...
Stream &Stream::ThenFindBatchNormalizationTrainingExReserveSpaceSize(
    int64_t batch_size, int64_t feature_count, int64_t y_size,
...
Stream& Stream::ThenMhaForward(const dnn::MhaDescriptor& mha_desc,
                               ...
                               ScratchAllocator* workspace_allocator) {
  if (dnn::DnnSupport* dnn = parent_->AsDnn()) {
    auto status = dnn->DoMhaForward(
        this, mha_desc, input_q_desc, input_q_data, input_k_desc, input_k_data,
        input_v_desc, input_v_data, seq_lengths_qo_data, seq_lengths_kv_data,
        params_data, output_desc, output_data, seq_len_q, seq_len_k,
...
Stream& Stream::ThenMhaBackward(
    ...
    ScratchAllocator* workspace_allocator) {
  if (dnn::DnnSupport* dnn = parent_->AsDnn()) {
    auto status = dnn->DoMhaBackward(
        this, mha_desc, input_q_desc, input_q_data, input_k_desc, input_k_data,
        input_v_desc, input_v_data, params_data, seq_lengths_qo_data,
        seq_lengths_kv_data, output_desc, output_data, output_backprop_data,
    ...
    SetError();
    LOG(WARNING) << "Attempting to call ThenMhaBackward without DNN support";
  }
  return *this;
}

Stream& Stream::ThenMhaBackward(const dnn::MhaDescriptor& mha_desc,
                                const dnn::MhaSeqDataDescriptor& input_q_desc,
                                const DeviceMemory<float>& input_q_data,
                                const dnn::MhaSeqDataDescriptor& input_k_desc,
                                const DeviceMemory<float>& input_k_data,
...
Stream& Stream::ThenTsrContraction(const void* A_raw, const void* B_raw,
                                   void* C_raw, void* work_raw) {
