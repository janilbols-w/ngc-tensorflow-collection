...
class AutoMixedPrecisionImpl {
 public:
  ...
  AutoMixedPrecisionImpl(Cluster* cluster,
      ...
      : devices_(GetDevices(cluster)),
        ...
        num_processable_nodes_(0),
        num_recognized_nodes_(0),
        num_nodes_changed_(0),
        num_nonvar_casts_to_f16_(0),
        num_allowlist_nodes_(0),
        num_denylist_nodes_(0),
        num_denylist_affected_nodes_(0),
        mode_(mode),
 ...
 private:
  ...
  void PropagateDenyFwdThroughClearAndInfer(absl::flat_hash_set<int>* deny_set);
  ...
  int cuda_version_;
  int cudnn_version_;
  int num_processable_nodes_;
  int num_recognized_nodes_;
  int num_nodes_changed_;
  int num_nonvar_casts_to_f16_;
  int num_allowlist_nodes_;
  int num_denylist_nodes_;
  int num_denylist_affected_nodes_;
...
Status AutoMixedPrecisionImpl::PrintSummaryInfo() const {
  LOG(INFO)
      << "Automatic Mixed Precision Grappler Pass Summary:\n\n"
      << "Total processable nodes: " << num_processable_nodes_ << "\n"
      << "Recognized nodes available for conversion: " << num_recognized_nodes_
      << "\nTotal nodes converted: " << num_nodes_changed_ << "\n"
      << "Total FP16 Cast ops used (excluding Const and Variable casts): "
      << num_nonvar_casts_to_f16_ << "\n"
      << "Allowlisted nodes converted: " << num_allowlist_nodes_ << "\n"
      << "Denylisted nodes blocking conversion: " << num_denylist_nodes_
      << "\nNodes blocked from conversion by denylisted nodes: "
      << num_denylist_affected_nodes_ << "\n\n"
...
Status AutoMixedPrecisionImpl::Optimize() {
  ...
  for (const NodeDef& node : graph_->node()) {
    ...
    switch (mode_) {
      case AutoMixedPrecisionMode::CUDA:
        ...
        should_process =
            !MustPreserve(node) && IsOnDevice(node, device_type) && !IsScopeDisabled(node) &&
  ...
  num_allowlist_nodes_ = allow_set.size();
  ...
  num_denylist_affected_nodes_ = deny_set.size() - num_denylist_nodes_;
...
void AutoMixedPrecisionImpl::CountRecognizedNodes() {
  ...
  for (int root_idx = 0; root_idx < graph_type_view_.num_nodes(); ++root_idx) {
    ...
    ++num_processable_nodes_;
    if (f16_allowlist_.count(root.node->op()) ||
        f16_denylist_.count(root.node->op()) ||
        f16_inferlist_.count(root.node->op())  ||
        f16_clearlist_.count(root.node->op())) {
      ++num_recognized_nodes_;
    }
  }
}
...
void AutoMixedPrecisionImpl::PropagateDenyFwdThroughClearAndInfer(
  ...
  for (int root_idx = 0; root_idx < graph_type_view_.num_nodes(); ++root_idx) {
    ...
    if (f16_denylist_.count(root.node->op())) ++num_denylist_nodes_;
    DfsTypeTraversal(graph_type_view_, {&root},
