...
#if GOOGLE_CUDA
template<typename T>
void GetAutotuneConvolveConfigs(
...
template<typename T>
bool DoAutotuneConvolveConfigs(
    OpKernelContext* ctx, const se::dnn::BatchDescriptor& input_desc,
    ...
    se::DeviceMemory<T>& output_tensor, int64 ConvolveScratchSize,
...
void LaunchConv2DOp<GPUDevice, T>::operator()(
  ...
  if (cudnn_use_autotune && !AutoTuneConv::GetInstance()->FindBasedOnScore(
    ...
    bool found_working_config = DoAutotuneConvolveConfigs<T>(
        ctx, input_desc, input_ptr, filter_desc, filter_ptr, conv_desc,
        output_desc, tf_allocator_adapter, rz_allocator, output_tensor,
        ConvolveScratchSize, configs, results);
    ...
    if (CudnnUseFrontend()) {
      ...
      LogConvAutotuneResults(
          se::dnn::ConvolutionKind::FORWARD, se::dnn::ToDataType<T>::value,
          input_ptr, filter_ptr, output_tensor, input_desc, filter_desc,
