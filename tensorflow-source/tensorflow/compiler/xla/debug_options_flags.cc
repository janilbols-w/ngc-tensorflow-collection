...
DebugOptions DefaultDebugOptionsIgnoringFlags() {
  ...
  // Set 6GB space limit for redzone allocator.
  opts.set_xla_gpu_redzone_max_megabytes((1LL << 12) + (1LL << 11));
  return opts;
}
...
static void AllocateFlags() {
  ...
  flag_objects->push_back(tensorflow::Flag(
      ...
      int32_setter_for(&DebugOptions::set_xla_gpu_use_cudnn_batchnorm_level),
      flag_values->xla_gpu_use_cudnn_batchnorm_level(),
      "Allows the GPU backend to implement batchnorm HLOs using cudnn, rather "
      "than expanding them to a soup of HLOs. 0 = off; 1 = on only when "
      "BatchNorm+Activation+Add cudnn fusion is possible; 2 = fully on."));
  ...
  flag_objects->push_back(tensorflow::Flag(
      "xla_gpu_redzone_max_megabytes",
      int64_setter_for(
          &DebugOptions::set_xla_gpu_redzone_max_megabytes),
      flag_values->xla_gpu_redzone_max_megabytes(),
      "Max size (in megabytes) for the GPU redzone allocator."));
