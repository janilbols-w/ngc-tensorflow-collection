 ...
 public:
  Status HandleDot(HloInstruction *instr) override {
    if (IsMatrixMultiplication(*instr)) {
      ...
      gemm_config.set_activation_mode(
          static_cast<int64>(se::dnn::ActivationMode::kNone));
  ...
  Status HandleAdd(HloInstruction *instr) override {
    ...
    if (tensorflow::EnableCublasLtGemm()) {
      if (Match(instr,
                m::AddAnyOrder(
                    m::Op(&existing_gemm).WithCustomCallTarget(kGemmCallTarget),
                    m::Broadcast(m::Op(&bias))))) {
        auto out_type = existing_gemm->shape().element_type();
        // Gemm+Bias epilogue fusion not supported for these datatypes.
        if (out_type == S32 || out_type == C64 || out_type == C128) {
          return Status::OK();
        }
        auto config =
            existing_gemm->backend_config<GemmBackendConfig>().ValueOrDie();
        const DotDimensionNumbers &dim_nums = config.dot_dimension_numbers();
        int64 col_dim = dim_nums.lhs_batch_dimensions_size() + 1;

        int64 output_num_cols = existing_gemm->shape().dimensions(col_dim);
        int64 batch_size = config.batch_size();
        if (config.beta() == 0 && bias->user_count() == 1 &&
            bias->users()[0]->user_count() == 1 /*broadcast user_count*/ &&
            existing_gemm->user_count() == 1 &&
            bias->shape().dimensions(0) == output_num_cols && batch_size == 1) {
          VLOG(1) << "Matched gemm+bias epilogue fusion pattern for "
          ...
          VLOG(1) << "Replacing with " << gemm_call->ToString();
          TF_RETURN_IF_ERROR(
              ReplaceWithNewInstruction(instr, std::move(gemm_call)));
        }
      }
  ...
  Status HandleMaximum(HloInstruction *instr) override {
    ...
    if (tensorflow::EnableCublasLtGemm()) {
      ...
      auto zero_pattern = m::Broadcast(m::ConstantScalar(0));
      if (Match(instr, m::MaximumAnyOrder(
                           zero_pattern,
                           m::Op(&existing_gemm)
                               .WithCustomCallTarget(kGemmCallTarget)))) {
        auto out_type = existing_gemm->shape().element_type();
        // Gemm+Bias+Act epilogue fusion not supported for these datatypes.
        // Technically Maximum is not defined for complex data types.
        if (out_type == S32 || out_type == C64 || out_type == C128) {
          return Status::OK();
        }
        auto config =
            existing_gemm->backend_config<GemmBackendConfig>().ValueOrDie();
        CHECK_GE(existing_gemm->operand_count(), 2);
        bool valid_fusion_pattern = (existing_gemm->operand_count() == 3)
                                        ? existing_gemm->operand(0)->shape() !=
                                              existing_gemm->operand(2)->shape()
                                        : true;

        if (valid_fusion_pattern && config.beta() == 0.0 &&
            existing_gemm->user_count() == 1) {
          config.set_activation_mode(
              static_cast<int64>(se::dnn::ActivationMode::kRelu));
          std::unique_ptr<HloInstruction> gemm_call =
              HloInstruction::CreateCustomCall(
                  instr->shape(), existing_gemm->operands(), kGemmCallTarget);
          TF_RETURN_IF_ERROR(gemm_call->set_backend_config(config));
          TF_RETURN_IF_ERROR(
              ReplaceWithNewInstruction(instr, std::move(gemm_call)));
        }
