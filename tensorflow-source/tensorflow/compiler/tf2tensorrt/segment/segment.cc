...
string GenerateNonConversionReport(
  ...
  // <OP Name, Total Unconverted for OP, <Reason, Count for this reason>>>
  using NotConvertedOPTuple = std::tuple<string, int, ReasonCounterVector>;

  std::vector<NotConvertedOPTuple> unconverted_ops_vec;

  // Populate the vector from the map
  for (auto& unconverted_op_data : nonconverted_ops_map) {
    int total_unconverted_op {0};
    ReasonCounterVector reason_occurances_vect;

    auto op_name = unconverted_op_data.first;
    auto op_data = unconverted_op_data.second;

    for (auto& notconversion_reason_data : op_data) {
      auto reason_count = notconversion_reason_data.second;
      total_unconverted_op += reason_count;
    ...
    unconverted_ops_vec.push_back(std::make_tuple(
        op_name, total_unconverted_op, reason_occurances_vect));
  }

  // Sort the vector by descending OP names.
  std::sort(
      unconverted_ops_vec.begin(), unconverted_ops_vec.end(),
      [](const NotConvertedOPTuple& a,
         const NotConvertedOPTuple& b) {
          return std::get<1>(a) > std::get<1>(b);
  });

  for (auto& notconverted_op_detail : unconverted_ops_vec) {
    auto& op_name = std::get<0>(notconverted_op_detail);
    auto& op_total_unconverted = std::get<1>(notconverted_op_detail);
  ...
  unsupported_op_report =
      StrCat(unsupported_op_report, "\n", string(80, '-'),
             ...
             "\n\t- Total unconverted OP Types: ", nonconverted_ops_map.size(),
