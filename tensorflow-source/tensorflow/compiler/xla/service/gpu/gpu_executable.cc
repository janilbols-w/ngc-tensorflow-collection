...
bool GpuExecutable::CanUseGpuGraphCapture(const ThunkSchedule& thunk_schedule) {
  if (!can_use_gpu_graph_capture_.first) {
    can_use_gpu_graph_capture_.first = true;
    SetCanUseGraphCaptureFlag(GpuExecutableSafeForGraphCapture(thunk_schedule));
  }
...
Status GpuExecutable::ExecuteThunks(
  ...
  bool use_gpu_graph_capture =
      GpuGraphCaptureEnabled(module_name_) && CanUseGpuGraphCapture(thunk_schedule) &&
  ...
  if (!use_gpu_graph_capture) {
    TF_RETURN_IF_ERROR(ExecuteThunkSequence(
        thunk_schedule, run_options, buffer_allocations, profiler, main_stream,
        capture_stream,
    ...
    if (exec_graph) {
      ...
      TF_RETURN_IF_ERROR(ExecuteThunkSequence(
          thunk_schedule, run_options, buffer_allocations, profiler,
          main_stream, capture_stream,
          async_comms_stream.ok() ? async_comms_stream->get() : nullptr,
          sub_streams, deferred_host_callbacks));
...
Status GpuExecutable::ExecuteThunkSequence(
  ...
  for (const std::unique_ptr<Thunk>& thunk : thunk_schedule.TotalOrder()) {
    ...
    int32 stream_no = thunk_schedule.StreamNumberForThunk(thunk.get());
    se::Stream* stream =
