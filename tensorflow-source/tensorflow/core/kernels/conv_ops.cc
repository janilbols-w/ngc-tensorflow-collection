...
int64_t GetDnnWorkspaceLimitByGpu(se::Stream* stream, DataType dtype) {
  // Performance can benefit from the TF32 available since Ampere, which may
  // require larger workspace to store cast intermediates. So, we double up the
  // workspace limit for the Ampere+ GPUs.
...
void LaunchConv2DOp<GPUDevice, T>::operator()(
  ...
  static int64_t ConvolveScratchSize = GetDnnWorkspaceLimitByGpu(stream, dtype);
