...
Status IrEmitterUnnested::EmitConvolutionThunk(mlir::Operation* op) {
  ...
  auto fill_conv_descriptor = [&](auto op, std::string xla_cluster) {
    ...
    descriptor.op_type = op.getOpType().str();
    descriptor.op_name = op.getOpName().str();
...
Status IrEmitterUnnested::EmitGemmThunk(mlir::Operation* op) {
  ...
  if (IsBefThunkEnabled(hlo_module_config_)) {
    ...
    // For NVTX ranges
    config.op_type = gemm.getOpType().str();
    config.op_name = gemm.getOpName().str();
    auto module = op->getParentOfType<mlir::ModuleOp>();
    config.cluster_name = mlir::GetNameFromLoc(module.getLoc());
...
Status IrEmitterUnnested::EmitCublasLtMatmulThunk(mlir::Operation* op) {
  ...
  if (IsBefThunkEnabled(hlo_module_config_)) {
    ...
    // For NVTX ranges
    plan.op_type = matmul.getOpType().str();
    plan.op_name = matmul.getOpName().str();
    auto module = op->getParentOfType<mlir::ModuleOp>();
    plan.cluster_name = mlir::GetNameFromLoc(module.getLoc());
