...
#if GOOGLE_CUDA
template<typename T>
void GetAutotuneConvolveBackwardDataConfigs(
    OpKernelContext* context, const se::dnn::BatchDescriptor& input_desc,
    ...
    bool use_fallback, std::vector<se::dnn::AlgorithmConfig>& configs) {
  ...
  if (CudnnUseFrontend()) {
    OP_REQUIRES(context,
                stream->parent()->GetConvolveExecutionPlans(
                    ...
                    filter_desc, output_desc, conv_desc, use_fallback, &plans),
                errors::Unknown(
...
template<typename T>
bool DoAutotuneConvolveBackwardDataConfigs(
    OpKernelContext* context, const se::dnn::BatchDescriptor& input_desc,
    const se::dnn::FilterDescriptor &filter_desc,
    const se::DeviceMemory<T> &filter_ptr,
    const se::dnn::ConvolutionDescriptor &conv_desc,
    const se::dnn::BatchDescriptor &output_desc,
    const se::DeviceMemory<T> &out_backprop_ptr,
    se::TfAllocatorAdapter& tf_allocator_adapter,
    se::RedzoneAllocator& rz_allocator,
    se::DeviceMemory<T>& in_backprop_ptr_rz,
    int64 ConvolveBackwardDataScratchSize,
...
template<typename T>
void GetAutotuneConvolveBackwardFilterConfigs(
    OpKernelContext* context, const se::dnn::BatchDescriptor& input_desc,
    const se::dnn::FilterDescriptor &filter_desc,
    const se::dnn::ConvolutionDescriptor &conv_desc,
    const se::dnn::BatchDescriptor &output_desc,
...
template<typename T>
bool DoAutotuneConvolveBackwardFilterConfigs(
    OpKernelContext* context, const se::dnn::BatchDescriptor& input_desc,
    const se::DeviceMemory<T> &input_ptr,
    const se::dnn::FilterDescriptor &filter_desc,
    const se::dnn::ConvolutionDescriptor &conv_desc,
    const se::dnn::BatchDescriptor &output_desc,
    const se::DeviceMemory<T> &out_backprop_ptr,
    se::DeviceMemory<T>& filter_backprop_ptr,
    int64 ConvolveBackwardFilterScratchSize,
    std::vector<se::dnn::AlgorithmConfig>& configs,
    std::vector<tensorflow::AutotuneResult>& results) {
  auto* stream = context->op_device_context()->stream();
  bool found_working_config = false;
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (cudnn_use_autotune_ &&
      ...
      GetAutotuneConvolveBackwardDataConfigs<T>(
          context, input_desc, filter_desc, conv_desc, output_desc,
          conv_parameters, algorithms, plans, /*use_fallback*/false, configs);
      ...
      bool found_working_config = DoAutotuneConvolveBackwardDataConfigs<T>(
          context, input_desc, filter_desc, filter_ptr, conv_desc, output_desc,
          out_backprop_ptr, tf_allocator_adapter, rz_allocator,
          in_backprop_ptr_rz, ConvolveBackwardDataScratchSize, configs,
          results);
      ...
      if (CudnnUseFrontend()) {
        ...
        if (!found_working_config) {
          ...
          GetAutotuneConvolveBackwardDataConfigs<T>(
              context, input_desc, filter_desc, conv_desc, output_desc,
              conv_parameters, algorithms, plans, /*use_fallback*/true,
              configs);
  
          DoAutotuneConvolveBackwardDataConfigs<T>(
              context, input_desc, filter_desc, filter_ptr, conv_desc,
              output_desc, out_backprop_ptr, tf_allocator_adapter, rz_allocator,
              in_backprop_ptr_rz, ConvolveBackwardDataScratchSize, configs,
              results);
        }
#endif // GOOGLE_CUDA
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (cudnn_use_autotune_ &&
      ...
      GetAutotuneConvolveBackwardFilterConfigs<T>(
          context, input_desc, filter_desc, conv_desc, output_desc,
          conv_parameters, algorithms, plans, /*use_fallback*/false, configs);

      std::vector<tensorflow::AutotuneResult> results;

      bool found_working_config = DoAutotuneConvolveBackwardFilterConfigs<T>(
          context, input_desc, input_ptr, filter_desc, conv_desc, output_desc,
          out_backprop_ptr, filter_backprop_ptr,
          ConvolveBackwardFilterScratchSize, configs, results);
      ...
      if (CudnnUseFrontend()) {
        ...
        if (!found_working_config) {
          ...
          DoAutotuneConvolveBackwardFilterConfigs<T>(
              context, input_desc, input_ptr, filter_desc, conv_desc,
              output_desc, out_backprop_ptr, filter_backprop_ptr,
              ConvolveBackwardFilterScratchSize, configs, results);
        }
#endif // GOOGLE_CUDA
