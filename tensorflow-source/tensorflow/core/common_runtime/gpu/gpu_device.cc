...
class EigenGpuStreamDevice : public ::Eigen::StreamInterface {
 public:
  ...
  void deallocate(void* buffer) const override {
    ...
    // Stream-aware allocators do not need to be called from a stream callback,
    // and in fact some *cannot* be called from a stream callback because CUDA
    // API calls are not allowed inside stream callbacks.
    // For stream-aware allocators, we call DeallocateRaw immediately (which
    // will enqueue the deallocation into the stream) and use the callback only
    // for logging purposes.
    const bool allocator_is_stream_aware = allocator_->IsGpuStreamAware();
    AsyncFreeData* afData =
        new AsyncFreeData(allocator_, buffer, operation_, step_id_,
                          /*do_logging_only=*/allocator_is_stream_aware);
#if GOOGLE_CUDA
...
int64_t MinSystemMemory(int64_t available_memory, int cc_major) {
  ...
  // 1800MiB (for cuda_compute_capability >= 8.x)
  // 4GiB (for cuda_compute_capability >= 9.x)
...
#if GOOGLE_CUDA
#if CUDA_CUTENSOR
  auto handle_cutsr = se::internal::DsoLoader::MaybeTryDlopenCUTENSORLibrary();
  if (!handle_cutsr.ok()) {
    VLOG(1) << "WARNING: " << "Cannot dlopen cuTENSOR libraries. Please make "
                              "sure it's installed properly. cuTENSOR support "
