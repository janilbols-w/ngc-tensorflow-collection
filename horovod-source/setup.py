...
def get_package_version():
    return __version__ + "+" + os.environ['HOROVOD_LOCAL_VERSION'] + "+nv" + \
    os.environ['RELEASE_VERSION'] \
    if 'HOROVOD_LOCAL_VERSION' in os.environ else \
    __version__  + "+nv" + os.environ['RELEASE_VERSION']


setup(name='nvidia-horovod',
      ...
      entry_points={
          'console_scripts': [
              'horovodrun = horovod.openmpi_dist.horovodrun:main',
              'mpiexec = horovod.openmpi_dist.mpiexec:main',
              'mpirun = horovod.openmpi_dist.mpirun:main',
              'ompi_info = horovod.openmpi_dist.ompi_info:main',
              'orted = horovod.openmpi_dist.orted:main',
              'orterun = horovod.openmpi_dist.orterun:main',
           ]
     },
