...
Status GpuExecutable::ExecuteThunks(
  ...
  if (!use_gpu_graph_capture) {
    ...
    VLOG(3) << "For gpu executable " << this << " tmp buffer base: "
            << temp_buffer_base_.opaque() << " key hash: "
...
StatusOr<ExecutionOutput> GpuExecutable::ExecuteAsyncOnStreamImpl(
  ...
  // ShapeIndex{-1, ...} is AsyncOut. Put its buffer into `buffers_in_result`.
  for (auto p : output_info_) {
    if (p.first[0] == -1) {
      buffers_in_result.insert(
          buffer_allocations.GetDeviceAddress(p.second.allocation_index));
    }
  }
...
GetOutputInfo(const HloModule& hlo_module, const BufferAssignment& assignment) {
  ...
  // Also put all input operands of AsyncOutSend into OutputInfoMap as they are
  // consudered outputs.
  int64 async_out_id = 0;
  for (auto* instr : hlo_module.entry_computation()->instructions()) {
    if (instr->opcode() != HloOpcode::kAsyncOutSend) {
      continue;
    }
    const HloInstruction* opnd = instr->operand(0);
    TF_ASSIGN_OR_RETURN(const BufferAllocation::Slice slice,
                        assignment.GetUniqueSlice(opnd, {}));
    CHECK_EQ(slice.offset(), 0) << "AsyncOut tensor should get its own slice";
    // ShapeIndex{-1, ...} are used as (virtual) indices for AsyncOuts.
    GpuExecutable::OutputInfo& info = output[ShapeIndex({-1, async_out_id++})];
    info.allocation_index = slice.index();
  }
