...
class AutoMixedPrecisionImpl {
 ...
 private:
  ...
  Status PrintSummaryInfo(int num_processable_nodes, int num_recognized_nodes,
                          ...
                          int num_allowlist_nodes, int num_denylist_nodes,
                          int num_denylist_affected_nodes);
  ...
  void PropagateDenyFwdThroughClearAndInfer(
      absl::flat_hash_set<int>* deny_set, int* num_denylist_nodes) const;
...
Status AutoMixedPrecisionImpl::PrintSummaryInfo(
    ...
    int num_nonvar_casts_to_fp16, int num_allowlist_nodes,
    int num_denylist_nodes, int num_denylist_affected_nodes) {
  LOG(INFO)
      ...
      << "Allowlisted nodes converted: " << num_allowlist_nodes << "\n"
      << "Denylisted nodes blocking conversion: " << num_denylist_nodes
      << "\nNodes blocked from conversion by denylisted nodes: "
      << num_denylist_affected_nodes << "\n\n"
...
Status AutoMixedPrecisionImpl::Optimize() {
  ...
  int num_allowlist_nodes = allow_set.size();

  VLOG(2) << "Beginning pass 2 to propagate deny forwards from denylist ops "
             "through clear/inferlist ops";
  int num_denylist_nodes = 0;
  PropagateDenyFwdThroughClearAndInfer(&deny_set, &num_denylist_nodes);
  ...
  int num_denylist_affected_nodes = deny_set.size() - num_denylist_nodes;
  ...
  TF_RETURN_IF_ERROR(ChangeTypeAttrsAndAddCasts(allow_set, &num_nodes_changed,
                                                &num_nonvar_casts_to_fp16));
  ...
  TF_RETURN_IF_ERROR(PrintSummaryInfo(
      ...
      num_nonvar_casts_to_fp16, num_allowlist_nodes, num_denylist_nodes,
      num_denylist_affected_nodes));
...
void AutoMixedPrecisionImpl::PropagateDenyFwdThroughClearAndInfer(
  ...
  for (int root_idx = 0; root_idx < graph_type_view_.num_nodes(); ++root_idx) {
    ...
    if (f16_denylist_.count(root.node->op())) ++*num_denylist_nodes;
    DfsTypeTraversal(graph_type_view_, {&root},
...
Status AutoMixedPrecisionImpl::ChangeTypeAttrsAndAddCasts(
    const absl::flat_hash_set<int>& allow_set, int* num_nodes_changed,
    int* num_nonvar_casts_to_f16) {
  int num_nodes_preop = graph_->node_size();
  for (int node_idx = 0; node_idx < num_nodes_preop; ++node_idx) {
    NodeDef* node = graph_->mutable_node(node_idx);
    for (const TypeAttrId& type_attr : node_type_map_.GetTypeAttrs(*node)) {
