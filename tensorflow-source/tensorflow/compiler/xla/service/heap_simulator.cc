...
ConstrainedGlobalDecreasingSizeBestFitHeap::Finish() {
  using BufferInterval = typename GlobalDecreasingSizeBestFitHeap<HloValue>::BufferInterval;
  using ChunkCandidate = typename GlobalDecreasingSizeBestFitHeap<HloValue>::ChunkCandidate;
  std::vector<BufferInterval> sorted_buffer_vec = this->GetSortedBufferIntervals();
  ...
  do {
    ...
    for (auto it = sorted_buffer_intervals.begin();
      ...
      ChunkCandidate chunk_candidate = this->FindChunkCandidate(buffer_interval);
      if (chunk_candidate.heap_size <= size_limit_per_heap_ ||
          ...
          this->result_.heap_size == 0) {
        this->CommitChunk(buffer_interval, chunk_candidate);
        it = sorted_buffer_intervals.erase(it);
        continue;
      }
    ...
    // states.
    multi_heap_result.heap_size += this->result_.heap_size;
    multi_heap_result.heap_results.push_back(std::move(this->result_));
    this->result_ = {};
    this->interval_tree_ = {};
