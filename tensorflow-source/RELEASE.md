...
*   `tf.data`:
    *   The `prefetch_to_device` functionality is improved to allow for concurrent kernel execution and data transfer.
        *   Previously the concurrency was disabled due to the GPU driver holding on the kernel execution in order to stage the data, which defeats the purpose of prefetching.
        *   With the changes introduced, the type of allocated memory, e.g. the `DeviceHostAllocator` to allocates pinned memory or BFC allocator as gpu host allocator is stored upon allocation.
        *   TensorBuffer contains the allocator info which allows queries to determine the memory type which helps to offload the staging task from the driver to TF itself and hence allowing synchronization between data copy and execution.
           ```python
...
*   Toolchains
    *   CUDNN_INSTALL_PATH, CUTENSOR_INSTALL_PATH, TENSORRT_INSTALL_PATH,
        NCCL_INSTALL_PATH, NCCL_HDR_PATH are deprecated. Use TF_CUDA_PATHS
        instead which supports a comma-separated list of base paths that are
        searched to find CUDA libraries and headers.
