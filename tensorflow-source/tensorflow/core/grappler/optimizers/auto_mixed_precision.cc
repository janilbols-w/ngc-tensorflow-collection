...
class NodeTypeAttrMap {
 public:
  ...
  NodeTypeAttrMap(const GraphDef& graph) { TF_CHECK_OK(Init(graph)); }
 ...
 private:
  Status AddNode(const NodeDef& node) {
    ...
    for (int i = 0; i < (int)output_arg_inds.size(); ++i) {
    ...
    for (const auto& attr : node.attr()) {
      ...
      if (attr_def->type() == "type") {
        ...
        for (int i = 0; i < (int)attr_value.list().type_size(); ++i) {
...
void DfsTypeTraversal(const GraphTypeTopologyView& graph_type_view,
  ...
  for (const NodeTypeId* node : from) {
    ...
    DCHECK(node_idx.has_value()) << "Illegal start node: "
                                 << node->node->name();
...
Status AutoMixedPrecisionImpl::Optimize() {
  ...
  size_t timestamp = std::chrono::duration_cast<std::chrono::milliseconds>(
                         ...
                         .count();
  TF_RETURN_IF_ERROR(PrintDebugLogs(/* is_preop = */ true, timestamp));

  VLOG(2) << "Identifying nodes that should be processed";
  for (const NodeDef& node : graph_->node()) {
    if (!MustPreserve(node) && IsOnGPU(node) && !IsScopeDisabled(node) &&
      ...
      for (int output_port : node_type_map_.GetOutputPorts(*node, type_attr)) {
        ...
        for (const MutableGraphView::InputPort& dst : fanout) {
          ...
          if (src_is_white != dst_is_white) {
            ...
            dst.node->set_input(dst.port_id, added_cast_node->name());
          }
        }
      }
    }
