...
class TrtConversionParams(
  ...
  Fields:
    ...
    enable_sparse_compute: only effective with TensorRT 8 and newer with an
      ...
      True. Can be turned off for debuging reason.
...
def export_graph_to_dot(graphdef, dot_output_filename):
  import re
  with open(dot_output_filename, "w") as f:
      ...
      for node in graphdef.node:
          ...
          if node.op == "TRTEngineOp":
            node_count, _ = _get_nodes_in_engine(graphdef, output_name)
            print(f"  \"{output_name}\" [label=\"{output_name} [{node_count}]\"];", file=f)
          else:
            print(f"  \"{output_name}\" [label=\"{node.op}\"];", file=f)
