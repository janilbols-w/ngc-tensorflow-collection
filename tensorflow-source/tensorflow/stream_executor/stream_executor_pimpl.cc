...
bool StreamExecutor::GetConvolveExecutionPlans(
  ...
  return dnn_support->GetConvolveExecutionPlans(
      ...
      output_descriptor, convolution_descriptor, use_fallback, out_exec_plans);
}
...
StreamExecutor::createMhaDescriptor(int num_heads, int batch_size, int q_size,
                                    int k_size, int v_size, int q_proj_size,
                                    int k_proj_size, int v_proj_size,
                                    int o_proj_size, int seq_len_q,
  ...
  return dnn_support->createMhaDescriptor(
      num_heads, batch_size, q_size, k_size, v_size, q_proj_size, k_proj_size,
      v_proj_size, o_proj_size, seq_len_q, seq_len_k, data_type, comp_prec,
      dropout_rate, is_proj_bias, seed, state_allocator);
}
...
StreamExecutor::createMhaSeqDataDescriptor(
    int batch_size, int max_seq_length, int vect_size, int seq_array_size,
    const absl::Span<const int>& seq_array, dnn::DataType data_type) {
  dnn::DnnSupport* dnn_support = AsDnn();
  if (!dnn_support) {
    return port::Status(port::error::UNKNOWN,
                        "Fail to find the dnn implementation.");
  }
  return dnn_support->createMhaSeqDataDescriptor(batch_size, max_seq_length,
                                                 vect_size, seq_array_size,
                                                 seq_array, data_type);
}
