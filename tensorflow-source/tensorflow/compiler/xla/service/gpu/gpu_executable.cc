...
StatusOr<ExecutionOutput> GpuExecutable::ExecuteAsyncOnStreamImpl(
  ...
  XLA_SCOPED_LOGGING_TIMER(absl::StrCat(
      "GpuExecutable::ExecuteAsyncOnStreamImpl(", module().name(), ")"));
      ...
      MaybeOwningDeviceMemory* maybe_owning_memory =
        ...
        if (auto* unowned_shapedbuffers =
                absl::get_if<absl::Span<const ShapedBuffer* const>>(
                    &arguments)) {
          CHECK_LT(alias->parameter_number, unowned_shapedbuffers->size());
          return nullptr;
        } else {
          auto unowned_execution_input =
              absl::get<absl::Span<ExecutionInput>>(arguments);
          CHECK_LT(alias->parameter_number, unowned_execution_input.size());
          ExecutionInput& input =
              unowned_execution_input[alias->parameter_number];
          return input.MutableBuffer(alias->parameter_index);
        }
      }();
