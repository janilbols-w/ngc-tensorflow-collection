...
namespace {
Status PrintGraphDefLogs(GraphDef* graph_def, bool post_opt) {
  string path;
  TF_RETURN_IF_ERROR(ReadStringFromEnvVar(
      "TF_GRAPPLER_GRAPH_DEF_PATH", "", &path));
  if (path.empty()) return Status::OK();

  size_t timestamp = Env::Default()->NowMicros() / 1000;
  string suffix = strings::StrCat("_", post_opt ? "post_opt" : "pre_opt", "_",
                                  timestamp);
  std::fstream f;

  string fname = io::JoinPath(path, strings::StrCat("graphdef", suffix, ".pb"));
  f.open(fname.c_str(), std::fstream::out | std::fstream::binary);
  f << graph_def->SerializeAsString();
  f.close();
  LOG(INFO) << "Saved " << (post_opt ? "post-optimization" : "pre-optimization")
            << " graph as binary to " << fname;

  fname = io::JoinPath(path, strings::StrCat("graphdef", suffix, ".pb.txt"));
  f.open(fname.c_str(), std::fstream::out);
  f << graph_def->DebugString();
  f.close();
  LOG(INFO) << "Saved " << (post_opt ? "post-optimization" : "pre-optimization")
...
Status MetaOptimizer::OptimizeGraph(
  ...
  *optimized_graph = std::move(item.graph);
  TF_RETURN_IF_ERROR(PrintGraphDefLogs(optimized_graph, /*post_opt=*/false));
  ...
  TF_RETURN_IF_ERROR(PrintGraphDefLogs(optimized_graph, /*post_opt=*/true));
