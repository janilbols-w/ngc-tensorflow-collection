...
Status MarkForCompilationPassImpl::FindCompilationCandidates() {
  ...
  for (const auto& s : allowlist) {
    if (string(s) != "DEFAULT" && !all_ops.contains(string(s))) {
  ...
  for (Node* node : sorted_nodes) {
    ...
    bool must_compile_node =
        allowlist.contains("DEFAULT") && allowlist.contains(node->def().op());

    // Override the clustering decision only if the op is deemed uncompilable
    // becoz it is slow or inaccurate.
    bool is_op_slow_or_inaccurate =
        checker.OpIsSlow(*node) || checker.OpIsInaccurate(*node);

    // If the node is not compilable due the any reason other than it being slow
    // or inaccurate, but 'must_compile_node' is true, throw an error.
    if (must_compile_node && !checker.IsCompilableNode(*node, lib_runtime) &&
        !is_op_slow_or_inaccurate) {
      return errors::InvalidArgument(
          "The op to be forced clustered needs to be deemed either too slow or "
          "inaccurate to be clustered. The requested operation '",
          node->def().op(), "' passed to --tf_xla_ops_to_cluster is neither.");
    }

    if (!checker.IsCompilableNode(*node, lib_runtime) && !must_compile_node) {
      continue;
    }
    ...
    // If "DEFAULT" appears in the user list, the default behaviour of allowlist
    // is changed. Without the string "DEFAULT" appearing in the list, only the
    // ops specified in --tf_xla_ops_to_cluster, will be considered for
    // compilation. However, if "DEFAULT" appears, all default ops will be
    // considered along with the additional ops parsed. Note that these
    // additional ops were not deemed worthy of being clustered because they are
    // eiher slow or inaccurate. The user choses to compile them anyway.
    if (!allowlist.empty() && !allowlist.contains("DEFAULT") &&
        !allowlist.contains(node->def().op())) {
