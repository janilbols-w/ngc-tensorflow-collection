...
class AutoMixedPrecisionTest(test.TestCase, parameterized.TestCase):
  ...
  def _run_simple_loop_test(self, mode, inp, body, out):
    ...
    output_val_ref, output_val, cost_graph, partition_graphs = self._run(mode, a)
  ...
  def test_conv_bn(self, mode):
    ...
    output_val_ref, output_val, cost_graph, partition_graphs = self._run(mode, output)
    node_map = _build_node_map(cost_graph.node)
    num_to_fp16, num_to_fp32 = _count_casts(mode, partition_graphs[0])
  ...
  def test_depthwise_conv2d(self, mode):
    ...
    if not test.is_gpu_available(cuda_only=True): return
    random_seed.set_random_seed(0)
    x = _input([2, 8, 8, 1])
    f = _weight([3, 3, 1, 4])
  ...
  def test_ingraph_train_loop(self, mode):
    ...
    output_val_ref, output_val, cost_graph, partition_graphs = self._run(mode, loss)
    node_map = _build_node_map(cost_graph.node)
  ...
  @test_util.run_deprecated_v1
  @test_util.disable_xla('This test does not pass with XLA')
  def test_scope_disable(self, mode):
    """Test graph with convolution followed by batch norm."""
