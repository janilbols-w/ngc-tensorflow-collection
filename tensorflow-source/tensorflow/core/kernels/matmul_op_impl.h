    ...
    if (EnableCublasLtGemm()) {
      ...
      if (!requires_mixed_broadcasting) {
        ...
        BatchMatmulParameters matmul_parameters(
            ...
            broadcast_b, dtype, dtype, allow_tf32, device_id, false, false);
        ...
        if (max_algorithm_count == 1) {
          ...
          // autotune. If no algorithms works, we add kNoAlgorithm to the
          // autotune map.
      ...
      if (batch_size == 1) {
        ...
        // Note that the GEMV call here does not support Eigen::half, so we do
        // not use this path in that case. A workaround is applied to the
        // pointers passed to the call itself to avoid compilation errors.
