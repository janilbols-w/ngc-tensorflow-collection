...
def tf_copts(
    ...
    return (
        ...
        if_cuda_cutensor(["-DCUDA_CUTENSOR=1"]) +
...
def tf_gpu_library(deps = None, cuda_deps = None, copts = tf_copts(), **kwargs):
    ...
    - In addition, when the library is also built with TensorRT enabled, it
        ...
        for cuTENSOR and -DCUDA_CUTENSOR=1.
    ...
    cc_library(
        ...
        copts = (copts + if_cuda(["-DGOOGLE_CUDA=1", "-DNV_CUDNN_DISABLE_EXCEPTION"]) + if_rocm(["-DTENSORFLOW_USE_ROCM=1"]) + if_xla_available(["-DTENSORFLOW_USE_XLA=1"]) + if_mkl(["-DINTEL_MKL=1"]) + if_enable_mkl(["-DENABLE_MKL"]) + if_tensorrt(["-DGOOGLE_TENSORRT=1"]) + if_cuda_cutensor(["-DCUDA_CUTENSOR=1"])),
        **kwargs
    )
...
def pywrap_tensorflow_macro(
    ...
    srcs += if_static([], otherwise = [clean_dep("//tensorflow:libtensorflow_cc.so.2")])
