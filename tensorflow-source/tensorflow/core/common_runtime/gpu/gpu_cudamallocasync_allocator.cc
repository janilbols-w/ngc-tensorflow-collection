...
GpuCudaMallocAsyncAllocator::GpuCudaMallocAsyncAllocator(
  ...
  for(int i = 0; i < all_pools_.size(); ++i) {
    ...
    if (auto status = cuDeviceCanAccessPeer(&canAccessPeer,
                                            ...
                                            map.location.id)) {
      pool_ = nullptr;
      LOG(FATAL) << "cuDeviceCanAccessPeer failed: "
    ...
    if (canAccessPeer == 1) {
      if (auto status = cuMemPoolSetAccess(*all_pools_[i], &map, /*count=*/ 1)) {
        pool_ = nullptr;
        LOG(FATAL) << "Error when setting access to the pool id: " << i
                   << " location id: " << map.location.id
...
std::vector<PlatformDeviceId> GpuCudaMallocAsyncAllocator::all_ids_;
#endif
}  // namespace tensorflow
