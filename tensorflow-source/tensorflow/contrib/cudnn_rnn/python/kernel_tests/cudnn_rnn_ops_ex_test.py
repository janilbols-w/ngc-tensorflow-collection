...
class CudnnRNNTestInference(TensorFlowTestCase):
  ...
  # Use the original cuDNN apis to calculate expected outputs
  def _createCheatSheet(self, rnn_mode, num_layers, num_units, input_size,
    ...
    # Calculate output values for every sequence (1 to seq_length)
    for seq_len in range(seq_length):
      seq_len += 1
      input_data = array_ops.ones([seq_len, batch_size, input_size])
      if has_input_c:
      ...
      with self.test_session(use_gpu=True, graph=ops.get_default_graph()) as sess:
        sess.run(variables.global_variables_initializer())
        if has_input_c:
          o_sum_, oh_sum_, oc_sum_ = sess.run([output_sum,
              output_h_sum, output_c_sum])
          v_o = o_sum_/batch_size/seq_len/num_units
          v_oh = oh_sum_/num_layers/batch_size/num_units
          v_oc = oc_sum_/num_layers/batch_size/num_units
          self.cheatsheet.append([v_o, v_oh, v_oc])
        else:
          o_sum_, oh_sum_ = sess.run([output_sum,
              output_h_sum])
          v_o = o_sum_/batch_size/seq_len/num_units
          v_oh = oh_sum_/num_layers/batch_size/num_units
          self.cheatsheet.append([v_o, v_oh])
  ...
  def _testOneSimpleInference(self, rnn_mode, num_layers, num_units, input_size,
    ...
    lengths_v = np.random.randint(low=1, high=seq_length+1, size=batch_size)
    # Make sure the longest sequence exist and consist. w/ the config
    lengths_v[0] = seq_length 
    bins_array = [i+1 for i in range(seq_length+1) ]
    counts, bins = np.histogram(lengths_v, bins=bins_array)

    self._createCheatSheet(rnn_mode, num_layers, num_units, input_size,
                        batch_size, seq_length, dir_count, dropout)
    expected = 0.0
    for idx, cnt in enumerate(counts):
      seq_num = cnt
      seq_len = idx+1
      s_o = seq_num * seq_len * num_units * self.cheatsheet[idx][0]
      s_oh = num_layers * seq_num * num_units * self.cheatsheet[idx][1]
      s_oc = (num_layers * seq_num * num_units * 
              self.cheatsheet[idx][2]) if has_input_c else 0.0
      expected += s_o + s_oh + s_oc 
    self.cheatsheet = []
