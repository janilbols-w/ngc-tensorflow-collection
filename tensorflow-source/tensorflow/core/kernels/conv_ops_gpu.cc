...
StatusOr<se::dnn::AlgorithmConfig> AutotuneFusedConv(
  ...
  if (cudnn_use_autotune) {
    ...
    if (CudnnUseFrontend()) {
      if (!stream->parent()
               ->GetFusedConvolveExecutionPlans(
                   ...
                   conv_desc, /*use_fallback=*/false, activation_mode, &plans)
               .ok()) {
    ...
    if (!CudnnUseFrontend() || found_working_engine) {
      ...
      std::vector<se::dnn::AlgorithmConfig> fallback_configs;
      std::vector<std::unique_ptr<
                      se::dnn::ConvolveExecutionPlan>> fallback_plans;
      if (!stream->parent()
               ->GetFusedConvolveExecutionPlans(
                   se::dnn::ConvolutionKind::FORWARD,
                   se::dnn::ToDataType<T>::value, conv_scale, side_input_scale,
                   stream, input_desc, filter_desc, bias_desc, output_desc,
                   conv_desc, /*use_fallback=*/true, activation_mode,
                   &fallback_plans)
               .ok()) {
        return errors::Unknown(
            "Failed to get convolution plans. This is probably because cuDNN "
      ...
      for (const auto& plan : fallback_plans) {
        fallback_configs.push_back(se::dnn::AlgorithmConfig(
            se::dnn::AlgorithmDesc{plan->getTag(), plan->get_raw_desc()},
            plan->getWorkspaceSize()));
      }

      SE_ASSIGN_OR_RETURN(auto fallback_results,
                          AutotuneConvImpl(ctx, fallback_configs, launch_func,
                                           scratch_size, rz_allocator));
      ...
      TF_RETURN_IF_ERROR(
          BestCudnnConvAlgorithm(fallback_results, &fallback_plans,
                                 &algorithm_config));
    }
  ...
  if (do_autotune && !autotune_map->Find(conv_parameters, &algorithm_config)) {
    ...
    if (CudnnUseFrontend()) {
      if (!stream->parent()->GetConvolveExecutionPlans(
              ...
              filter_desc, output_desc, conv_desc, /*use_fallback=*/false,
              &plans)) {
    ...
    if (!CudnnUseFrontend() || found_working_engine) {
      ...
      if (!stream->parent()->GetConvolveExecutionPlans(
              kind, se::dnn::ToDataType<T>::value, stream, input_desc,
              filter_desc, output_desc, conv_desc, /*use_fallback=*/true,
              &fallback_plans)) {
        return get_algo_failed_error;
      }
      for (const auto& plan : fallback_plans) {
