...
#	TRTIS - Determines whether the TRTIS-specific libary is built (1 to build, 0 to skip)
...
TRTIS_TARGET=
if [[ $TRTIS -eq 1 ]]; then
  TRTIS_TARGET="//tensorflow:libtensorflow_trtis.so"
fi
if [[ $IN_CONTAINER -eq 1 ]]; then
  bazel build $BAZEL_OPTS \
      tensorflow/tools/pip_package:build_pip_package \
      //tensorflow:libtensorflow_cc.so $TRTIS_TARGET
  BAZEL_BUILD_RETURN=$?
  cp bazel-bin/tensorflow/libtensorflow_cc.so.? ${LIBS_OUT}
  if [[ $TRTIS -eq 1 ]]; then
    cp bazel-bin/tensorflow/libtensorflow_trtis.so.? ${LIBS_OUT}
  fi
else
  bazel build $BAZEL_OPTS \
      tensorflow/tools/pip_package:build_pip_package $TRTIS_TARGET
  if [[ $TRTIS -eq 1 ]]; then
    cp bazel-bin/tensorflow/libtensorflow_trtis.so.? ${LIBS_OUT}
  fi
  BAZEL_BUILD_RETURN=$?
...
pip$PYVER install --no-cache-dir --upgrade $WHL_OUT/tensorflow-*.whl
PIP_INSTALL_RETURN=$?
