...
class OpConverterBase {
 public:
  ...
  virtual std::vector<DataType> AllowedDataTypes(OpConverterParams* params) {
  ...
  Status ValidateNodeDefDataType() {
    ...
    VLOG(1) << params_->node_def.DebugString();

    // Check allowed data types.
    const auto& node_def = params_->node_def;
    const auto& allowed_dtypes = AllowedDataTypes(params_);
