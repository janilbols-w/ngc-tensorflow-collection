...
StatusOr<Operation*> LhloDialectEmitter::EmitGemm(
  ...
  // Pass the metadata from HLO to MLIR (for use in NVTX ranges).
  const xla::OpMetadata metadata = custom_call->metadata();
  std::string op_name = metadata.op_name();
  std::string op_type = metadata.op_type();
  op.setOpNameAttr(builder_.getStringAttr(op_name));
  op.setOpTypeAttr(builder_.getStringAttr(op_type));
...
StatusOr<Operation*> LhloDialectEmitter::EmitDnnConvolution(
  ...
  auto set_common_conv_attributes = [&, this](auto op) -> Operation* {
    ...
    std::string op_name = metadata.op_name();
    std::string op_type = metadata.op_type();
    attrs.set(op.getOpNameAttrName(), builder_.getStringAttr(op_name));
    attrs.set(op.getOpTypeAttrName(), builder_.getStringAttr(op_type));
