...
static StatusOr<absl::optional<se::blas::AlgorithmType>> DoUncachedGemmAutotune(
  ...
  for (se::blas::AlgorithmType algorithm : algorithms) {
    ...
    Status st = RunGemm(config, lhs_buffer, rhs_buffer, output_buffer, stream,
                        ...
                        nullptr /* profile_algorithm */, /*profiler=*/nullptr,
...
static StatusOr<absl::optional<se::blas::AlgorithmType>> DoGemmAutotune(
  ...
  if (tensorflow::EnableCublasLtGemm() && output_shape.element_type() != S32) {
    bool is_gemm_bias_epilogue_fusion =
        ...
        (instr->shape() != instr->operand(2)->shape() &&
         gemm_config.beta() == 0);

    if (!se::dnn::ActivationMode_IsValid(gemm_config.activation_mode())) {
      return InternalError("Bad activation mode: %s",
                           gemm_config.ShortDebugString());
    }
    auto activation_mode =
        static_cast<se::dnn::ActivationMode>(gemm_config.activation_mode());
    bool has_activation_relu =
        activation_mode == se::dnn::ActivationMode::kRelu;

    se::DeviceMemoryBase bias_buffer{};
    if (is_gemm_bias_epilogue_fusion) {
      TF_ASSIGN_OR_RETURN(bias_buffer,
                          get_initialized_buffer(instr->operand(2)));
    }
    ...
    tensorflow::BatchMatmulParameters matmul_parameters(
        ...
        allow_tf32, device_id, is_gemm_bias_epilogue_fusion,
        has_activation_relu);
    ...
    if (!plan_and_algorithms) {
      se::blas::BlasLtMatmulPlanParams plan_params =
          CreatePlanParams(batch_size, blas_dtype, dtype, lhs_matrix,
                           rhs_matrix, output_matrix,
                           is_gemm_bias_epilogue_fusion, has_activation_relu)
              .ConsumeValueOrDie();
      ...
      VLOG(2) << "Inserting plans and algorithms into "
                 "BatchMatmulPlanMapSingleton for matmul params: trans_x: "
              << trans_x << " trans_y: " << trans_y << " adj_x: " << false
              << " adj_y: " << false << " m: " << m << " n: " << n
              << " k: " << k << " batch_size: " << batch_size
              << " broadcast: " << broadcast << " broadcast: " << broadcast
              << " dtype: " << dtype << " allow_tf32: " << allow_tf32
              << " device_id: " << device_id
              << " is_gemm_bias_epilogue_fusion: "
              << is_gemm_bias_epilogue_fusion
      ...
      VLOG(4) << "Cache Hit for BatchMatmul parameters: "
              << "trans_x: " << trans_x << " trans_y: " << trans_y
              << " adj_x: " << false << " adj_y: " << false << " m: " << m
    ...
    if (!tensorflow::AutoTuneBatchMatmul::GetInstance()->Find(
      ...
      for (size_t i = 0; i != algorithms.size(); ++i) {
        ...
        Status st = RunGemm(
            ...
            /*profiler=*/nullptr,
            /*profile_result=*/&profile_result, absl::nullopt, bias_buffer);
        CHECK(st.ok()) << st.ToString();
