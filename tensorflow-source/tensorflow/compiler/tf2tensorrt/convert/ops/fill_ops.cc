 ...
 public:
  ...
  Status Validate() {
    ...
    if (dims_type != nvinfer1::DataType::kINT32) {
      return errors::InvalidArgument("The dims parameter of the ", params.node_def.op(),
                                     " operation in ", params.node_def.name(),
    ...
    if (nbDims < 0) {
        return errors::InvalidArgument("The dims parameter shape of the ", params.node_def.op(),
                                        " operation in ", params.node_def.name(),
                                        " cannot be partial.");
    }
    return Status::OK();
