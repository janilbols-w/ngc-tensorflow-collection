...
class TRTEngineOp : public AsyncOpKernel {
 ...
 private:
  ...
  // DLA use flags.
  int dla_core_;
  int dla_fallback_layers_;

  int64 workspace_size_;

  // Whether to allow TensorRT to utilize structured sparsity.
  bool enable_sparse_compute_;
...
TRTEngineOp::TRTEngineOp(OpKernelConstruction* context)
  ...
  OP_REQUIRES_OK(context,
                 context->GetAttr("enable_sparse_compute", &enable_sparse_compute_));
  ...
  OP_REQUIRES_OK(context, context->GetAttr("dla_core", &dla_core_));
  OP_REQUIRES_OK(context, context->GetAttr("dla_fallback_layers", &dla_fallback_layers_));
...
StatusOr<TrtUniquePtrType<nvinfer1::ICudaEngine>> TRTEngineOp::BuildEngine(
  ...
  auto status = convert::ConvertGraphDefToEngine(
      ...
      enable_sparse_compute_, conversion_input_shapes, &logger, cache_resource->allocator_.get(),
      calibrator, &engine, use_calibration, use_implicit_batch_, nullptr,
      &cache_resource->profiles_, name(), use_explicit_precision_, dla_core_,
      dla_fallback_layers_, &cluster, ctx->device()->name());
...
Status TRTEngineOp::AllocateCalibrationResources(
  ...
  cres->thr_.reset(new std::thread([this, cres, shapes, conversion_input_shapes,
    ...
    auto s = convert::ConvertGraphDefToEngine(
        ...
        this->enable_sparse_compute_, conversion_input_shapes, &cache_res->GetLogger(),
        cache_res->allocator_.get(), cres->calibrator_.get(), &cres->engine_,
        /*use_calibration=*/true, this->use_implicit_batch_,
        /*convert_successfully=*/nullptr, /*profiles=*/&cache_res->profiles_,
        name(), /*use_explicit_precision=*/use_explicit_precision_,
        dla_core_, dla_fallback_layers_, /*cluster=*/&cluster,
        ctx->device()->name());
    if (!s.ok()) {
