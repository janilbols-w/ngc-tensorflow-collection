...
class MutexedGraphExecCache {
 public:
  ...
  void SetCacheSize(int64_t cache_size);
...
struct GraphCacheStats {
  ...
  std::atomic<uint64_t> temp_buffer_cache_hits{0};
  std::atomic<uint64_t> cache_miss{0};
  std::atomic<uint64_t> times_called{0};
  std::atomic<size_t> last_buf_key_hash{0};
  std::atomic<uint64_t> last_buf_key_hits{0};
