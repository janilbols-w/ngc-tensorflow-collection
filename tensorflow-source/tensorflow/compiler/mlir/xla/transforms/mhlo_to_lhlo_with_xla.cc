...
StatusOr<lmhlo::AsyncOutSendOp> LhloDialectEmitter::EmitAsyncOutSendOp(
  ...
  TF_RETURN_IF_ERROR(GetOrCreateView(instr->operand(0), &operands));
  auto async_out_op = CreateOpWithoutAttrs<lmhlo::AsyncOutSendOp>(instr, operands);
  async_out_op.rendezvous_keyAttr(builder_.getStringAttr(async_out->rendezvous_key()));
  return async_out_op;
}
