...
using xla::HloAsyncOutSendInstruction;
using xla::HloProto;
using xla::Shape;
using xla::StatusOr;
...
StatusOr<mlir::Operation*> LhloDialectEmitter::EmitOp(
  ...
  switch (instr->opcode()) {
    ...
    case HloOpcode::kAsyncOutSend:
      return EmitAsyncOutSendOp(instr);
...
StatusOr<Operation*> LhloDialectEmitter::EmitGemm(
  ...
  if (custom_call->operand_count() == 2) {
    ...
    TF_RETURN_IF_ERROR(set_bn_activation(gemm));
  ...
  if (custom_call->operand_count() == 3) {
    ...
    TF_RETURN_IF_ERROR(set_bn_activation(gemm_bias));
...
StatusOr<lmhlo::AsyncOutSendOp> LhloDialectEmitter::EmitAsyncOutSendOp(
    const HloInstruction* instr) {
  const HloAsyncOutSendInstruction* async_out =
      xla::Cast<HloAsyncOutSendInstruction>(instr);
  SmallVector<Value, 1> operands;
  TF_RETURN_IF_ERROR(GetOrCreateView(instr->operand(0), &operands));
  auto async_out_op =
      CreateOpWithoutAttrs<lmhlo::AsyncOutSendOp>(instr, operands);
  async_out_op.rendezvous_keyAttr(
      builder_.getStringAttr(async_out->rendezvous_key()));
  return async_out_op;
}
...
Status LhloDialectEmitter::Initialize() {
  ...
  // The operand of AsyncOut is treated as an output.
  absl::flat_hash_map<const BufferAllocation*, BufferAllocation::Index>
      allocation_to_async_out;
  for (auto* instr : computation_.instructions()) {
    if (instr->opcode() != xla::HloOpcode::kAsyncOutSend) {
      continue;
    }
    const HloInstruction* opnd = instr->operand(0);
    TF_ASSIGN_OR_RETURN(const BufferAllocation::Slice slice,
                        assignment_.GetUniqueSlice(opnd, {}));
    const BufferAllocation* alloc = slice.allocation();
    TF_RET_CHECK(slice.offset() == 0);
    TF_RET_CHECK(slice.size() == alloc->size());
    allocation_to_async_out[alloc] = alloc->index();
  }
  ...
  for (const BufferAllocation* alloc : ordered_allocations) {
    ...
    auto iter_async_out = allocation_to_async_out.find(alloc);
    if (iter_async_out != allocation_to_async_out.end()) {
      arg_attr_list.set("lmhlo.async_out_index",
