...
cudnnBatchNormOps_t GetBNOps(bool side_input,
...
struct MhaDescriptorDeleter {
  void operator()(cudnnAttnDescriptor_t descriptor) const {
    CHECK_CUDNN_OK(cudnnDestroyAttnDescriptor(descriptor));
  }
};
struct SeqDataDescriptorDeleter {
  void operator()(cudnnSeqDataDescriptor_t descriptor) const {
    CHECK_CUDNN_OK(cudnnDestroySeqDataDescriptor(descriptor));
  }
};
...
using MhaDescriptor = std::unique_ptr<cudnnAttnStruct, MhaDescriptorDeleter>;
using SeqDataDescriptor =
    std::unique_ptr<cudnnSeqDataStruct, SeqDataDescriptorDeleter>;
#if CUDNN_VERSION >= 7603
...
MhaDescriptor CreateMhaDescriptor() {
  cudnnAttnDescriptor_t result;
  CHECK_CUDNN_OK(cudnnCreateAttnDescriptor(&result));
  return MhaDescriptor(result);
}
SeqDataDescriptor CreateSeqDataDescriptor() {
  cudnnSeqDataDescriptor_t result;
  CHECK_CUDNN_OK(cudnnCreateSeqDataDescriptor(&result));
  return SeqDataDescriptor(result);
}
#if CUDNN_VERSION >= 7603
...
class CudnnTensorDescriptor {
 public:
  CudnnTensorDescriptor(const dnn::BatchDescriptor& batch_descriptor,
                        ...
                        absl::optional<int64_t> batch_limit = {})
    ...
    switch (batch_descriptor.layout()) {
      ...
      case dnn::DataLayout::kBatchDepthYX: {
        ...
        // For big tensors that have too many elements for int32 indexing,
        // 'batch_limit' can be used to cap the batch dimension. This shouldn't
        // affect the computation of strides in neither kBatchYXDepth nor
        // kBatchDepthYX format.
        if (batch_limit.has_value()) {
          dims64[0] = *batch_limit;
        }
    ...
    VLOG(1) << "Creating TensorDescriptor for Batch Descriptor "
...
bool DisableReducedPrecisionReduction() {
  static bool disable_reduced_precision_reduction = [] {
    bool disable_cudnn_reduced_precision_reduction = false;
    TF_CHECK_OK(tensorflow::ReadBoolFromEnvVar("TF_DISABLE_REDUCED_PRECISION_REDUCTION",
                                               /*default_val=*/false,
                                               &disable_cudnn_reduced_precision_reduction));
    return disable_cudnn_reduced_precision_reduction;
  }();
  return disable_reduced_precision_reduction;
}
...
// A helper function to decide whether to early exit when we have enough working
// heuristics engines.
int64_t GetCudnnHeuristicsLimit() {
  static int64_t heuristics_limit = [] {
    int64_t heuristics_limit;
    TF_CHECK_OK(tensorflow::ReadInt64FromEnvVar("TF_CUDNN_HEURISTICS_LIMIT",
                                               /*default_val=*/8,
                                               &heuristics_limit));
    return heuristics_limit;
  }();
  return heuristics_limit;
}
...
class CudnnMhaParamsDescriptor {
  typedef dnn::RnnDescriptor::ParamsRegions ParamsRegions;
  CudnnMhaParamsDescriptor(int64_t params_size_in_bytes, ParamsRegions weights,
 ...
 public:
  CudnnMhaParamsDescriptor(CudnnMhaParamsDescriptor&&) = default;

  static port::StatusOr<CudnnMhaParamsDescriptor> Create(
      const CudnnHandle& cudnn, cudnnDataType_t data_type,
      cudnnAttnDescriptor_t mha_desc);
 ...
 private:
  ...
  SE_DISALLOW_COPY_AND_ASSIGN(CudnnMhaParamsDescriptor);
};
...
class CudnnMhaDescriptor : public dnn::MhaDescriptor {
  CudnnMhaDescriptor(const CudnnHandle& cudnn, gpu::MhaDescriptor mha_desc,
                     int num_heads, int batch_size, int q_size, int k_size,
                     int v_size, int q_proj_size, int k_proj_size,
                     int v_proj_size, int o_proj_size, int seq_len_q,
                     int seq_len_k, size_t params_size_in_bytes,
                     size_t workspace_size_in_bytes,
                     size_t reserve_size_in_bytes, cudnnDataType_t data_type,
                     cudnnDataType_t comp_prec, float dropout_rate,
                     bool is_proj_bias, CudnnDropoutDescriptor dropout_desc)
      : mha_desc_(std::move(mha_desc)),
        num_heads_(num_heads),
        batch_size_(batch_size),
        q_size_(q_size),
        k_size_(k_size),
        v_size_(v_size),
        q_proj_size_(q_proj_size),
        k_proj_size_(k_proj_size),
        v_proj_size_(v_proj_size),
        o_proj_size_(o_proj_size),
        seq_len_q_(seq_len_q),
        seq_len_k_(seq_len_k),
        params_size_in_bytes_(params_size_in_bytes),
        workspace_size_in_bytes_(workspace_size_in_bytes),
        reserve_size_in_bytes_(reserve_size_in_bytes),
        data_type_(data_type),
        comp_prec_(comp_prec),
        dropout_rate_(dropout_rate),
        is_proj_bias_(is_proj_bias),
        dropout_desc_(std::move(dropout_desc)) {}

 public:
  CudnnMhaDescriptor(CudnnMhaDescriptor&& other) = default;

  static port::StatusOr<CudnnMhaDescriptor> Create(
      const CudnnHandle& cudnn, int num_heads, int batch_size, int q_size,
      int k_size, int v_size, int q_proj_size, int k_proj_size, int v_proj_size,
      int o_proj_size, int seq_len_q, int seq_len_k, cudnnDataType_t data_type,
      cudnnDataType_t comp_prec, float dropout_rate, bool is_proj_bias,
      uint64_t seed, ScratchAllocator* state_allocator) {
    SE_ASSIGN_OR_RETURN(CudnnDropoutDescriptor dropout_desc,
                        CudnnDropoutDescriptor::Create(cudnn, dropout_rate,
                                                       seed, state_allocator));

    gpu::MhaDescriptor mha_desc = CreateMhaDescriptor();
    ...
    if (tensorflow::tensor_float_32_execution_enabled()) {
      if (data_type == CUDNN_DATA_HALF) {
        ...
        // TODO(shuw): it also supports CUDNN_TENSOR_OP_MATH_ALLOW_CONVERSION.
    ...
    unsigned attn_mode = CUDNN_ATTN_QUERYMAP_ALL_TO_ONE;
    attn_mode |= is_proj_bias ? CUDNN_ATTN_ENABLE_PROJ_BIASES
                              : CUDNN_ATTN_DISABLE_PROJ_BIASES;
    RETURN_IF_CUDNN_ERROR(cudnnSetAttnDescriptor(
        /*attn_desc=*/mha_desc.get(), /*attnMode=*/attn_mode,
        /*numHeads=*/num_heads,
        /*smScaler=*/1.0, /*dataType=*/data_type, /*compPrec=*/comp_prec,
        /*mathType=*/math_type,
        dropout_rate > 0.0 ? dropout_desc.handle() : nullptr,
        /*postDropoutDesc=*/nullptr, /*qSize=*/q_size, /*kSize=*/k_size,
        /*vSize=*/v_size,
        /*qProjSize=*/q_proj_size, /*kProjSize=*/k_proj_size,
        /*vProjSize=*/v_proj_size,
        /*oProjSize=*/o_proj_size, /*seqLenQ=*/seq_len_q,
        /*seqLenK=*/seq_len_k, /*batchSize=*/batch_size,
        /*beamSize=*/beam_size));

    size_t size_weights = 0, size_workspace = 0, size_reserved = 0;

    RETURN_IF_CUDNN_ERROR(cudnnGetMultiHeadAttnBuffers(
        cudnn.handle(), mha_desc.get(), &size_weights, &size_workspace,
        &size_reserved));

    return CudnnMhaDescriptor(
        cudnn, std::move(mha_desc), num_heads, batch_size, q_size, k_size,
        v_size, q_proj_size, k_proj_size, v_proj_size, o_proj_size, seq_len_q,
        seq_len_k, size_weights, size_workspace, size_reserved, data_type,
        comp_prec, dropout_rate, is_proj_bias, std::move(dropout_desc));
  }

  cudnnAttnDescriptor_t handle() const { return mha_desc_.get(); }
  size_t ParamsSizeInBytes() const override { return params_size_in_bytes_; }
  size_t WorkspaceSizeInBytes() const { return workspace_size_in_bytes_; }
  size_t ReserveSizeInBytes() const { return reserve_size_in_bytes_; }

 private:
  gpu::MhaDescriptor mha_desc_;
  int num_heads_;
  int batch_size_;
  int q_size_;
  int k_size_;
  ...
  size_t reserve_size_in_bytes_;
  cudnnDataType_t data_type_;
  cudnnDataType_t comp_prec_;
  float dropout_rate_;
  bool is_proj_bias_;
  CudnnDropoutDescriptor dropout_desc_;
  SE_DISALLOW_COPY_AND_ASSIGN(CudnnMhaDescriptor);
};
...
class CudnnMhaSeqDataDescriptor : public dnn::MhaSeqDataDescriptor {
 public:
  CudnnMhaSeqDataDescriptor(GpuExecutor* parent, int batch_size,
                            int max_seq_length, int vect_size,
                            int seq_array_size, cudnnDataType_t data_type,
                            SeqDataDescriptor handle)
      : batch_size_(batch_size),
        max_seq_length_(max_seq_length),
        vect_size_(vect_size),
        seq_array_size_(seq_array_size),
        handle_(std::move(handle)) {}

 public:
  CudnnMhaSeqDataDescriptor(CudnnMhaSeqDataDescriptor&&) = default;

  static port::StatusOr<CudnnMhaSeqDataDescriptor> Create(
      GpuExecutor* parent, int batch_size, int max_seq_length, int vect_size,
      int seq_array_size, const absl::Span<const int>& seq_array,
      cudnnDataType_t data_type) {
    if (max_seq_length <= 0) {
      return port::Status(port::error::INVALID_ARGUMENT, "max_seq_length <= 0");
    }
    ...
    dims[CUDNN_SEQDATA_TIME_DIM] = max_seq_length;
    dims[CUDNN_SEQDATA_VECT_DIM] = vect_size;
    ...
    SeqDataDescriptor seq_desc = CreateSeqDataDescriptor();
    const int* seq_array_ptr = seq_array.data();
    RETURN_IF_CUDNN_ERROR(cudnnSetSeqDataDescriptor(
        /*tensorDesc=*/seq_desc.get(), /*dataType=*/data_type,
        /*nbDims=*/CUDNN_SEQDATA_DIM_COUNT, /*dimA=*/dims,
        /*dataAxes=*/data_axes, /*qSeqArraySize=*/seq_array_size,
        /*qSeqArray=*/seq_array_ptr, nullptr));
    return CudnnMhaSeqDataDescriptor(parent, batch_size, max_seq_length,
                                     vect_size, seq_array_size, data_type,
                                     std::move(seq_desc));
  }

  cudnnSeqDataDescriptor_t handle() const { return handle_.get(); }

  int max_seq_length() const { return max_seq_length_; }
  int batch_size() const { return batch_size_; }
  int vect_size() const { return vect_size_; }
  int seq_array_size() const { return seq_array_size_; }

 private:
  SeqDataDescriptor handle_;
  int batch_size_;
  int max_seq_length_;
  int vect_size_;
  int seq_array_size_;
  cudnnDataType_t data_type_;
  SE_DISALLOW_COPY_AND_ASSIGN(CudnnMhaSeqDataDescriptor);
};
...
port::StatusOr<size_t> GetBatchNormBackwardWorkspaceSizeInternal(
    const CudnnHandle& cudnn, const cudnnBatchNormMode_t& mode,
    const cudnnBatchNormOps_t& bn_ops,
    const cudnnActivationDescriptor_t& activation_desc,
    const CudnnTensorDescriptor& x_descriptor,
...
port::StatusOr<DeviceMemory<uint8>> CreateBatchNormBackwardWorkspace(
  ...
  size_t workspace_size_in_bytes = 0;
  SE_ASSIGN_OR_RETURN(workspace_size_in_bytes,
                      GetBatchNormBackwardWorkspaceSizeInternal(
                          cudnn, mode, bn_ops, activation_desc, x_descriptor,
                          scale_offset_descriptor));
  // Allocate the workspace.
  if (workspace_size_in_bytes == 0) {
...
bool CudnnSupport::DoMhaForward(Stream* stream,
                                const dnn::MhaDescriptor& mha_desc,
                                const dnn::MhaSeqDataDescriptor& input_q_desc,
                                const DeviceMemory<double>& input_q_data,
                                const dnn::MhaSeqDataDescriptor& input_k_desc,
                                const DeviceMemory<double>& input_k_data,
                                const dnn::MhaSeqDataDescriptor& input_v_desc,
                                const DeviceMemory<double>& input_v_data,
                                const DeviceMemory<int>& seq_lengths_qo_data,
                                const DeviceMemory<int>& seq_lengths_kv_data,
                                DeviceMemory<double>* params_data,
                                const dnn::MhaSeqDataDescriptor& output_desc,
                                DeviceMemory<double>* output_data,
                                int seq_len_q, int seq_len_k, bool is_training,
                                bool residual_link,
                                ScratchAllocator* reserve_space_allocator,
                                ScratchAllocator* workspace_allocator) {
  const CudnnMhaDescriptor& cudnn_mha_desc =
      static_cast<const CudnnMhaDescriptor&>(mha_desc);
  const CudnnMhaSeqDataDescriptor& cudnn_input_q_desc =
      static_cast<const CudnnMhaSeqDataDescriptor&>(input_q_desc);
  const CudnnMhaSeqDataDescriptor& cudnn_input_k_desc =
      static_cast<const CudnnMhaSeqDataDescriptor&>(input_k_desc);
  const CudnnMhaSeqDataDescriptor& cudnn_input_v_desc =
      static_cast<const CudnnMhaSeqDataDescriptor&>(input_v_desc);
  const CudnnMhaSeqDataDescriptor& cudnn_output_desc =
      static_cast<const CudnnMhaSeqDataDescriptor&>(output_desc);

  return IsStatusOk(
      DoMhaForwardImpl<double>(
          stream, cudnn_mha_desc, cudnn_input_q_desc, input_q_data,
          cudnn_input_k_desc, input_k_data, cudnn_input_v_desc, input_v_data,
          seq_lengths_qo_data, seq_lengths_kv_data, params_data,
          cudnn_output_desc, output_data, seq_len_q, seq_len_k, is_training,
          residual_link, reserve_space_allocator, workspace_allocator),
      /*report_error=*/true);
}
...
bool CudnnSupport::DoMhaForward(Stream* stream,
                                const dnn::MhaDescriptor& mha_desc,
                                const dnn::MhaSeqDataDescriptor& input_q_desc,
                                const DeviceMemory<Eigen::half>& input_q_data,
                                const dnn::MhaSeqDataDescriptor& input_k_desc,
                                const DeviceMemory<Eigen::half>& input_k_data,
                                const dnn::MhaSeqDataDescriptor& input_v_desc,
                                const DeviceMemory<Eigen::half>& input_v_data,
                                const DeviceMemory<int>& seq_lengths_qo_data,
                                const DeviceMemory<int>& seq_lengths_kv_data,
                                DeviceMemory<Eigen::half>* params_data,
                                const dnn::MhaSeqDataDescriptor& output_desc,
...
bool CudnnSupport::DoMhaForward(Stream* stream,
                                const dnn::MhaDescriptor& mha_desc,
                                const dnn::MhaSeqDataDescriptor& input_q_desc,
                                const DeviceMemory<float>& input_q_data,
                                const dnn::MhaSeqDataDescriptor& input_k_desc,
                                const DeviceMemory<float>& input_k_data,
                                const dnn::MhaSeqDataDescriptor& input_v_desc,
                                const DeviceMemory<float>& input_v_data,
                                const DeviceMemory<int>& seq_lengths_qo_data,
                                const DeviceMemory<int>& seq_lengths_kv_data,
                                DeviceMemory<float>* params_data,
                                const dnn::MhaSeqDataDescriptor& output_desc,
                                DeviceMemory<float>* output_data, int seq_len_q,
                                int seq_len_k, bool is_training,
                                bool residual_link,
                                ScratchAllocator* reserve_space_allocator,
                                ScratchAllocator* workspace_allocator) {
  const CudnnMhaDescriptor& cudnn_mha_desc =
...
bool CudnnSupport::DoMhaBackward(
    Stream* stream, const dnn::MhaDescriptor& mha_desc,
    const dnn::MhaSeqDataDescriptor& input_q_desc,
    const DeviceMemory<double>& input_q_data,
    const dnn::MhaSeqDataDescriptor& input_k_desc,
    const DeviceMemory<double>& input_k_data,
    const dnn::MhaSeqDataDescriptor& input_v_desc,
    const DeviceMemory<double>& input_v_data,
    const DeviceMemory<double>* params_data,
    const DeviceMemory<int>& seq_lengths_qo_data,
    const DeviceMemory<int>& seq_lengths_kv_data,
    const dnn::MhaSeqDataDescriptor& output_desc,
    const DeviceMemory<double>& output_data,
    const DeviceMemory<double>& output_backprop_data,
    DeviceMemory<double>* input_q_backprop_data,
    DeviceMemory<double>* input_k_backprop_data,
    DeviceMemory<double>* input_v_backprop_data,
    DeviceMemory<double>* params_backprop_data, int seq_len_q, int seq_len_k,
    DeviceMemory<uint8>* reserve_space_uint8,
    ScratchAllocator* workspace_allocator) {
  const CudnnMhaDescriptor& cudnn_mha_desc =
      static_cast<const CudnnMhaDescriptor&>(mha_desc);
  const CudnnMhaSeqDataDescriptor& cudnn_input_q_desc =
  ...
  return IsStatusOk(
      DoMhaBackwardImpl<double>(
          stream, cudnn_mha_desc, cudnn_input_q_desc, input_q_data,
          cudnn_input_k_desc, input_k_data, cudnn_input_v_desc, input_v_data,
          seq_lengths_qo_data, seq_lengths_kv_data, params_data,
          cudnn_output_desc, output_data, output_backprop_data,
          input_q_backprop_data, input_k_backprop_data, input_v_backprop_data,
          params_backprop_data, seq_len_q, seq_len_k, reserve_space_uint8,
          workspace_allocator),
      /*report_error=*/true);
}
...
bool CudnnSupport::DoMhaBackward(
    ...
    const dnn::MhaSeqDataDescriptor& input_v_desc,
    const DeviceMemory<float>& input_v_data,
    const DeviceMemory<float>* params_data,
    const DeviceMemory<int>& seq_lengths_qo_data,
    const DeviceMemory<int>& seq_lengths_kv_data,
    const dnn::MhaSeqDataDescriptor& output_desc,
    const DeviceMemory<float>& output_data,
    const DeviceMemory<float>& output_backprop_data,
    DeviceMemory<float>* input_q_backprop_data,
    DeviceMemory<float>* input_k_backprop_data,
    DeviceMemory<float>* input_v_backprop_data,
    DeviceMemory<float>* params_backprop_data, int seq_len_q, int seq_len_k,
    DeviceMemory<uint8>* reserve_space_uint8,
    ScratchAllocator* workspace_allocator) {
  const CudnnMhaDescriptor& cudnn_mha_desc =
      static_cast<const CudnnMhaDescriptor&>(mha_desc);
...
bool CudnnSupport::DoMhaBackward(
    ...
    const dnn::MhaSeqDataDescriptor& input_v_desc,
    const DeviceMemory<Eigen::half>& input_v_data,
    const DeviceMemory<Eigen::half>* params_data,
    const DeviceMemory<int>& seq_lengths_qo_data,
    const DeviceMemory<int>& seq_lengths_kv_data,
    const dnn::MhaSeqDataDescriptor& output_desc,
    const DeviceMemory<Eigen::half>& output_data,
    const DeviceMemory<Eigen::half>& output_backprop_data,
    DeviceMemory<Eigen::half>* input_q_backprop_data,
    DeviceMemory<Eigen::half>* input_k_backprop_data,
    DeviceMemory<Eigen::half>* input_v_backprop_data,
    DeviceMemory<Eigen::half>* params_backprop_data, int seq_len_q,
    int seq_len_k, DeviceMemory<uint8>* reserve_space_uint8,
    ScratchAllocator* workspace_allocator) {
  const CudnnMhaDescriptor& cudnn_mha_desc =
      static_cast<const CudnnMhaDescriptor&>(mha_desc);
  const CudnnMhaSeqDataDescriptor& cudnn_input_q_desc =
...
template <class T>
port::Status CudnnSupport::DoMhaForwardImpl(
    Stream* stream, const CudnnMhaDescriptor& mha_desc,
    const CudnnMhaSeqDataDescriptor& input_q_desc,
    const DeviceMemory<T>& input_q_data,
    const CudnnMhaSeqDataDescriptor& input_k_desc,
    const DeviceMemory<T>& input_k_data,
    const CudnnMhaSeqDataDescriptor& input_v_desc,
    const DeviceMemory<T>& input_v_data,
    const DeviceMemory<int>& seq_lengths_qo_data,
    const DeviceMemory<int>& seq_lengths_kv_data, DeviceMemory<T>* params_data,
    const CudnnMhaSeqDataDescriptor& output_desc, DeviceMemory<T>* output_data,
    int seq_len_q, int seq_len_k, bool is_training, bool residual_link,
    ScratchAllocator* reserve_space_allocator,
    ScratchAllocator* workspace_allocator) {
  ...
  DeviceMemory<uint8> weights_space;

  int currIdx = -1;
  std::vector<int> lo_win_idx(seq_len_q, 0);
  std::vector<int> hi_win_idx(seq_len_q, seq_len_k);

  size_t size_weights = 0, size_workspace = 0, size_reserve = 0;
  if (is_training) {
    RETURN_IF_CUDNN_ERROR(cudnnGetMultiHeadAttnBuffers(
        cudnn.handle(), mha_desc.handle(), &size_weights, &size_workspace,
        &size_reserve));
  } else {
    RETURN_IF_CUDNN_ERROR(
        cudnnGetMultiHeadAttnBuffers(cudnn.handle(), mha_desc.handle(),
                                     &size_weights, &size_workspace, nullptr));
  }

  if (size_workspace > 0) {
    SE_ASSIGN_OR_RETURN(workspace,
                        workspace_allocator->AllocateBytes(size_workspace));
  }

  if (size_reserve > 0) {
    SE_ASSIGN_OR_RETURN(reserve_space,
                        reserve_space_allocator->AllocateBytes(size_reserve));
  }

  if (is_training) {
    RETURN_IF_CUDNN_ERROR(cudnnMultiHeadAttnForward(
        /*handle=*/cudnn.handle(), /*mhaDesc=*/mha_desc.handle(),
        /*currIdx=*/currIdx, /*loWinIdx=*/lo_win_idx.data(),
        /*hiWinIdx=*/hi_win_idx.data(),
        /*devSeqLengthsQO=*/
        static_cast<const int*>(seq_lengths_qo_data.opaque()),
        /*devSeqLengthsKV=*/
        static_cast<const int*>(seq_lengths_kv_data.opaque()),
        /*qDesc=*/input_q_desc.handle(),
        /*queries=*/input_q_data.opaque(),
        /*residuals*/ residual_link ? input_q_data.opaque() : nullptr,
        /*kDesc=*/input_k_desc.handle(), /*keys=*/input_k_data.opaque(),
        /*vesc=*/input_v_desc.handle(), /*values=*/input_v_data.opaque(),
        /*oDesc=*/output_desc.handle(), /*out=*/output_data->opaque(),
        /*weightSizeInBytes=*/size_weights,
        /*weights=*/size_weights > 0 ? params_data->opaque() : nullptr,
        /*workSpaceSizeInBytes=*/size_workspace,
        /*workSpace=*/workspace.opaque(),
        /*reserveSpaceSizeInBytes=*/size_reserve,
    ...
    RETURN_IF_CUDNN_ERROR(cudnnMultiHeadAttnForward(
        ...
        /*weights=*/size_weights > 0 ? params_data->opaque() : nullptr,
        /*workSpaceSizeInBytes=*/size_workspace,
        /*workSpace=*/size_workspace > 0 ? workspace.opaque() : nullptr,
...
template <class T>
port::Status CudnnSupport::DoMhaBackwardImpl(
    Stream* stream, const CudnnMhaDescriptor& mha_desc,
    const CudnnMhaSeqDataDescriptor& input_q_desc,
    const DeviceMemory<T>& input_q_data,
    const CudnnMhaSeqDataDescriptor& input_k_desc,
    ...
    const DeviceMemory<int>& seq_lengths_qo_data,
    const DeviceMemory<int>& seq_lengths_kv_data,
    const DeviceMemory<T>* params_data,
    const CudnnMhaSeqDataDescriptor& output_desc,
    const DeviceMemory<T>& output_data,
    const DeviceMemory<T>& output_backprop_data,
    DeviceMemory<T>* input_q_backprop_data,
    DeviceMemory<T>* input_k_backprop_data,
    DeviceMemory<T>* input_v_backprop_data,
    DeviceMemory<T>* params_backprop_data, int seq_len_q, int seq_len_k,
    DeviceMemory<uint8>* reserve_space_uint8,
    ScratchAllocator* workspace_allocator) {
  auto cudnn = cudnn_->GetHandle(parent_, stream);
  ...
  RETURN_IF_CUDNN_ERROR(cudnnMultiHeadAttnBackwardData(
      /*handle=*/cudnn.handle(), /*attnDesc=*/mha_desc.handle(),
      /*loWinIdx=*/lo_win_idx.data(), /*hiWinIdx=*/hi_win_idx.data(),
      /*devSeqLengthsQO=*/
      static_cast<const int*>(seq_lengths_qo_data.opaque()),
      /*devSeqLengthsKV=*/
      static_cast<const int*>(seq_lengths_kv_data.opaque()),
      /*doDesc=*/output_desc.handle(),
      /*dout=*/output_backprop_data.opaque(),
      /*dqDesc=*/input_q_desc.handle(),
      /*dqueries=*/input_q_backprop_data->opaque(),
      /*queries=*/input_q_data.opaque(),
      /*dkDesc=*/input_k_desc.handle(),
      /*dkeys=*/input_k_backprop_data->opaque(),
      /*keys=*/input_k_data.opaque(),
      /*dvDesc=*/input_v_desc.handle(),
      /*dvalues=*/input_v_backprop_data->opaque(),
      /*values=*/input_v_data.opaque(),
      /*weightSizeInBytes=*/ size_weights,
      /*weights=*/size_weights > 0 ? params_data->opaque() : nullptr,
      /*workSpaceSizeInBytes=*/size_workspace,
      /*workSpace=*/workspace.opaque(),
      /*reserveSpaceSizeInBytes=*/reserve_space_uint8->size(),
      /*reserveSpace=*/reserve_space_uint8->opaque()));
  if (params_backprop_data != nullptr) {
    // Clear the dw to zeros.
    stream->ThenMemZero(params_backprop_data, params_backprop_data->size());
    RETURN_IF_CUDNN_ERROR(cudnnMultiHeadAttnBackwardWeights(
        /*handle=*/cudnn.handle(), /*attnDesc=*/mha_desc.handle(),
        /*addGrad=*/CUDNN_WGRAD_MODE_ADD,
        /*qDesc=*/input_q_desc.handle(),
        /*queries=*/input_q_data.opaque(),
        /*kDesc=*/input_k_desc.handle(),
        /*keys=*/input_k_data.opaque(),
        /*vDesc=*/input_v_desc.handle(),
        /*values=*/input_v_data.opaque(),
        /*doDesc=*/output_desc.handle(),
        /*dout=*/output_backprop_data.opaque(),
        /*weightSizeInBytes=*/ size_weights,
        /*weights=*/size_weights > 0 ? params_data->opaque() : nullptr,
        /*dweights=*/size_weights > 0 ? params_backprop_data->opaque() : nullptr,
        /*workSpaceSizeInBytes=*/size_workspace,
        /*workSpace=*/workspace.opaque(),
        /*reserveSpaceSizeInBytes=*/reserve_space_uint8->size(),
        /*reserveSpace=*/reserve_space_uint8->opaque()));
...
port::StatusOr<std::unique_ptr<dnn::MhaDescriptor>>
CudnnSupport::createMhaDescriptor(int num_heads, int batch_size, int q_size,
                                  int k_size, int v_size, int q_proj_size,
                                  int k_proj_size, int v_proj_size,
                                  int o_proj_size, int seq_len_q, int seq_len_k,
                                  dnn::DataType data_type,
                                  dnn::DataType comp_prec, float dropout_rate,
                                  bool is_proj_bias, uint64_t seed,
                                  ScratchAllocator* state_allocator) {
  auto cudnn = cudnn_->GetHandle(parent_, /*stream=*/nullptr);
  SE_ASSIGN_OR_RETURN(
      CudnnMhaDescriptor mha_desc,
      CudnnMhaDescriptor::Create(
          cudnn, num_heads, batch_size, q_size, k_size, v_size, q_proj_size,
          k_proj_size, v_proj_size, o_proj_size, seq_len_q, seq_len_k,
          ToCudnnDataType(data_type), ToCudnnDataType(comp_prec), dropout_rate,
          is_proj_bias, seed, state_allocator));
  return std::unique_ptr<dnn::MhaDescriptor>(
      new CudnnMhaDescriptor(std::move(mha_desc)));
}

port::StatusOr<std::unique_ptr<dnn::MhaSeqDataDescriptor>>
CudnnSupport::createMhaSeqDataDescriptor(int batch_size, int max_seq_length,
                                         int vect_size, int seq_array_size,
                                         const absl::Span<const int>& seq_array,
                                         dnn::DataType data_type) {
  SE_ASSIGN_OR_RETURN(
      CudnnMhaSeqDataDescriptor descriptor,
      CudnnMhaSeqDataDescriptor::Create(parent_, batch_size, max_seq_length,
                                        vect_size, seq_array_size, seq_array,
                                        ToCudnnDataType(data_type)));

  return std::unique_ptr<dnn::MhaSeqDataDescriptor>(
      new CudnnMhaSeqDataDescriptor(std::move(descriptor)));
}
...
bool GenericEngineFilter(cudnnBackendDescriptor_t engine_config,
  ...
  if (disable_reduced_precision_reduction) {
    ret |= cudnn_frontend::hasNumericalNote<CUDNN_NUMERICAL_NOTE_REDUCED_PRECISION_REDUCTION>(
        engine_config);
  }return ret;
}
...
namespace {
// There is a potential race condition issue in cudnn when "heuristics_mode_b"
// is used in the context of multithreading. To avoid this, we add locking
// mechanism when querying the heuristics.
// TODO(kaixih@nvidia): Remove this when cudnn fixes the issue.
class CudnnHeuristics {
 public:
  port::Status Get(
      const std::unique_ptr<cudnn_frontend::OperationGraph>& op_graph,
      std::function<bool(cudnnBackendDescriptor_t)> filter_fn,
      bool use_fallback, cudnn_frontend::EngineConfigList& filtered_configs) {
    absl::MutexLock lock(&mutex_);
    std::vector<cudnnStatus_t> statuses =
        cudnn_frontend::get_heuristics_list<1>(
            std::array<std::string, 1>{use_fallback ? "heuristics_fallback"
                                                    : "heuristics_mode_b"},
            *op_graph, filter_fn, filtered_configs);

    for (auto& status : statuses) {
      if (!SE_PREDICT_TRUE(status == CUDNN_STATUS_SUCCESS)) {
        return port::Status(port::error::UNKNOWN, "get_heuristics_list error");
      }
    }
 ...
 private:
  CudnnHeuristics() = default;
  absl::Mutex mutex_;
  friend class CudnnHeuristicsSingleton;
  TF_DISALLOW_COPY_AND_ASSIGN(CudnnHeuristics);
};

class CudnnHeuristicsSingleton {
 public:
  static CudnnHeuristics* GetInstance() {
    static CudnnHeuristics* instance = new CudnnHeuristics();
    return instance;
  }
};
}  // namespace
...
GetFirstWorkingExecutionPlan(
  ...
  auto generic_filter_fn = [=](cudnnBackendDescriptor_t engine_config) -> bool {
    return GenericEngineFilter(
        ...
        /*disable_reduced_precision_reduction*/ DisableReducedPrecisionReduction());
  };

  cudnn_frontend::EngineConfigList filtered_configs;
  std::vector<cudnnStatus_t> heuristics_statuses =
      cudnn_frontend::get_heuristics_list<2>(
            {"heuristics_mode_b", "heuristics_fallback"},
             *op_graph, generic_filter_fn, filtered_configs);
  for (auto &status : heuristics_statuses) {
    if (!SE_PREDICT_TRUE(status == CUDNN_STATUS_SUCCESS)) {
      return port::Status(port::error::UNKNOWN,
                          "cuDNN's get_heuristics_list error");  
    }
  }
...
port::Status CudnnSupport::GetConvolveRunners(
  ...
  CudnnHeuristicsSingleton::GetInstance()->Get(op_graph, generic_filter_fn,
                                               use_fallback, filtered_configs);
...
#if CUDNN_VERSION >= 8300
    // Early exit when finding TF_CUDNN_HEURISTICS_LIMIT (default: 8) number
    // of working heuristics engines. If none of the heuristics engines are
    // found, we collect all working fallback engines.
    if (heuristics_count >= GetCudnnHeuristicsLimit()) {
      break;
...
port::Status CudnnSupport::GetFusedConvolveRunners(
  ...
  for (int i = 0; i < filtered_configs.size(); i++) {
    ...
    // Early exit when finding TF_CUDNN_HEURISTICS_LIMIT (default: 8) number
    // of working heuristics engines. If none of the heuristics engines are
    // found, we collect all working fallback engine.
    if (heuristics_count >= GetCudnnHeuristicsLimit()) {
      break;
    } else if (!use_fallback) {
      heuristics_count++;
...
bool CudnnSupport::GetBatchNormalizationWorkspaceSize(
  ...
  return IsStatusOk(GetBatchNormalizationWorkspaceSizeImpl(
                        stream, input_data_type, scale_data_type, x_desc,
                        scale_offset_desc, workspace_size_in_bytes, kind, mode,
                        apply_side_input, apply_side_input_backprop),
                    /*report error*/ true);
}

port::Status CudnnSupport::GetBatchNormalizationWorkspaceSizeImpl(
    Stream* stream, dnn::DataType input_data_type,
    dnn::DataType scale_data_type, const dnn::BatchDescriptor& x_desc,
    const dnn::BatchDescriptor& scale_offset_desc,
    size_t* workspace_size_in_bytes,
  ...
  const cudnnBatchNormOps_t bn_ops_fwd =
      GetBNOps(apply_side_input, activation_mode);
  const cudnnBatchNormOps_t bn_ops_bwd =
      GetBNOps(apply_side_input_backprop, activation_mode);
  CudnnActivationDescriptor activation_desc(
      activation_mode, CUDNN_PROPAGATE_NAN, x_desc.value_max());
  switch (kind) {
    case stream_executor::BatchNormalizationKind::kBatchnormForward: {
      SE_ASSIGN_OR_RETURN(*workspace_size_in_bytes,
                          GetBatchNormForwardWorkspaceSizeInternal(
                              cudnn, mode, bn_ops_fwd, activation_desc.handle(),
    ...
    case stream_executor::BatchNormalizationKind::kBatchnormBackward: {
      SE_ASSIGN_OR_RETURN(*workspace_size_in_bytes,
                          GetBatchNormBackwardWorkspaceSizeInternal(
                              cudnn, mode, bn_ops_bwd, activation_desc.handle(),
                              x_descriptor, scale_offset_descriptor));
      break;
    }
    default:
      return port::InternalError(
          absl::StrCat("Unexpected batchnorm kind ", static_cast<int>(kind)));
  }
#endif
  return port::Status::OK();
}
    ...
    if (workspace_allocator != nullptr) {
      SE_ASSIGN_OR_RETURN(
          ...
          CreateBatchNormBackwardWorkspace(
              stream, cudnn, mode, bn_ops, activation_desc.handle(),
              x_descriptor, scale_offset_descriptor, workspace_allocator))
    }
...
// 2^31-1 elements. For pooling operations, if we are dealing with big tensors
// that have too many elements for int32 indexing, we can split the tensor
// [N,...] along the batch axis to multiple [N0,...], [N1,...] and then call the
// Cudnn pooling APIs sequentially.
...
GetTensorSplits(const dnn::BatchDescriptor& input_descriptor,
  ...
  int64_t elements_per_batch_input = input_descriptor.feature_map_count() *
                                     input_descriptor.height() *
                                     input_descriptor.width();

  int64_t elements_per_batch_output = output_descriptor.feature_map_count() *
                                      output_descriptor.height() *
                                      output_descriptor.width();

  int64_t total_elements = input_descriptor.ElementCount();
  int64_t max_batches_per_split =
      std::numeric_limits<int>::max() / elements_per_batch_input;
  if (max_batches_per_split == 0) {
    return port::Status(port::error::INTERNAL, absl::StrCat(
        "Tensor has too many elements for int32 indexing: batches=",
        num_batches, " elements_per_batch=", elements_per_batch_input, "."));
  }
  // Each tuple of the returned vector contains three values: (1) number of
  // batches in the split; (2) the split's offset (in elements) to the
  // flattened input; (3) the split's offset (in elements) to the flattened
  // output.
  ...
  while (processed_batches < num_batches) {
    ...
    out.push_back(std::make_tuple(num_batches_per_split, offset_input,
                                  offset_output));
...
bool CudnnSupport::DoPoolForward(
  ...
  auto splits_or = GetTensorSplits(input_dimensions, output_dimensions);
  if (!splits_or.ok()) return false;
  ...
  for (int i = 0; i < splits.size(); i++) {
    ...
    const auto status = cudnn_launcher(
        src_desc, dest_desc,
        input_data.opaque() + std::get<1>(splits[i]) * sizeof(double),
        output_data->opaque() + std::get<2>(splits[i]) * sizeof(double));
...
bool CudnnSupport::DoPoolForward(
  ...
  for (int i = 0; i < splits.size(); i++) {
    ...
    const auto status = cudnn_launcher(
        src_desc, dest_desc,
        input_data.opaque() + std::get<1>(splits[i]) * sizeof(float),
        output_data->opaque() + std::get<2>(splits[i]) * sizeof(float));
    if (!IsStatusOk(status, /*report_error=*/true)) {
      return false;
    }
  }
...
bool CudnnSupport::DoPoolForward(
  ...
  for (int i = 0; i < splits.size(); i++) {
    ...
    const auto status = cudnn_launcher(
        src_desc, dest_desc,
        input_data.opaque() + std::get<1>(splits[i]) * sizeof(Eigen::half),
        output_data->opaque() + std::get<2>(splits[i]) * sizeof(Eigen::half));
    if (!IsStatusOk(status, /*report_error=*/true)) {
      return false;
    }
  }
...
bool CudnnSupport::DoPoolBackward(
  ...
  for (int i = 0; i < splits.size(); i++) {
    ...
    const auto status = cudnn_launcher(
        src_desc, dest_desc,
        output_data.opaque() + std::get<2>(splits[i]) * sizeof(double),
        input_diff_data.opaque() + std::get<2>(splits[i]) * sizeof(double),
        input_data.opaque() + std::get<1>(splits[i]) * sizeof(double),
        output_diff_data->opaque() + std::get<1>(splits[i]) * sizeof(double));
    if (!IsStatusOk(status, /*report_error=*/true)) {
      return false;
    }
  }
...
bool CudnnSupport::DoPoolBackward(
  ...
  for (int i = 0; i < splits.size(); i++) {
    ...
    const auto status = cudnn_launcher(
        src_desc, dest_desc,
        output_data.opaque() + std::get<2>(splits[i]) * sizeof(float),
        input_diff_data.opaque() + std::get<2>(splits[i]) * sizeof(float),
        input_data.opaque() + std::get<1>(splits[i]) * sizeof(float),
        output_diff_data->opaque() + std::get<1>(splits[i]) * sizeof(float));
    if (!IsStatusOk(status, /*report_error=*/true)) {
      return false;
    }
  }
...
bool CudnnSupport::DoPoolBackward(
  ...
  for (int i = 0; i < splits.size(); i++) {
    ...
    const auto status = cudnn_launcher(
        src_desc, dest_desc,
        output_data.opaque() + std::get<2>(splits[i]) * sizeof(Eigen::half),
        input_diff_data.opaque() +
            std::get<2>(splits[i]) * sizeof(Eigen::half),
        input_data.opaque() + std::get<1>(splits[i]) * sizeof(Eigen::half),
        output_diff_data->opaque() +
            std::get<1>(splits[i]) * sizeof(Eigen::half));
    if (!IsStatusOk(status, /*report_error=*/true)) {
      return false;
    }
  }
