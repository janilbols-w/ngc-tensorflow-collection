 ...
 public:
  ...
  Status Validate() {
    ...
    if (params.use_implicit_batch && repl.is_tensor()) {
      return errors::InvalidArgument("Conversion for Tile is not implemented for multipliers "
    ...
    if (multiplies) {
      ...
      if (mult_numb != nb_dims) {
        return errors::InvalidArgument("The length of the replication vector (", mult_numb,
                                       ") of the Tile operation in '", params.node_def.name(),
                                       "' is expected to be equal to the rank of the input vector (",
                                       nb_dims, ").");
      }

      if (std::any_of(multiplies, multiplies+nb_dims, [](int i){ return i <= 0; })) {
      ...
      if (repl_dims.nbDims != 1) {
        return errors::InvalidArgument("When replications are defined as a tensor, that tensor must be "
                                       "1-dimensional. Got ", repl_dims.nbDims, "-dimensional tensor.");
      }

      // Check the number of elements in multiplyer for tensors with non-dynamic shape
      if (repl_dims.d[0] >= 0 && repl_dims.d[0] != nb_dims) {
        return errors::InvalidArgument("When replications are defined as a tensor, "
  ...
  Status Convert() {
    ...
    if (!dynamic_flag) {
      ...
      dynamic_flag = std::any_of(pSize+1-dim_adj, pSize+nb_dims, [](int i){ return i < 0; });
    ...
    if (tensor.is_weights() || dynamic_flag && replics.is_weights()) {
      builder = TRTNetworkBuilder::Create(converter->network(), params.weight_store);
    ...
    if (dynamic_flag) {
      ...
      if (replics.is_weights()) {
        StatusOr<nvinfer1::IConstantLayer*> weights_const =
                builder->WeightsToConstant(replics.weights().GetTrtWeights(), replics.GetTrtDims());
        TRT_ENSURE_PTR_OK(weights_const);
      ...
      target_shape = network->addElementWise(*shape, *mult, nvinfer1::ElementWiseOperation::kPROD)
                            ->getOutput(0);
    }

    nvinfer1::Dims start{nb_dims, {}};
    DimsAdapter stride(std::vector<int>(nb_dims, 1));
    auto layer = network->addSlice(input_trt_tensor, start, size, stride.AsTrtDims());
