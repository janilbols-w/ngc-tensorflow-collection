...
void UpdateLaunchDimensions(const LaunchDimensions& launch_dims, Thunk* thunk,
  ...
  auto kernel_name = kernel_thunk->kernel_name();
...
static std::string ToString(mlir::Value v) {
  std::string str;
...
Status IrEmitterUnnested::EmitConvolutionThunk(mlir::Operation* op) {
  ...
  auto fill_conv_descriptor = [&](auto op, std::string xla_cluster) {
    ...
    descriptor.op_type = op.op_type().str();
    descriptor.op_name = op.op_name().str();
    descriptor.cluster_name = xla_cluster;
  ...
  std::string cluster_name = mlir::GetNameFromLoc(module.getLoc());
  if (auto conv = dyn_cast<ConvForwardOp>(op)) {
    descriptor.kind = CudnnConvKind::kForward;
    fill_conv_descriptor(conv, cluster_name);
...
Status IrEmitterUnnested::EmitGemmThunk(mlir::Operation* op) {
  auto make_thunk_for_gemm =
      [&](auto op, std::string xla_cluster,
          absl::optional<BufferAllocation::Slice> bias = absl::nullopt,
          absl::optional<double> gemm_bias_beta = absl::nullopt,
          bool implements_whole_instruction = true,
          bool is_gemm_bias_epilogue_fusion =
              false) -> StatusOr<std::unique_ptr<Thunk>> {
    ...
    config.op_type = op.op_type().str();
    config.op_name = op.op_name().str();
    config.cluster_name = xla_cluster;
    backend.Clear();
    if (op.algorithm()) {
    ...
    TF_ASSIGN_OR_RETURN(stream_executor::dnn::ActivationMode activation_mode,
                        ConvertConvActivationMode(op.activation_mode()));
    backend.set_activation_mode(static_cast<int64>(activation_mode));
    ...
    // Bias is part of the inputs only if it is a gemm+bias epilogue fusion.
    if (bias.has_value() && is_gemm_bias_epilogue_fusion) {
      inputs.push_back(bias.value());
    }

    return std::unique_ptr<Thunk>(
        new GemmThunk(GetThunkInfo(op), std::move(config), std::move(inputs),
                      output, implements_whole_instruction));
  };
  ...
  TF_ASSIGN_OR_RETURN(auto thunk, [&]() -> StatusOr<std::unique_ptr<Thunk>> {
    if (auto gemm = mlir::dyn_cast<mlir::lmhlo_gpu::GEMMOp>(op)) {
      return make_thunk_for_gemm(gemm, cluster_name);
    }
    ...
    if (auto gemm = mlir::dyn_cast<mlir::lmhlo_gpu::GEMM_BiasOp>(op)) {
      ...
      if (VLOG_IS_ON(2)) {
        VLOG(2) << "Emitting gem_bias thunk with  "
                << "lhs: " << ToString(gemm.lhs())
                << "rhs: " << ToString(gemm.rhs())
                << "bias: " << ToString(gemm.bias())
                << "output: " << ToString(gemm.output())
                << "beta: " << gemm_bias_beta;
      }
      // If it is a gemm_bias_epilogue fusion (using cublasLt), the bias
      // broadcast is implicit (i.e, fused). Hence, the shape of the bias is not
      // equal to the output and beta parameter is set to 0.
      bool is_bias_not_broadcasted =
          (gemm.bias().getType().cast<mlir::MemRefType>().getShape() !=
           gemm.output().getType().cast<mlir::MemRefType>().getShape());
      if (is_bias_not_broadcasted) {
        DCHECK_EQ(gemm_bias_beta, 0)
            << "Incorrect lowering. When bias broadcast is implicit, beta "
               "should be 0";
        VLOG(2) << "Emitting thunk for gemm+bias epilogue fusion";
        return make_thunk_for_gemm(gemm, cluster_name, bias, gemm_bias_beta,
                                   true, true);
      }
      // If the bias is broadcasted, the bias is passed inside the output
      // buffer. If those buffers are shared we can just use it, otherwise copy
      // the bias values into the output buffer first.
      if (bias == output) {
        return make_thunk_for_gemm(gemm, cluster_name, bias, gemm_bias_beta);
      }
      ...
      TF_ASSIGN_OR_RETURN(
          auto thunk,
          make_thunk_for_gemm(gemm, cluster_name, bias, gemm_bias_beta,
...
Status IrEmitterUnnested::EmitBatchNormThunk(mlir::Operation* op) {
  ...
  if (auto bn_grad =
          mlir::dyn_cast<mlir::lmhlo_gpu::BatchNormGradOp>(op)) {
    // mlir::ArrayRef<NamedValue> optional_values;
    std::vector<NamedValue> optional_values{};
...
Status IrEmitterUnnested::EmitAsyncOutSend(mlir::Operation* op) {
  auto async_out_op = mlir::cast<mlir::lmhlo::AsyncOutSendOp>(op);
  TF_ASSIGN_OR_RETURN(BufferAllocation::Slice slice,
                      GetAllocationSlice(async_out_op.operand()));
  const Shape& shape = TypeToShape(async_out_op.operand().getType());
  ShapedSlice source_slice = ShapedSlice{slice, shape};

  AddThunkToThunkSequence(absl::make_unique<AsyncOutSendThunk>(
      GetThunkInfo(op), std::move(source_slice),
      async_out_op.rendezvous_key().str()));
  return Status::OK();
}
...
static IrArray::Index GetUnnormalizedIndex(
  ...
  if (simplify && unnormalized_shape.rank() == 2 && unnormalized_shape.has_layout() &&
...
Status IrEmitterUnnested::EmitOp(mlir::Operation* op) {
  ...
  if (mlir::isa<mlir::lmhlo::AsyncOutSendOp>(op)) {
    return EmitAsyncOutSend(op);
  }
