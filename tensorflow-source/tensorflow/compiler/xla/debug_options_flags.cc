...
static void AllocateFlags() {
  ...
  // Custom "sub-parser" lambda for xla_gpu_llvm_file.
  auto setter_for_xla_gpu_llvm_file = [](string value) {
    flag_values->add_xla_gpu_llvm_file(value);
    return true;
  };
  ...
  flag_objects->push_back(tensorflow::Flag(
      "xla_gpu_llvm_file", setter_for_xla_gpu_llvm_file, "",
      "If non-empty, specifies a file containing llvm to use. The filename "
      "prefix must have the same pattern as LLVM dumped by XLA. This allows to "
      "match one specific module. General workflow. Get the generated module "
      "llvm from XLA, modify it, then pass it back via this option."));
  flag_objects->push_back(tensorflow::Flag(
