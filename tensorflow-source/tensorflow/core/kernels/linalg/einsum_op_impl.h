...
struct EinsumHelper {
  ...
  // their DimensionType.
  static Status ParseEquationImpl(
      ...
      gtl::InlinedVector<bool, 2>* input_has_ellipsis,
      bool* output_has_ellipsis, bool* support_cutensor) {
    ...
    *support_cutensor = !trivial_reduction && !single_labels &&
  ...
  static Status ParseEquation(const string& equation,
    ...
    bool cutensor_support = false;
    return ParseEquationImpl(equation, input_labels, output_labels, label_types,
  ...
  static Status ParseEquationWithCuTensorSupport(
      EinsumOpInputFeatures& input_features) {
    return ParseEquationImpl(
        input_features.equation, &(input_features.input_labels),
        &(input_features.output_labels), &(input_features.label_types),
        &(input_features.input_label_counts),
        &(input_features.output_label_counts),
        &(input_features.input_has_ellipsis),
        &(input_features.output_has_ellipsis),
        &(input_features.support_cutensor));
  }
...
#undef NDIMS_CASE
  ...
  template <typename Device, typename T>
  struct ReduceRank2TensorFunctor {
  ...
  template <typename T>
  struct ReduceRank2TensorFunctor<GPUDevice, T> {
    void operator()(OpKernelContext* ctx, const Tensor* input, Tensor* output,
                    const int64 output_size, const int64 reduce_size) {
      if (UseCuTensorEinsum()) {
        functor::EinsumCutensorGpuFunctor<T>::ReduceRank2Tensor(
            ctx, input, output, output_size, reduce_size);
      } else {
        functor::ReduceFunctor<GPUDevice, Eigen::internal::SumReducer<T>>::
            Reduce(ctx, output->shaped<T, 1>({output_size}),
                   const_cast<const Tensor&>(*input).shaped<T, 2>(
                       {output_size, reduce_size}),
                   Eigen::array<typename TTypes<T>::Tensor::Index, 1>({1}),
                   Eigen::internal::SumReducer<T>());
  ...
  static Status ReduceOperand(OpKernelContext* ctx, const Tensor& input,
    ...
    ReduceRank2TensorFunctor<Device, T> rank2_tensor_reduction_func;
    rank2_tensor_reduction_func(ctx, &input_deduped, output, output_size,
                                reshape[kReduce]);
  ...
  struct ContractRank3TensorsFunctor<GPUDevice, T> {
    void operator()(OpKernelContext* ctx, const Tensor& input0,
      ...
      if (UseCuTensorEinsum()) {
        ...
        LaunchBatchMatMul<GPUDevice, T>::Launch(
            ctx, lhs, rhs, /*adj_x=*/false,
            /*adj_y=*/false, trans_x, trans_y, bcast, use_autotune, output);
      }
    }
  };
  ...
  static Status ContractOperands(OpKernelContext* ctx,
    ...
    rank3_tensors_contraction_func(ctx, inputs[0], inputs[1], trans_x, trans_y,
                                   bcast, &output_reshaped, use_autotune);
...
struct EinsumGenericFunctor {
  void operator()(OpKernelContext* ctx,
    ...
    std::vector<EinsumHelper::DimensionType> label_types(features.label_types);
    ...
    LabelToDimSizes label_to_dim_sizes;
    bool use_autotune(features.use_autotune);
    ...
    OP_REQUIRES_OK(ctx, EinsumHelper::ContractOperands<Device, T>(
                            ...
                            use_autotune, &contraction_output_reshaped));
...
class EinsumOp : public OpKernel {
 public:
  explicit EinsumOp(OpKernelConstruction* c) : OpKernel(c) {
    ...
    InputFeatures.use_autotune = MatmulAutotuneEnable();
  }

  void Compute(OpKernelContext* ctx) override {
    functor::EinsumGenericFunctor<Device, T> einsum_generic_func;
...
class EinsumOp<GPUDevice, T> : public OpKernel {
 public:
  explicit EinsumOp(OpKernelConstruction* c) : OpKernel(c) {
    OP_REQUIRES_OK(c, c->GetAttr("equation", &(InputFeatures.equation)));
    OP_REQUIRES_OK(
        c, EinsumHelper::ParseEquationWithCuTensorSupport(InputFeatures));
    InputFeatures.use_autotune = MatmulAutotuneEnable();
  }

  void Compute(OpKernelContext* ctx) override {
    if (UseCuTensorEinsum() && InputFeatures.support_cutensor) {
      functor::EinsumCutensorGpuFunctor<T> einsum_cutensor_func;
      einsum_cutensor_func(ctx, InputFeatures);
    } else {
      functor::EinsumGenericFunctor<GPUDevice, T> einsum_generic_func;
      einsum_generic_func(ctx, InputFeatures);
    }
  }
