...
std::unique_ptr<llvm::Module> MaybeLoadLLVMFromFile(const HloModule* module, llvm::Module* llvm_module) {
  // If the xla_gpu_llvm_file option is set, be explicit if a file is used
  // and warn when a file is not used to ease catching typo in filename.

  std::string prefix = xla::FilenameFor(*module, "", "");
  std::string matched_filename;
  for (const string& full_filename :
       module->config().debug_options().xla_gpu_llvm_file()) {
    // To ease comparing many LLVM versions, accept different suffixes then
    // the original filename.
    auto filename = tensorflow::io::Basename(full_filename);
    if (absl::StartsWith(filename, prefix)) {
      matched_filename = full_filename;
      VLOG(0) << "RunBackend() - Will load LLVM from file: " << full_filename;
      break;
    }
  }
  if (module->config().debug_options().xla_gpu_llvm_file().size() > 0 &&
      matched_filename.empty()) {
    VLOG(0) << "RunBackend() - For module with prefix '" << prefix
  ...
  if (!matched_filename.empty()) {
    ...
    std::unique_ptr<llvm::Module> loaded_module = llvm::parseIRFile(matched_filename, err, context);
...
NVPTXCompiler::CompileTargetBinary(const HloModule* module,
  ...
  std::unique_ptr<llvm::Module> loaded_module = MaybeLoadLLVMFromFile(module, llvm_module);
