...
def _count_casts(mode, partition_graph):
  ...
  for node in partition_graph.node:
    if _is_cast_to_fp16(node.name, node.op):
      num_to_fp16 += 1
    if _is_cast_to_bf16(node.name, node.op):
      num_to_bf16 += 1
...
class AutoMixedPrecisionTest(test.TestCase, parameterized.TestCase):
  ...
  def _run_simple_loop_test(self, mode, inp, body, out):
    ...
    output_val_ref, output_val, cost_graph, _ = self._run(mode, a)
  ...
  def test_conv3d(self, mode):
    ...
    output_val_ref, output_val, cost_graph, _ = self._run(mode, output)
    node_map = _build_node_map(cost_graph.node)
    self._assert_output_f16(mode, node_map, 'Conv3D')
