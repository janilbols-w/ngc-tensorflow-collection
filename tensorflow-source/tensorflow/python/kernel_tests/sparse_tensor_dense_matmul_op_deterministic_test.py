...
class SparseTensorDenseMatmulOpDeterminismExceptionsTest(test.TestCase):
  ...
  ops are enabled for floating point types np.complex128 and np.float64. For
  other fp types np.float16, np.float32 and np.complex64, GPU support for
  determinism is implemented by super-accumulator approach though there is a
  flow for some extreme cases. See PR #47749.
  ...
  def testExceptionThrowing(self):
    with self.session(), test_util.force_gpu():
      for data_type in [np.float64, np.complex128]:
...
class SparseTensorDenseMatmulOpDeterministicTest(test.TestCase):
  """Test that SparseTensorDenseMatul operates reproducibly for all floating
     points types on CPU. On GPU, certain types operate deterministically.
  """

  def _gen_d9m_data(self, data_type):
    sparse_input, dense_input = _gen_data(
        m=2430, k=615, n=857, nnz=(1<<16)+243, row_occupied_rate=0.02,
        data_type=data_type, seed=123)
    return sparse_input, dense_input

  @test_util.run_in_graph_and_eager_modes
  def testForward_CPU(self):
    for data_type in [
        np.float16, np.float32, np.float64, np.complex64, np.complex128
    ]:  # skipping int32 and bfloat16
      sparse_input, dense_input = self._gen_d9m_data(data_type)
  ...
  @test_util.run_in_graph_and_eager_modes
  def testForward_GPU(self):
    gpu_supported_types = (np.float32, np.float16, np.complex64,)
