...
static Status GetMlirAllocationInfo(mlir::FuncOp func,
  ...
  for (int i = 0; i < func.getNumArguments(); i++) {
    for (const mlir::NamedAttribute& attr : func.getArgAttrs(i)) {
      TF_RET_CHECK(attr.first == "lmhlo.params" ||
                   ...
                   attr.first == "lmhlo.async_out_index");
    }
  }
  ...
  for (int i = 0; i < func.getNumArguments(); i++) {
    ...
    if (auto async_out_attr = func.getArgAttr(i, "lmhlo.async_out_index")) {
      CHECK_EQ(i, async_out_attr.cast<mlir::IntegerAttr>().getInt());
      allocations->at(i).set_maybe_live_out(true);
      // ShapeIndex{-1, ...} are used as (virtual) indices for AsyncOuts.
      auto& o = (*output_info)[ShapeIndex({-1, i})];
      o.allocation_index = i;
    }
  }
