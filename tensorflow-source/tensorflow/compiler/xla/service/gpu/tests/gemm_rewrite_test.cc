...
; CHECK-NEXT:    %y = f32[2,2]{1,0} parameter(1)
; CHECK-NEXT:    %x = f32[2,2]{1,0} parameter(0)
; CHECK-NEXT:    ROOT %custom-call = f32[2,2]{1,0} custom-call(%y, %x), custom_call_target="__cublas$gemm", backend_config="{\"alpha_real\":1,\"alpha_imag\":0,\"beta\":0,\"dot_dimension_numbers\":{\"lhs_contracting_dimensions\":[\"0\"],\"rhs_contracting_dimensions\":[\"1\"],\"lhs_batch_dimensions\":[],\"rhs_batch_dimensions\":[]},\"batch_size\":\"1\",\"activation_mode\":\"0\",\"selected_algorithm\":\"{{-?[0-9]+}}\"}"
      )");
}
...
TEST_F(GemmRewriteTest, AlphaSimpleRewriteBiasAddActivation) {
  const char* hlo_text = R"(
HloModule AlphaSimpleRewriteBiasAddActivation
...
ENTRY AddDotsFunc {
  ...
  broadcast.1 = f32[2,2]{1,0} broadcast(f32[2]{0} %bias), dimensions={1}
  add.1 = f32[2,2]{1,0} add(f32[2,2]{1,0} %dot_a_multiplied, f32[2,2]{1,0} %broadcast.1)
  constant.1 = f32[] constant(0)
  broadcast.2 = f32[2,2]{1,0} broadcast(f32[] %constant.1), dimensions={}
  ROOT max.1 = f32[2,2]{1,0} maximum(f32[2,2]{1,0} %add.1, f32[2,2]{1,0} %broadcast.2)
}
...
; CHECK-LABEL: ENTRY %AddDotsFunc (x: f32[2,2], y: f32[2,2], bias: f32[2]) -> f32[2,2] {
; CHECK-NEXT:    %x = f32[2,2]{1,0} parameter(0)
; CHECK-NEXT:    %y = f32[2,2]{1,0} parameter(1)
; CHECK-NEXT:    %bias = f32[2]{0} parameter(2)
; CHECK-NEXT:    ROOT %custom-call.2 = f32[2,2]{1,0} custom-call(%x, %y, %bias), custom_call_target="__cublas$gemm", backend_config="{\"alpha_real\":3,\"alpha_imag\":0,\"beta\":0,\"dot_dimension_numbers\":{\"lhs_contracting_dimensions\":[\"1\"],\"rhs_contracting_dimensions\":[\"0\"],\"lhs_batch_dimensions\":[],\"rhs_batch_dimensions\":[]},\"batch_size\":\"1\",\"activation_mode\":\"2\",\"selected_algorithm\":\"{{-?[0-9]+}}\"}"
      )");
  }
}
...
TEST_F(GemmRewriteTest, SimpleRewriteBiasAddActivation) {
  const char* hlo_text = R"(
HloModule Dot_BiasAddActivation_8x8_8x8F16

ENTRY %Dot_BiasAddActivation_8x8_8x8F16 (lhs.1: f16[8,8], rhs.3: f16[8,8], bias.5: f16[8]) -> f16[8,8] {
  %lhs.1 = f16[8,8]{1,0} parameter(0)
  %rhs.3 = f16[8,8]{1,0} parameter(1)
  %dot = f16[8,8]{1,0} dot(f16[8,8]{1,0} %lhs.1, f16[8,8]{1,0} %rhs.3), lhs_contracting_dims={0}, rhs_contracting_dims={0}
  %bias.5 = f16[8]{0} parameter(2)
  %broadcast.6 = f16[8,8]{1,0} broadcast(f16[8]{0} %bias.5), dimensions={1}
  %add.7 = f16[8,8]{1,0} add(f16[8,8]{1,0} %dot, f16[8,8]{1,0} %broadcast.6)
  %constant.8 = f16[] constant(0)
  %broadcast.9 = f16[8,8]{1,0} broadcast(f16[] %constant.8), dimensions={}
  ROOT %maximum.10 = f16[8,8]{1,0} maximum(f16[8,8]{1,0} %add.7, f16[8,8]{1,0} %broadcast.9)
}

)";
  /*The error margins need to be increased for cublas runs. Only 5 out of 64
   * values deviate by error margin used here. cublasLt error is bound
   * within 1e-5. */
  EXPECT_TRUE(RunAndCompare(hlo_text, ErrorSpec{5e-4, 4e-5}));
  if (tensorflow::EnableCublasLtGemm()) {
    MatchOptimizedHlo(hlo_text,
                      R"(

; CHECK-LABEL: ENTRY %Dot_BiasAddActivation_8x8_8x8F16 (lhs.1: f16[8,8], rhs.3: f16[8,8], bias.5: f16[8]) -> f16[8,8] {
; CHECK-NEXT:    %lhs.1 = f16[8,8]{1,0} parameter(0)
; CHECK-NEXT:    %rhs.3 = f16[8,8]{1,0} parameter(1)
; CHECK-NEXT:    %bias.5 = f16[8]{0} parameter(2)
; CHECK-NEXT:    ROOT %custom-call.2 = f16[8,8]{1,0} custom-call(%lhs.1, %rhs.3, %bias.5), custom_call_target="__cublas$gemm", backend_config="{\"alpha_real\":1,\"alpha_imag\":0,\"beta\":0,\"dot_dimension_numbers\":{\"lhs_contracting_dimensions\":[\"0\"],\"rhs_contracting_dimensions\":[\"0\"],\"lhs_batch_dimensions\":[],\"rhs_batch_dimensions\":[]},\"batch_size\":\"1\",\"activation_mode\":\"2\",\"selected_algorithm\":\"{{-?[0-9]+}}\"}"
      )");
  }
}

TEST_F(GemmRewriteTest, SimpleRewriteBiasAdd) {
  const char* hlo_text = R"(
HloModule Dot_BiasAdd_2x2_2x2F32

ENTRY %Dot_BiasAdd_2x2_2x2F32 (lhs.1: f32[2,2], rhs.3: f32[2,2], bias.5: f32[2]) -> f32[2,2] {
  %lhs.1 = f32[2,2]{1,0} parameter(0)
  %rhs.3 = f32[2,2]{1,0} parameter(1)
  %dot = f32[2,2]{1,0} dot(f32[2,2]{1,0} %lhs.1, f32[2,2]{1,0} %rhs.3), lhs_contracting_dims={0}, rhs_contracting_dims={0}
  %bias.5 = f32[2]{0} parameter(2)
  %broadcast.6 = f32[2,2]{1,0} broadcast(f32[2]{0} %bias.5), dimensions={1}
  ROOT %add.7 = f32[2,2]{1,0} add(f32[2,2]{1,0} %dot, f32[2,2]{1,0} %broadcast.6)
}
...
; CHECK-LABEL: ENTRY %Dot_BiasAdd_2x2_2x2F32 (lhs.1: f32[2,2], rhs.3: f32[2,2], bias.5: f32[2]) -> f32[2,2] {
; CHECK-NEXT:    %lhs.1 = f32[2,2]{1,0} parameter(0)
; CHECK-NEXT:    %rhs.3 = f32[2,2]{1,0} parameter(1)
; CHECK-NEXT:    %bias.5 = f32[2]{0} parameter(2)
; CHECK-NEXT:    ROOT %custom-call.1 = f32[2,2]{1,0} custom-call(%lhs.1, %rhs.3, %bias.5), custom_call_target="__cublas$gemm", backend_config="{\"alpha_real\":1,\"alpha_imag\":0,\"beta\":0,\"dot_dimension_numbers\":{\"lhs_contracting_dimensions\":[\"0\"],\"rhs_contracting_dimensions\":[\"0\"],\"lhs_batch_dimensions\":[],\"rhs_batch_dimensions\":[]},\"batch_size\":\"1\",\"activation_mode\":\"0\",\"selected_algorithm\":\"{{-?[0-9]+}}\"}"
      )");
  }
}

TEST_F(GemmRewriteTest, SimpleRewriteActivation) {
  const char* hlo_text = R"(
HloModule Dot_Activation_2x2_2x2F32

ENTRY %Dot_Activation_2x2_2x2F32 (lhs.1: f32[2,2], rhs.3: f32[2,2]) -> f32[2,2] {
  %lhs.1 = f32[2,2]{1,0} parameter(0)
  %rhs.3 = f32[2,2]{1,0} parameter(1)
  %dot = f32[2,2]{1,0} dot(f32[2,2]{1,0} %lhs.1, f32[2,2]{1,0} %rhs.3), lhs_contracting_dims={0}, rhs_contracting_dims={0}
  %constant.5 = f32[] constant(0)
  %broadcast.6 = f32[2,2]{1,0} broadcast(f32[] %constant.5), dimensions={}
  ROOT %maximum.7 = f32[2,2]{1,0} maximum(f32[2,2]{1,0} %dot, f32[2,2]{1,0} %broadcast.6)
}
...
; CHECK-LABEL: ENTRY %Dot_Activation_2x2_2x2F32 (lhs.1: f32[2,2], rhs.3: f32[2,2]) -> f32[2,2] {
; CHECK-NEXT:    %lhs.1 = f32[2,2]{1,0} parameter(0)
; CHECK-NEXT:    %rhs.3 = f32[2,2]{1,0} parameter(1)
; CHECK-NEXT:    ROOT %custom-call.1 = f32[2,2]{1,0} custom-call(%lhs.1, %rhs.3), custom_call_target="__cublas$gemm", backend_config="{\"alpha_real\":1,\"alpha_imag\":0,\"beta\":0,\"dot_dimension_numbers\":{\"lhs_contracting_dimensions\":[\"0\"],\"rhs_contracting_dimensions\":[\"0\"],\"lhs_batch_dimensions\":[],\"rhs_batch_dimensions\":[]},\"batch_size\":\"1\",\"activation_mode\":\"2\",\"selected_algorithm\":\"{{-?[0-9]+}}\"}"
      )");
  }
}
