...
bool ShouldCheckRedzone(const HloCustomCallInstruction* instr) {
  const HloModuleConfig& hlo_module_config = instr->GetModule()->config();
  return hlo_module_config.debug_options().xla_gpu_enable_redzone_check();
}
}  // namespace
...
GpuConvAlgorithmPicker::AutotuneOneConvRunner(
  ...
  *result.mutable_run_time() = tensorflow::proto_utils::ToDurationProto(
  ...
  bool input_output_allocator_redzone_clear = true;
  bool scratch_allocator_redzone_clear = true;
  if (ShouldCheckRedzone(instr)) {
    TF_ASSIGN_OR_RETURN(
        input_output_allocator_redzone_clear,
...
GpuConvAlgorithmPicker::PickBestAlgorithmNoCacheCuda(
  ...
  for (auto& runner_cache : runners) {
    ...
    if (result.has_failure() &&
      ...
      return InternalError("Redzone check failed.");
    }
